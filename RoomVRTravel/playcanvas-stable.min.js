;(function (root, factory) {
  ...
  /*
 PlayCanvas Engine v1.9.0 revision 7e0dfdc
 http://playcanvas.com
 Copyright 2011-2018 PlayCanvas Ltd. All rights reserved.
*/
var _typeLookup=function(){for(var e={},b="Array Object Function Date RegExp Float32Array".split(" "),a=0;a<b.length;a++)e["[object "+b[a]+"]"]=b[a].toLowerCase();return e}(),pc={version:"1.9.0",revision:"7e0dfdc",config:{},common:{},apps:{},data:{},unpack:function(){console.warn("pc.unpack has been deprecated and will be removed shortly. Please update your code.")},makeArray:function(e){var b,a=[],c=e.length;for(b=0;b<c;++b)a.push(e[b]);return a},type:function(e){if(null===
...
containsPoint:function(a){var b=this.getMin(),d=this.getMax();return a.x<b.x||a.x>d.x||a.y<b.y||a.y>d.y||a.z<b.z||a.z>d.z?!1:!0},setFromTransformedAabb:function(a,b){var d=this.center,c=this.halfExtents,f=a.center,g=a.halfExtents;b=b.data;var k=b[0],e=b[4],r=b[8],t=b[1],v=b[5],y=b[9],w=b[2],u=b[6],x=b[10],B=Math.abs(k),C=Math.abs(e),A=Math.abs(r),z=Math.abs(t),E=Math.abs(v),J=Math.abs(y),H=Math.abs(w),G=Math.abs(u),D=Math.abs(x);d.set(b[12]+k*f.x+e*f.y+r*f.z,b[13]+t*f.x+v*f.y+y*f.z,b[14]+w*f.x+u*
f.y+x*f.z);c.set(B*g.x+C*g.y+A*g.z,z*g.x+E*g.y+J*g.z,H*g.x+G*g.y+D*g.z)},compute:function(a){for(var d=e.set(a[0],a[1],a[2]),c=b.set(a[0],a[1],a[2]),f=a.length/3,g=1;g<f;g++){var n=a[3*g+0],q=a[3*g+1],m=a[3*g+2];n<d.x&&(d.x=n);q<d.y&&(d.y=q);m<d.z&&(d.z=m);n>c.x&&(c.x=n);q>c.y&&(c.y=q);m>c.z&&(c.z=m)}this.setMinMax(d,c)},intersectsBoundingSphere:function(a){return this._distanceToBoundingSphereSq(a)<=a.radius*a.radius?!0:!1},_distanceToBoundingSphereSq:function(a){for(var b=this.getMin(),d=this.getMax(),
...
0,0,d,c,0,0,e,f);return h}return a},c=function(a,c){var d;this.canvas=a;this.indexBuffer=this.shader=null;this.vertexBuffers=[];this.vbOffsets=[];this._enableAutoInstancing=!1;this.autoInstancingMaxObjects=16384;this.attributesInvalidated=!0;this.boundElementBuffer=this.boundBuffer=null;this.instancedAttribs={};this.enabledAttributes={};this.activeFramebuffer=this.transformFeedbackBuffer=null;this.textureUnit=0;this.textureUnits=[];this._maxPixelRatio=1;this.feedback=this.renderTarget=null;this._height=
this._width=0;this.updateClientRect();this.shaders=[];this.buffers=[];this.textures=[];this.targets=[];this.contextLost=!1;a.addEventListener("webglcontextlost",function(a){a.preventDefault();this.contextLost=!0;this.fire("devicelost")}.bind(this),!1);a.addEventListener("webglcontextrestored",function(){this.initializeContext();this.contextLost=!1;this.fire("devicerestored")}.bind(this),!1);var f=c&&void 0!==c.preferWebGl2?c.preferWebGl2:!0,l=f?["webgl2","experimental-webgl2","webgl","experimental-webgl"]:
["webgl","experimental-webgl"],p=null;c=c||{};c.stencil=!0;for(d=0;d<l.length;d++){try{p=a.getContext(l[d],c)}catch(v){}if(p){this.webgl2=f&&2>d;break}}if(!p)throw Error("WebGL not supported");this.gl=p;this.initializeExtensions();this.initializeCapabilities();this.initializeRenderState();for(d=0;d<this.maxCombinedTextures;d++)this.textureUnits.push([null,null,null]);this.defaultClearOptions={color:[0,0,0,1],depth:1,stencil:0,flags:pc.CLEARFLAG_COLOR|pc.CLEARFLAG_DEPTH};this.glAddress=[p.REPEAT,p.CLAMP_TO_EDGE,
p.MIRRORED_REPEAT];this.glBlendEquation=[p.FUNC_ADD,p.FUNC_SUBTRACT,p.FUNC_REVERSE_SUBTRACT,this.webgl2?p.MIN:this.extBlendMinmax?this.extBlendMinmax.MIN_EXT:p.FUNC_ADD,this.webgl2?p.MAX:this.extBlendMinmax?this.extBlendMinmax.MAX_EXT:p.FUNC_ADD];this.glBlendFunction=[p.ZERO,p.ONE,p.SRC_COLOR,p.ONE_MINUS_SRC_COLOR,p.DST_COLOR,p.ONE_MINUS_DST_COLOR,p.SRC_ALPHA,p.SRC_ALPHA_SATURATE,p.ONE_MINUS_SRC_ALPHA,p.DST_ALPHA,p.ONE_MINUS_DST_ALPHA];this.glComparison=[p.NEVER,p.LESS,p.EQUAL,p.LEQUAL,p.GREATER,
p.NOTEQUAL,p.GEQUAL,p.ALWAYS];this.glStencilOp=[p.KEEP,p.ZERO,p.REPLACE,p.INCR,p.INCR_WRAP,p.DECR,p.DECR_WRAP,p.INVERT];this.glClearFlag=[0,p.COLOR_BUFFER_BIT,p.DEPTH_BUFFER_BIT,p.COLOR_BUFFER_BIT|p.DEPTH_BUFFER_BIT,p.STENCIL_BUFFER_BIT,p.STENCIL_BUFFER_BIT|p.COLOR_BUFFER_BIT,p.STENCIL_BUFFER_BIT|p.DEPTH_BUFFER_BIT,p.STENCIL_BUFFER_BIT|p.COLOR_BUFFER_BIT|p.DEPTH_BUFFER_BIT];this.glCull=[0,p.BACK,p.FRONT,p.FRONT_AND_BACK];this.glFilter=[p.NEAREST,p.LINEAR,p.NEAREST_MIPMAP_NEAREST,p.NEAREST_MIPMAP_LINEAR,
p.LINEAR_MIPMAP_NEAREST,p.LINEAR_MIPMAP_LINEAR];this.glPrimitive=[p.POINTS,p.LINES,p.LINE_LOOP,p.LINE_STRIP,p.TRIANGLES,p.TRIANGLE_STRIP,p.TRIANGLE_FAN];this.glType=[p.BYTE,p.UNSIGNED_BYTE,p.SHORT,p.UNSIGNED_SHORT,p.INT,p.UNSIGNED_INT,p.FLOAT];this.targetToSlot={};this.targetToSlot[p.TEXTURE_2D]=0;this.targetToSlot[p.TEXTURE_CUBE_MAP]=1;this.targetToSlot[p.TEXTURE_3D]=2;var h,n,q,m,r;this.commitFunction=[];this.commitFunction[pc.UNIFORMTYPE_BOOL]=function(a,b){a.value!==b&&(p.uniform1i(a.locationId,
b),a.value=b)};this.commitFunction[pc.UNIFORMTYPE_INT]=this.commitFunction[pc.UNIFORMTYPE_BOOL];this.commitFunction[pc.UNIFORMTYPE_FLOAT]=function(a,b){a.value!==b&&(p.uniform1f(a.locationId,b),a.value=b)};this.commitFunction[pc.UNIFORMTYPE_VEC2]=function(a,b){r=a.value;h=b[0];n=b[1];if(r[0]!==h||r[1]!==n)p.uniform2fv(a.locationId,b),r[0]=h,r[1]=n};this.commitFunction[pc.UNIFORMTYPE_VEC3]=function(a,b){r=a.value;h=b[0];n=b[1];q=b[2];if(r[0]!==h||r[1]!==n||r[2]!==q)p.uniform3fv(a.locationId,b),r[0]=
h,r[1]=n,r[2]=q};this.commitFunction[pc.UNIFORMTYPE_VEC4]=function(a,b){r=a.value;h=b[0];n=b[1];q=b[2];m=b[3];if(r[0]!==h||r[1]!==n||r[2]!==q||r[3]!==m)p.uniform4fv(a.locationId,b),r[0]=h,r[1]=n,r[2]=q,r[3]=m};this.commitFunction[pc.UNIFORMTYPE_IVEC2]=function(a,b){r=a.value;h=b[0];n=b[1];if(r[0]!==h||r[1]!==n)p.uniform2iv(a.locationId,b),r[0]=h,r[1]=n};this.commitFunction[pc.UNIFORMTYPE_BVEC2]=this.commitFunction[pc.UNIFORMTYPE_IVEC2];this.commitFunction[pc.UNIFORMTYPE_IVEC3]=function(a,b){r=a.value;
h=b[0];n=b[1];q=b[2];if(r[0]!==h||r[1]!==n||r[2]!==q)p.uniform3iv(a.locationId,b),r[0]=h,r[1]=n,r[2]=q};this.commitFunction[pc.UNIFORMTYPE_BVEC3]=this.commitFunction[pc.UNIFORMTYPE_IVEC3];this.commitFunction[pc.UNIFORMTYPE_IVEC4]=function(a,b){r=a.value;h=b[0];n=b[1];q=b[2];m=b[3];if(r[0]!==h||r[1]!==n||r[2]!==q||r[3]!==m)p.uniform4iv(a.locationId,b),r[0]=h,r[1]=n,r[2]=q,r[3]=m};this.commitFunction[pc.UNIFORMTYPE_BVEC4]=this.commitFunction[pc.UNIFORMTYPE_IVEC4];this.commitFunction[pc.UNIFORMTYPE_MAT2]=
function(a,b){p.uniformMatrix2fv(a.locationId,!1,b)};this.commitFunction[pc.UNIFORMTYPE_MAT3]=function(a,b){p.uniformMatrix3fv(a.locationId,!1,b)};this.commitFunction[pc.UNIFORMTYPE_MAT4]=function(a,b){p.uniformMatrix4fv(a.locationId,!1,b)};this.commitFunction[pc.UNIFORMTYPE_FLOATARRAY]=function(a,b){p.uniform1fv(a.locationId,b)};this.scope=new pc.ScopeSpace("Device");this.programLib=new pc.ProgramLibrary(this);for(var t in pc.programlib)this.programLib.register(t,pc.programlib[t]);pc.events.attach(this);
this.supportsBoneTextures=this.extTextureFloat&&0<this.maxVertexTextures;this.useTexCubeLod=this.extTextureLod&&16>this.maxTextures;this.boneLimit=Math.floor((this.vertexUniformsCount-16-8-1-16)/4);this.boneLimit=Math.min(this.boneLimit,128);"Mali-450 MP"===this.unmaskedRenderer&&(this.boneLimit=34);"Apple A8 GPU"===this.unmaskedRenderer&&(this.forceCpuParticles=!0);this._shaderSwitchesPerFrame=this._drawCallsPerFrame=0;this._primsPerFrame=[];for(d=pc.PRIMITIVE_POINTS;d<=pc.PRIMITIVE_TRIFAN;d++)this._primsPerFrame[d]=
0;this._renderTargetCreationTime=0;this._vram={tex:0,vb:0,ib:0};this._shaderStats={vsCompiled:0,fsCompiled:0,linked:0,materialShaders:0,compileTime:0};this.constantTexSource=this.scope.resolve("source");this.textureFloatRenderable=this.extTextureFloat?this.webgl2?!!this.extColorBufferFloat:e(p,p.FLOAT):!1;this.textureHalfFloatRenderable=this.extTextureHalfFloat?this.webgl2?!!this.extColorBufferFloat:e(p,this.extTextureHalfFloat.HALF_FLOAT_OES):!1;this.textureFloatHighPrecision=b(this)};Object.assign(c.prototype,
{getPrecision:function(){var a=this.gl,b="highp";if(a.getShaderPrecisionFormat){var c=a.getShaderPrecisionFormat(a.VERTEX_SHADER,a.HIGH_FLOAT),k=a.getShaderPrecisionFormat(a.VERTEX_SHADER,a.MEDIUM_FLOAT),l=a.getShaderPrecisionFormat(a.FRAGMENT_SHADER,a.HIGH_FLOAT),a=a.getShaderPrecisionFormat(a.FRAGMENT_SHADER,a.MEDIUM_FLOAT),k=0<k.precision&&0<a.precision;0<c.precision&&0<l.precision||(b=k?"mediump":"lowp")}return b},initializeExtensions:function(){var a=this.gl,b,c=a.getSupportedExtensions(),k=
function(){for(var b=null,d=0;d<arguments.length;d++)-1!==c.indexOf(arguments[d])&&(b=a.getExtension(arguments[d]));return b};if(this.webgl2)this.extVertexArrayObject=this.extUintElement=this.extTextureLod=this.extTextureHalfFloatLinear=this.extTextureHalfFloat=this.extTextureFloat=this.extStandardDerivatives=this.extInstancing=this.extDrawBuffers=this.extBlendMinmax=!0,this.extColorBufferFloat=k("EXT_color_buffer_float");else{this.extBlendMinmax=k("EXT_blend_minmax");this.extDrawBuffers=k("EXT_draw_buffers");
if(this.extInstancing=k("ANGLE_instanced_arrays"))b=this.extInstancing,a.drawArraysInstanced=b.drawArraysInstancedANGLE.bind(b),a.drawElementsInstanced=b.drawElementsInstancedANGLE.bind(b),a.vertexAttribDivisor=b.vertexAttribDivisorANGLE.bind(b);this.extStandardDerivatives=k("OES_standard_derivatives");this.extTextureFloat=k("OES_texture_float");this.extTextureHalfFloat=k("OES_texture_half_float");this.extTextureHalfFloatLinear=k("OES_texture_half_float_linear");this.extTextureLod=k("EXT_shader_texture_lod");
this.extUintElement=k("OES_element_index_uint");if(this.extVertexArrayObject=k("OES_vertex_array_object"))b=this.extVertexArrayObject,a.createVertexArray=b.createVertexArrayOES.bind(b),a.deleteVertexArray=b.deleteVertexArrayOES.bind(b),a.isVertexArray=b.isVertexArrayOES.bind(b),a.bindVertexArray=b.bindVertexArrayOES.bind(b);this.extColorBufferFloat=null}this.extDebugRendererInfo=k("WEBGL_debug_renderer_info");this.extTextureFloatLinear=k("OES_texture_float_linear");this.extTextureFilterAnisotropic=
k("EXT_texture_filter_anisotropic","WEBKIT_EXT_texture_filter_anisotropic");this.extCompressedTextureETC1=k("WEBGL_compressed_texture_etc1");this.extCompressedTextureETC=k("WEBGL_compressed_texture_etc");this.extCompressedTexturePVRTC=k("WEBGL_compressed_texture_pvrtc","WEBKIT_WEBGL_compressed_texture_pvrtc");this.extCompressedTextureS3TC=k("WEBGL_compressed_texture_s3tc","WEBKIT_WEBGL_compressed_texture_s3tc");this.extParallelShaderCompile=k("KHR_parallel_shader_compile")},initializeCapabilities:function(){var a=
...
c.fullscreenQuadVS,c.outputTex2DPS,"outputTex2D")),this.constantTexSource.setValue(a._colorBuffer),pc.drawQuadWithShader(this,b,this._copyShader);return!0},updateBegin:function(){var a=this.gl;this.boundElementBuffer=this.boundBuffer=null;var b=this.renderTarget;if(b)if(b._glFrameBuffer)this.setFramebuffer(b._glFrameBuffer);else{b._device=this;b._glFrameBuffer=a.createFramebuffer();this.setFramebuffer(b._glFrameBuffer);var c=b._colorBuffer;c&&(c._glTexture||(c._width=Math.min(c.width,this.maxRenderBufferSize),
c._height=Math.min(c.height,this.maxRenderBufferSize),this.setTexture(c,0)),a.framebufferTexture2D(a.FRAMEBUFFER,a.COLOR_ATTACHMENT0,c._cubemap?a.TEXTURE_CUBE_MAP_POSITIVE_X+b._face:a.TEXTURE_2D,c._glTexture,0));var k=b._depthBuffer;k&&this.webgl2?(k._glTexture||(k._width=Math.min(k.width,this.maxRenderBufferSize),k._height=Math.min(k.height,this.maxRenderBufferSize),this.setTexture(k,0)),b._stencil?a.framebufferTexture2D(a.FRAMEBUFFER,a.DEPTH_STENCIL_ATTACHMENT,k._cubemap?a.TEXTURE_CUBE_MAP_POSITIVE_X+
b._face:a.TEXTURE_2D,b._depthBuffer._glTexture,0):a.framebufferTexture2D(a.FRAMEBUFFER,a.DEPTH_ATTACHMENT,k._cubemap?a.TEXTURE_CUBE_MAP_POSITIVE_X+b._face:a.TEXTURE_2D,b._depthBuffer._glTexture,0)):!b._depth||1<b._samples&&this.webgl2||(b._glDepthBuffer||(b._glDepthBuffer=a.createRenderbuffer()),a.bindRenderbuffer(a.RENDERBUFFER,b._glDepthBuffer),b._stencil?(a.renderbufferStorage(a.RENDERBUFFER,a.DEPTH_STENCIL,b.width,b.height),a.framebufferRenderbuffer(a.FRAMEBUFFER,a.DEPTH_STENCIL_ATTACHMENT,a.RENDERBUFFER,
...
a.RENDERBUFFER,b._glMsaaDepthBuffer)):(a.renderbufferStorageMultisample(a.RENDERBUFFER,b._samples,a.DEPTH_COMPONENT32F,b.width,b.height),a.framebufferRenderbuffer(a.FRAMEBUFFER,a.DEPTH_ATTACHMENT,a.RENDERBUFFER,b._glMsaaDepthBuffer))));this.targets.push(b)}else this.setFramebuffer(null)},updateEnd:function(){var a=this.gl,b=this.renderTarget;if(b){var c=b._colorBuffer;c&&c._glTexture&&c.mipmaps&&c._pot&&(this.activeTexture(this.maxCombinedTextures-1),this.bindTexture(c),a.generateMipmap(c._glTarget));
this.webgl2&&1<b._samples&&b.autoResolve&&b.resolve()}},initializeTexture:function(a){var b=this.gl,c;a._glTexture=b.createTexture();a._glTarget=a._cubemap?b.TEXTURE_CUBE_MAP:a._volume?b.TEXTURE_3D:b.TEXTURE_2D;switch(a._format){case pc.PIXELFORMAT_A8:a._glFormat=b.ALPHA;a._glInternalFormat=b.ALPHA;a._glPixelType=b.UNSIGNED_BYTE;break;case pc.PIXELFORMAT_L8:a._glFormat=b.LUMINANCE;a._glInternalFormat=b.LUMINANCE;a._glPixelType=b.UNSIGNED_BYTE;break;case pc.PIXELFORMAT_L8_A8:a._glFormat=b.LUMINANCE_ALPHA;
...
a._glPixelType=b.UNSIGNED_SHORT);break;case pc.PIXELFORMAT_DEPTHSTENCIL:a._glFormat=b.DEPTH_STENCIL;a._glInternalFormat=b.DEPTH24_STENCIL8;a._glPixelType=b.UNSIGNED_INT_24_8;break;case pc.PIXELFORMAT_111110F:a._glFormat=b.RGB;a._glInternalFormat=b.R11F_G11F_B10F;a._glPixelType=b.FLOAT;break;case pc.PIXELFORMAT_SRGB:a._glFormat=b.RGB;a._glInternalFormat=b.SRGB8;a._glPixelType=b.UNSIGNED_BYTE;break;case pc.PIXELFORMAT_SRGBA:a._glFormat=b.RGBA,a._glInternalFormat=b.SRGB8_ALPHA8,a._glPixelType=b.UNSIGNED_BYTE}this.textures.push(a)},
destroyTexture:function(a){if(a._glTexture){var b=this.textures.indexOf(a);-1!==b&&this.textures.splice(b,1);for(var c in this.scope.variables)b=this.scope.variables[c],b.value===a&&(b.value=null);for(c=0;c<this.textureUnits.length;c++)for(var b=this.textureUnits[c],d=0;d<b.length;d++)b[d]===a._glTexture&&(b[d]=null);this.gl.deleteTexture(a._glTexture);delete a._glTexture;delete a._glTarget;delete a._glFormat;delete a._glInternalFormat;delete a._glPixelType;this._vram.tex-=a._gpuSize}},setUnpackFlipY:function(a){if(this.unpackFlipY!==
a){this.unpackFlipY=a;var b=this.gl;b.pixelStorei(b.UNPACK_FLIP_Y_WEBGL,a)}},setUnpackPremultiplyAlpha:function(a){if(this.unpackPremultiplyAlpha!==a){this.unpackPremultiplyAlpha=a;var b=this.gl;b.pixelStorei(b.UNPACK_PREMULTIPLY_ALPHA_WEBGL,a)}},uploadTexture:function(b){var c=this.gl;if(b._needsUpload||!(b._needsMipmapsUpload&&b._mipmapsUploaded||!b._pot)){for(var d=0,k,l;b._levels[d]||0===d;){if(b._needsUpload||0!==d){if(d&&(!b._needsMipmapsUpload||!b._mipmaps))break;k=b._levels[d];1!=d||b._compressed||
(c.generateMipmap(b._glTarget),b._mipmapsUploaded=!0);if(b._cubemap){var e;if(k[0]instanceof HTMLCanvasElement||k[0]instanceof HTMLImageElement||k[0]instanceof HTMLVideoElement)for(e=0;6>e;e++)b._levelsUpdated[0][e]&&(l=k[e],l instanceof HTMLImageElement&&(l.width>this.maxCubeMapSize||l.height>this.maxCubeMapSize)&&(l=a(l,this.maxCubeMapSize),0===d&&(b.width=l.width,b.height=l.height)),this.setUnpackFlipY(!1),this.setUnpackPremultiplyAlpha(b._premultiplyAlpha),c.texImage2D(c.TEXTURE_CUBE_MAP_POSITIVE_X+
e,d,b._glInternalFormat,b._glFormat,b._glPixelType,l));else for(l=1/Math.pow(2,d),e=0;6>e;e++)if(b._levelsUpdated[0][e]){var h=k[e];b._compressed?c.compressedTexImage2D(c.TEXTURE_CUBE_MAP_POSITIVE_X+e,d,b._glInternalFormat,Math.max(b._width*l,1),Math.max(b._height*l,1),0,h):(this.setUnpackFlipY(!1),this.setUnpackPremultiplyAlpha(b._premultiplyAlpha),c.texImage2D(c.TEXTURE_CUBE_MAP_POSITIVE_X+e,d,b._glInternalFormat,Math.max(b._width*l,1),Math.max(b._height*l,1),0,b._glFormat,b._glPixelType,h))}}else b._volume?
(l=1/Math.pow(2,d),b._compressed?c.compressedTexImage3D(c.TEXTURE_3D,d,b._glInternalFormat,Math.max(b._width*l,1),Math.max(b._height*l,1),Math.max(b._depth*l,1),0,k):(this.setUnpackFlipY(!1),this.setUnpackPremultiplyAlpha(b._premultiplyAlpha),c.texImage3D(c.TEXTURE_3D,d,b._glInternalFormat,Math.max(b._width*l,1),Math.max(b._height*l,1),Math.max(b._depth*l,1),0,b._glFormat,b._glPixelType,k))):(k instanceof HTMLCanvasElement||k instanceof HTMLImageElement||k instanceof HTMLVideoElement?(k instanceof
HTMLImageElement&&(k.width>this.maxTextureSize||k.height>this.maxTextureSize)&&(k=a(k,this.maxTextureSize),0===d&&(b.width=k.width,b.height=k.height)),this.setUnpackFlipY(b._flipY),this.setUnpackPremultiplyAlpha(b._premultiplyAlpha),c.texImage2D(c.TEXTURE_2D,d,b._glInternalFormat,b._glFormat,b._glPixelType,k)):(l=1/Math.pow(2,d),b._compressed?c.compressedTexImage2D(c.TEXTURE_2D,d,b._glInternalFormat,Math.max(b._width*l,1),Math.max(b._height*l,1),0,k):(this.setUnpackFlipY(!1),this.setUnpackPremultiplyAlpha(b._premultiplyAlpha),
c.texImage2D(c.TEXTURE_2D,d,b._glInternalFormat,Math.max(b._width*l,1),Math.max(b._height*l,1),0,b._glFormat,b._glPixelType,k))),b._mipmapsUploaded=0===d?!1:!0)}d++}if(b._needsUpload)if(b._cubemap)for(d=0;6>d;d++)b._levelsUpdated[0][d]=!1;else b._levelsUpdated[0]=!1;!b._compressed&&b._mipmaps&&b._needsMipmapsUpload&&b._pot&&1===b._levels.length&&(c.generateMipmap(b._glTarget),b._mipmapsUploaded=!0);b._gpuSize&&(this._vram.tex-=b._gpuSize);b._gpuSize=b.gpuSize;this._vram.tex+=b._gpuSize}},activeTexture:function(a){this.textureUnit!==
a&&(this.gl.activeTexture(this.gl.TEXTURE0+a),this.textureUnit=a)},bindTexture:function(a){var b=a._glTarget;a=a._glTexture;var c=this.textureUnit,d=this.targetToSlot[b];this.textureUnits[c][d]!==a&&(this.gl.bindTexture(b,a),this.textureUnits[c][d]=a)},bindTextureOnUnit:function(a,b){var c=a._glTarget,d=a._glTexture,f=this.targetToSlot[c];this.textureUnits[b][f]!==d&&(this.activeTexture(b),this.gl.bindTexture(c,d),this.textureUnits[b][f]=d)},setTextureParameters:function(a){var b=this.gl,c=a._parameterFlags,
...
b.texParameteri(d,b.TEXTURE_WRAP_S,this.glAddress[a._pot?a._addressU:pc.ADDRESS_CLAMP_TO_EDGE]));c&8&&(this.webgl2?b.texParameteri(d,b.TEXTURE_WRAP_T,this.glAddress[a._addressV]):b.texParameteri(d,b.TEXTURE_WRAP_T,this.glAddress[a._pot?a._addressV:pc.ADDRESS_CLAMP_TO_EDGE]));c&16&&this.webgl2&&b.texParameteri(d,b.TEXTURE_WRAP_R,this.glAddress[a._addressW]);c&32&&this.webgl2&&b.texParameteri(d,b.TEXTURE_COMPARE_MODE,a._compareOnRead?b.COMPARE_REF_TO_TEXTURE:b.NONE);c&64&&this.webgl2&&b.texParameteri(d,
b.TEXTURE_COMPARE_FUNC,this.glComparison[a._compareFunc]);c&128&&(c=this.extTextureFilterAnisotropic)&&b.texParameterf(d,c.TEXTURE_MAX_ANISOTROPY_EXT,Math.max(1,Math.min(Math.round(a._anisotropy),this.maxAnisotropy)))},setTexture:function(a,b){a._glTexture||this.initializeTexture(a);if(0<a._parameterFlags||a._needsUpload||a._needsMipmapsUpload){if(this.activeTexture(b),this.bindTexture(a),a._parameterFlags&&(this.setTextureParameters(a),a._parameterFlags=0),a._needsUpload||a._needsMipmapsUpload)this.uploadTexture(a),
a._needsUpload=!1,a._needsMipmapsUpload=!1}else this.bindTextureOnUnit(a,b)},setBuffers:function(a){var b=this.gl,c,d,l,e,h=this.shader.attributes;if(this.attributesInvalidated){for(var n=0,q=h.length;n<q;n++)c=h[n],d=c.scopeId.value,null!==d&&(l=this.vertexBuffers[d.stream],e=this.vbOffsets[d.stream]||0,l=l.bufferId,this.boundBuffer!==l&&(b.bindBuffer(b.ARRAY_BUFFER,l),this.boundBuffer=l),c=c.locationId,this.enabledAttributes[c]||(b.enableVertexAttribArray(c),this.enabledAttributes[c]=!0),b.vertexAttribPointer(c,
d.numComponents,this.glType[d.dataType],d.normalize,d.stride,d.offset+e),1===d.stream&&1<a?this.instancedAttribs[c]||(b.vertexAttribDivisor(c,1),this.instancedAttribs[c]=!0):this.instancedAttribs[c]&&(b.vertexAttribDivisor(c,0),this.instancedAttribs[c]=!1));this.attributesInvalidated=!1}l=this.indexBuffer?this.indexBuffer.bufferId:null;this.boundElementBuffer!==l&&(b.bindBuffer(b.ELEMENT_ARRAY_BUFFER,l),this.boundElementBuffer=l)},draw:function(a,b){var c=this.gl,d,f,e,h,n,q,m,r;d=this.shader;r=d.samplers;
var t=d.uniforms;1<b&&(this.boundBuffer=null,this.attributesInvalidated=!0);this.setBuffers(b);var v=0;d=0;for(e=r.length;d<e;d++)if(h=r[d],n=h.scopeId.value)if(n instanceof pc.Texture)q=n,this.setTexture(q,v),h.slot!==v&&(c.uniform1i(h.locationId,v),h.slot=v),v++;else{h.array.length=0;m=n.length;for(f=0;f<m;f++)q=n[f],this.setTexture(q,v),h.array[f]=v,v++;c.uniform1iv(h.locationId,h.array)}d=0;for(e=t.length;d<e;d++)if(r=t[d],f=r.scopeId,h=r.version,n=f.versionObject.version,h.globalId!==n.globalId||
h.revision!==n.revision)if(h.globalId=n.globalId,h.revision=n.revision,null!==f.value)this.commitFunction[r.dataType](r,f.value);this.webgl2&&this.transformFeedbackBuffer&&(c.bindBufferBase(c.TRANSFORM_FEEDBACK_BUFFER,0,this.transformFeedbackBuffer.bufferId),c.beginTransformFeedback(c.POINTS));d=this.glPrimitive[a.type];e=a.count;a.indexed?(r=this.indexBuffer,t=r.glFormat,r=a.base*r.bytesPerIndex,1<b?c.drawElementsInstanced(d,e,t,r,b):c.drawElements(d,e,t,r)):(t=a.base,1<b?c.drawArraysInstanced(d,
t,e,b):c.drawArrays(d,t,e));this.webgl2&&this.transformFeedbackBuffer&&(c.endTransformFeedback(),c.bindBufferBase(c.TRANSFORM_FEEDBACK_BUFFER,0,null))},clear:function(a){var b=this.defaultClearOptions;a=a||b;var c=void 0==a.flags?b.flags:a.flags;if(0!==c){var d=this.gl;if(c&pc.CLEARFLAG_COLOR){var e=void 0==a.color?b.color:a.color;this.setClearColor(e[0],e[1],e[2],e[3])}c&pc.CLEARFLAG_DEPTH&&(this.setClearDepth(void 0==a.depth?b.depth:a.depth),this.depthWrite||d.depthMask(!0));c&pc.CLEARFLAG_STENCIL&&
this.setClearStencil(void 0==a.stencil?b.stencil:a.stencil);d.clear(this.glClearFlag[c]);c&pc.CLEARFLAG_DEPTH&&(this.depthWrite||d.depthMask(!1))}},readPixels:function(a,b,c,k,e){var d=this.gl;d.readPixels(a,b,c,k,d.RGBA,d.UNSIGNED_BYTE,e)},setClearDepth:function(a){a!==this.clearDepth&&(this.gl.clearDepth(a),this.clearDepth=a)},setClearColor:function(a,b,c,k){if(a!==this.clearRed||b!==this.clearGreen||c!==this.clearBlue||k!==this.clearAlpha)this.gl.clearColor(a,b,c,k),this.clearRed=a,this.clearGreen=
b,this.clearBlue=c,this.clearAlpha=k},setClearStencil:function(a){a!==this.clearStencil&&(this.gl.clearStencil(a),this.clearStencil=a)},setRenderTarget:function(a){this.renderTarget=a},getRenderTarget:function(){return this.renderTarget},getDepthTest:function(){return this.depthTest},setDepthTest:function(a){if(this.depthTest!==a){var b=this.gl;a?b.enable(b.DEPTH_TEST):b.disable(b.DEPTH_TEST);this.depthTest=a}},setDepthFunc:function(a){this.depthFunc!==a&&(this.gl.depthFunc(this.glComparison[a]),
this.depthFunc=a)},getDepthWrite:function(){return this.depthWrite},setDepthWrite:function(a){this.depthWrite!==a&&(this.gl.depthMask(a),this.depthWrite=a)},setColorWrite:function(a,b,c,k){if(this.writeRed!==a||this.writeGreen!==b||this.writeBlue!==c||this.writeAlpha!==k)this.gl.colorMask(a,b,c,k),this.writeRed=a,this.writeGreen=b,this.writeBlue=c,this.writeAlpha=k},setAlphaToCoverage:function(a){this.webgl2&&this.alphaToCoverage!==a&&((this.alphaToCoverage=a)?this.gl.enable(this.gl.SAMPLE_ALPHA_TO_COVERAGE):
...
c||this.stencilFailBack!==a||this.stencilZfailBack!==b||this.stencilZpassBack!==c)this.gl.stencilOp(this.glStencilOp[a],this.glStencilOp[b],this.glStencilOp[c]),this.stencilFailFront=this.stencilFailBack=a,this.stencilZfailFront=this.stencilZfailBack=b,this.stencilZpassFront=this.stencilZpassBack=c;if(this.stencilWriteMaskFront!==k||this.stencilWriteMaskBack!==k)this.gl.stencilMask(k),this.stencilWriteMaskBack=this.stencilWriteMaskFront=k},setStencilOperationFront:function(a,b,c,k){if(this.stencilFailFront!==
...
this.glStencilOp[a],this.glStencilOp[b],this.glStencilOp[c]),this.stencilFailBack=a,this.stencilZfailBack=b,this.stencilZpassBack=c;this.stencilWriteMaskBack!==k&&(this.gl.stencilMaskSeparate(this.gl.BACK,k),this.stencilWriteMaskBack=k)},setBlendFunction:function(a,b){if(this.blendSrc!==a||this.blendDst!==b||this.separateAlphaBlend)this.gl.blendFunc(this.glBlendFunction[a],this.glBlendFunction[b]),this.blendSrc=a,this.blendDst=b,this.separateAlphaBlend=!1},setBlendFunctionSeparate:function(a,b,c,
k){this.blendSrc===a&&this.blendDst===b&&this.blendSrcAlpha===c&&this.blendDstAlpha===k&&this.separateAlphaBlend||(this.gl.blendFuncSeparate(this.glBlendFunction[a],this.glBlendFunction[b],this.glBlendFunction[c],this.glBlendFunction[k]),this.blendSrc=a,this.blendDst=b,this.blendSrcAlpha=c,this.blendDstAlpha=k,this.separateAlphaBlend=!0)},setBlendEquation:function(a){if(this.blendEquation!==a||this.separateAlphaEquation)this.gl.blendEquation(this.glBlendEquation[a]),this.blendEquation=a,this.separateAlphaEquation=
...
f?[.9,.85,.7,.4,.25]:[512,128,32,8,2],y=[64,32,16,8,4],w,u,x;u=l===pc.PIXELFORMAT_R8_G8_B8;w=!1;var B;k&&(w=d._levels[0][0]instanceof HTMLImageElement);if((u||w)&&k){l=pc.PIXELFORMAT_R8_G8_B8_A8;B=new pc.Texture(b,{cubemap:!0,rgbm:p,format:l,width:r,height:r,mipmaps:!1});for(x=0;6>x;x++)w=new pc.RenderTarget(b,B,{face:x,depth:!1}),m.x=x,m.y=0,n.setValue(d),q.setValue(m.data),pc.drawQuadWithShader(b,w,h),e(b,w,x);d=B}if(128<r){var C=Math.round(Math.log2(r))-Math.round(Math.log2(128));for(u=0;u<C;u++){var r=
.5*d.width,A=0===f?1:Math.pow(2,Math.round(Math.log2(v[0])+2*(C-u)));B=new pc.Texture(b,{cubemap:!0,rgbm:p,format:l,width:r,height:r,mipmaps:!1});for(x=0;6>x;x++)w=new pc.RenderTarget(b,B,{face:x,depth:!1}),m.x=x,m.y=A,m.z=r,m.w=p?3:0,n.setValue(d),q.setValue(m.data),pc.drawQuadWithShader(b,w,h),u===C-1&&k&&e(b,w,x);d=B}}a.sourceCubemap=d;B=null;if(!p&&a.filteredFixedRgbm)for(B=new pc.Texture(b,{cubemap:!0,rgbm:!0,format:pc.PIXELFORMAT_R8_G8_B8_A8,width:r,height:r,mipmaps:!1}),x=0;6>x;x++)w=new pc.RenderTarget(b,
B,{face:x,depth:!1}),m.x=x,m.w=2,n.setValue(d),q.setValue(m.data),pc.drawQuadWithShader(b,w,h),e(b,w,x);r=0===f?1:2048;w=0===f?0:-1;t[w]=[];for(u=0;5>u;u++)for(h=w;h<t.length;h++)null!=t[h]&&(t[h][u]=new pc.Texture(b,{cubemap:!0,rgbm:2>h?p:!0,format:2>h?l:pc.PIXELFORMAT_R8_G8_B8_A8,fixCubemapSeams:1===h||3===h,width:y[u],height:y[u],mipmaps:!1}));for(h=w;h<t.length;h++)if(null!=t[h])if(1<h&&p)t[h]=t[h-2];else for(u=0;5>u;u++)for(x=0;6>x;x++)w=new pc.RenderTarget(b,t[h][u],{face:x,depth:!1}),m.x=x,
m.y=0>h?r:v[u],m.z=y[u],m.w=p?3:h,n.setValue(0===u?d:0===f?t[0][u-1]:t[-1][u-1]),q.setValue(m.data),pc.drawQuadWithShader(b,w,g),k&&e(b,w,x);a.filtered=t[0];if(k&&a.singleFilteredFixed){d=[d,a.filteredFixed[0],a.filteredFixed[1],a.filteredFixed[2],a.filteredFixed[3],a.filteredFixed[4],a.filteredFixed[5]];p=new pc.Texture(b,{cubemap:!0,rgbm:p,fixCubemapSeams:!0,format:l,width:128,height:128,addressU:pc.ADDRESS_CLAMP_TO_EDGE,addressV:pc.ADDRESS_CLAMP_TO_EDGE});for(u=0;6>u;u++)p._levels[u]=d[u]._levels[0];
p.upload();p._prefilteredMips=!0;a.singleFilteredFixed=p}if(k&&a.singleFilteredFixedRgbm&&a.filteredFixedRgbm){d=[B,a.filteredFixedRgbm[0],a.filteredFixedRgbm[1],a.filteredFixedRgbm[2],a.filteredFixedRgbm[3],a.filteredFixedRgbm[4],a.filteredFixedRgbm[5]];p=new pc.Texture(b,{cubemap:!0,rgbm:!0,fixCubemapSeams:!0,format:pc.PIXELFORMAT_R8_G8_B8_A8,width:128,height:128,addressU:pc.ADDRESS_CLAMP_TO_EDGE,addressV:pc.ADDRESS_CLAMP_TO_EDGE});for(u=0;6>u;u++)p._levels[u]=d[u]._levels[0];p.upload();p._prefilteredMips=
!0;a.singleFilteredFixedRgbm=p}}},shFromCubemap:function(a,c){var d,f=a.width,g,k;if(a.format!=pc.PIXELFORMAT_R8_G8_B8_A8)console.error("ERROR: SH: cubemap must be RGBA8");else{if(a._levels[0]){if(!a._levels[0][0].length)if(a._levels[0][0]instanceof HTMLImageElement){g=pc.Application.getApplication().graphicsDevice;k=g.gl;d=pc.shaderChunks;var e=d.createShaderFromCode(g,d.fullscreenQuadVS,d.fullscreenQuadPS,"fsQuadSimple"),p=g.scope.resolve("source");for(d=0;6>d;d++){var h=a._levels[0][d],n=new pc.Texture(g,
{cubemap:!1,rgbm:!1,format:a.format,width:f,height:f,mipmaps:!1});n._levels[0]=h;n.upload();h=new pc.Texture(g,{cubemap:!1,rgbm:!1,format:a.format,width:f,height:f,mipmaps:!1});h=new pc.RenderTarget(g,h,{depth:!1});p.setValue(n);pc.drawQuadWithShader(g,h,e);var q=new Uint8Array(f*f*4);k.bindFramebuffer(k.FRAMEBUFFER,h._glFrameBuffer);k.readPixels(0,0,n.width,n.height,k.RGBA,k.UNSIGNED_BYTE,q);a._levels[0][d]=q}}else{console.error("ERROR: SH: cubemap must be composed of arrays or images");return}e=
[];for(k=0;k<f;k++)for(g=0;g<f;g++)e[k*f+g]=(new pc.Vec3(g/(f-1)*2-1,k/(f-1)*2-1,1)).normalize();var p=new Float32Array(27),m,r,t,v,y,w,u,x,B,C,A;for(d=h=0;6>d;d++)for(k=0;k<f;k++)for(g=0;g<f;g++){n=k*f+g;q=g;u=k;x=f;r=(2*(q+.5)/x-1)*(1-1/x);A=(2*(u+.5)/x-1)*(1-1/x);m=1/x;B=r-m;C=A-m;r+=m;A+=m;B=b(B,C)-b(B,A)-b(r,C)+b(r,A);if(0===q&&0===u||q===x-1&&0===u||0===q&&u===x-1||q===x-1&&u===x-1)B/=3;else if(0===q||0===u||q===x-1||u===x-1)B*=.5;q=B;u=4*q/17;x=8*q/17;B=15*q/17;C=5*q/68;A=15*q/68;m=e[n];0==
d?(v=m.z,y=-m.y,w=-m.x):1==d?(v=-m.z,y=-m.y,w=m.x):2==d?(v=m.x,y=m.z,w=m.y):3==d?(v=m.x,y=-m.z,w=-m.y):4==d?(v=m.x,y=-m.y,w=m.z):5==d&&(v=-m.x,y=-m.y,w=-m.z);c||(v=-v);r=a._levels[0][d][4*n+3]/255;for(m=0;3>m;m++)t=a._levels[0][d][4*n+m]/255,a.rgbm?(t*=8*r,t*=t):t=Math.pow(t,2.2),p[0+m]+=t*u,p[3+m]+=t*x*v,p[6+m]+=t*x*y,p[9+m]+=t*x*w,p[12+m]+=t*B*v*w,p[15+m]+=t*B*w*y,p[18+m]+=t*B*y*v,p[21+m]+=t*C*(3*w*w-1),p[24+m]+=t*A*(v*v-y*y),h+=q}for(m=0;m<p.length;m++)p[m]*=4*Math.PI/h;return p}console.error("ERROR: SH: cubemap must be synced to CPU")}}}}());Object.assign(pc,function(){return{paraboloidFromCubemap:function(e,b,a,c){var d=pc.shaderChunks,d=d.createShaderFromCode(e,d.fullscreenQuadVS,(b.fixCubemapSeams?d.fixCubemapSeamsStretchPS:d.fixCubemapSeamsNonePS)+d.genParaboloidPS,"genParaboloid"),f=e.scope.resolve("source"),g=e.scope.resolve("params"),k=new pc.Vec4,l=b.width,p=b.rgbm,h=b.format,l=2*Math.max(l,8),l=new pc.Texture(e,{rgbm:p,format:h,width:2*l,height:l,mipmaps:!1}),p=new pc.RenderTarget(e,l,{depth:!1});k.x=a;k.y=c?-1:1;f.setValue(b);
g.setValue(k.data);pc.drawQuadWithShader(e,p,d);return l},generateDpAtlas:function(e,b,a){var c,d;d=new pc.Vec4;for(var f=new pc.Vec4,g=4*b[0].width,k=pc.shaderChunks,k=k.createShaderFromCode(e,k.fullscreenQuadVS,k.dpAtlasQuadPS,"dpAtlasQuad"),l=e.scope.resolve("source"),p=e.scope.resolve("params"),h=new pc.Texture(e,{rgbm:b[0].rgbm,format:b[0].format,width:g,height:g,mipmaps:!1}),n=new pc.RenderTarget(e,h,{depth:!1}),q=(g+2)/g-1,m=0;6>m;m++){c=pc.paraboloidFromCubemap(e,b[m],m,a);l.setValue(c);c=
d;var r=m;c.x=.5*pc.math.clamp(r-2,0,1);var r=r-6*c.x,t=1-c.x;c.y=Math.min(.5*r,.75)*t+c.x;c.z=(1-.5*pc.math.clamp(r,0,1))*t;c.w=.5*c.z;c=1/c.z;f.x=c*q;f.y=2*f.x;f.x+=1;f.y+=1;p.setValue(f.data);d.x*=g;d.y*=g;d.z*=g;d.w*=g;pc.drawQuadWithShader(e,n,k,d)}return h}}}());pc.shaderChunks.TBNPS="void getTBN() {\n    dTBN = mat3(normalize(dTangentW), normalize(dBinormalW), normalize(dVertexNormalW));\n}\n";pc.shaderChunks.TBNderivativePS="// http://www.thetenthplanet.de/archives/1180\nvoid getTBN() {\n    vec2 uv = $UV;\n    // get edge vectors of the pixel triangle\n    vec3 dp1 = dFdx( vPositionW );\n    vec3 dp2 = dFdy( vPositionW );\n    vec2 duv1 = dFdx( uv );\n    vec2 duv2 = dFdy( uv );\n    // solve the linear system\n    vec3 dp2perp = cross( dp2, dVertexNormalW );\n    vec3 dp1perp = cross( dVertexNormalW, dp1 );\n    vec3 T = dp2perp * duv1.x + dp1perp * duv2.x;\n    vec3 B = dp2perp * duv1.y + dp1perp * duv2.y;\n    // construct a scale-invariant frame\n    float invmax = 1.0 / sqrt( max( dot(T,T), dot(B,B) ) );\n    dTBN = mat3( T * invmax, B * invmax, dVertexNormalW );\n}\n";
...
f===pc.SHADOW_VSM8&&(h+="vec2 encodeFloatRG( float v ) {\n",h+="    vec2 enc = vec2(1.0, 255.0) * v;\n",h+="    enc = fract(enc);\n",h+="    enc -= enc.yy * vec2(1.0/255.0, 1.0/255.0);\n",h+="    return enc;\n",h+="}\n\n"):h+=m.packDepthPS;h+=pc.programlib.begin();b.alphaTest&&(h+="   getOpacity();\n",h+="   alphaTest(dAlpha);\n");var B=f===pc.SHADOW_VSM8||f===pc.SHADOW_VSM16||f===pc.SHADOW_VSM32,h=r===pc.LIGHTTYPE_POINT||B&&r!==pc.LIGHTTYPE_DIRECTIONAL?h+"   float depth = min(distance(view_position, vPositionW) / light_radius, 0.99999);\n":
...
a+"_innerConeAngle, light"+a+"_outerConeAngle);\n",x=!0)),h+="       dAtten *= getLightDiffuse();\n",n.castShadows&&!b.noShadow&&(n._shadowType===pc.SHADOW_VSM8?(l="VSM8",B="0.0"):n._shadowType===pc.SHADOW_VSM16?(l="VSM16",B="5.54"):n._shadowType===pc.SHADOW_VSM32?(l="VSM32",B=e.textureFloatHighPrecision?"15.0":"5.54"):l=n._shadowType===pc.SHADOW_PCF5?"PCF5x5":"PCF3x3",null!==l&&(r===pc.LIGHTTYPE_POINT?(d="(light"+a+"_shadowMap, light"+a+"_shadowParams);\n",n._normalOffsetBias&&(h+="       normalOffsetPointShadow(light"+
a+"_shadowParams);\n"),h+="       dAtten *= getShadowPoint"+l+d):(v===a?l+="VS":(d="(light"+a+"_shadowMatrix, light"+a+"_shadowParams);\n",h+=this._nonPointShadowMapProjection(e,b.lights[a],d)),r===pc.LIGHTTYPE_SPOT&&(l="Spot"+l),h+="       dAtten *= getShadow"+l+"(light"+a+"_shadowMap, light"+a+"_shadowParams"+(n._isVsm?", "+B:"")+");\n"))),h+="       dDiffuseLight += dAtten * light"+a+"_color"+(g?" * dAtten3":"")+";\n",b.useSpecular&&(h+="       dAtten *= getLightSpecular();\n",h+="       dSpecularLight += dAtten * light"+
...
this.lightmapMaxResolution=b.render.lightmapMaxResolution;this.lightmapMode=b.render.lightmapMode;this.exposure=b.render.exposure;this._skyboxIntensity=void 0===b.render.skyboxIntensity?1:b.render.skyboxIntensity;this._skyboxMip=void 0===b.render.skyboxMip?0:b.render.skyboxMip;this._resetSkyboxModel();this.updateShaders=!0};e.prototype._updateSkybox=function(b){if(this._skyboxCubeMap&&!this.skyboxModel){var a=new pc.Material,c=this;a.updateShader=function(a,d,f,g,k){this.shader=b.getProgramLibrary().getProgram("skybox",
{rgbm:c._skyboxCubeMap.rgbm,hdr:c._skyboxCubeMap.rgbm||c._skyboxCubeMap.format===pc.PIXELFORMAT_RGBA32F,useIntensity:1!==c.skyboxIntensity,mip:c._skyboxCubeMap.fixCubemapSeams?c.skyboxMip:0,fixSeams:c._skyboxCubeMap.fixCubemapSeams,gamma:k===pc.SHADER_FORWARDHDR?c.gammaCorrection?pc.GAMMA_SRGBHDR:pc.GAMMA_NONE:c.gammaCorrection,toneMapping:k===pc.SHADER_FORWARDHDR?pc.TONEMAP_LINEAR:c.toneMapping})};a.updateShader();var d;if(this._skyboxCubeMap.fixCubemapSeams&&c._skyboxMip){var f=this["skyboxPrefiltered"+
...
[];this.lightDirVs=[];this.lightDirVsId=[];this.lightRadiusId=[];this.lightPos=[];this.lightPosId=[];this.lightInAngleId=[];this.lightOutAngleId=[];this.lightPosVsId=[];this.lightCookieId=[];this.lightCookieIntId=[];this.lightCookieMatrixId=[];this.lightCookieOffsetId=[];this.depthMapId=a.resolve("uDepthMap");this.screenSizeId=a.resolve("uScreenSize");this._screenSize=new Float32Array(4);this.sourceId=a.resolve("source");this.pixelOffsetId=a.resolve("pixelOffset");this.weightId=a.resolve("weight[0]");
var b=pc.shaderChunks;this.blurVsmShaderCode=[b.blurVSMPS,"#define GAUSS\n"+b.blurVSMPS];this.blurPackedVsmShaderCode=["#define PACKED\n"+this.blurVsmShaderCode[0],"#define PACKED\n"+this.blurVsmShaderCode[1]];this.blurVsmShader=[{},{}];this.blurPackedVsmShader=[{},{}];this.blurVsmWeights={};this.polygonOffsetId=a.resolve("polygonOffset");this.polygonOffset=new Float32Array(2);this.fogColor=new Float32Array(3);this.ambientColor=new Float32Array(3)}function k(a,b){a.data[0]=b.data[0];a.data[1]=b.data[1];
a.data[2]=b.data[2];a.data[3]=b.data[4];a.data[4]=b.data[5];a.data[5]=b.data[6];a.data[6]=b.data[8];a.data[7]=b.data[9];a.data[8]=b.data[10]}for(var l=(new pc.Mat4).mul2((new pc.Mat4).setTranslate(.5,.5,.5),(new pc.Mat4).setScale(.5,.5,.5)),p={r:1,g:2,b:3,a:4},h=[(new pc.Quat).setFromEulerAngles(0,90,180),(new pc.Quat).setFromEulerAngles(0,-90,180),(new pc.Quat).setFromEulerAngles(90,0,0),(new pc.Quat).setFromEulerAngles(-90,0,0),(new pc.Quat).setFromEulerAngles(0,180,180),(new pc.Quat).setFromEulerAngles(0,
0,180)],n=[{},{},{},{},{}],q=new Float32Array(2),m={x:1,y:1,z:0,w:0},r=new pc.Mat4,t=new pc.Mat4,v=new pc.Mat4,y=new pc.Mat4,w=new pc.Mat4,u=new pc.Mat3,x=new pc.Mat4,B,C=new pc.Mat4,A=new pc.Mat4,z=new pc.Mat4,E=new pc.Mat4,J=new pc.Vec3,H=new pc.Vec3,G,D,F=new pc.Mat4,I=new pc.Mat4,S=new pc.Mat4,V=new pc.Mat4,X=new pc.Vec3,W={center:null,radius:0},P,U=new pc.BoundingBox,N=[0,0],R,Q,K,da,O={},T,aa,M=[],ba=0;8>ba;ba++)M.push(new pc.Vec3);var L=[new pc.Vec3,new pc.Vec3,new pc.Vec3,new pc.Vec3,new pc.Vec3,
new pc.Vec3,new pc.Vec3,new pc.Vec3];Object.assign(g.prototype,{sortCompare:function(a,b){if(a.layer===b.layer){if(a.drawOrder&&b.drawOrder)return a.drawOrder-b.drawOrder;if(a.zdist&&b.zdist)return b.zdist-a.zdist;if(a.zdist2&&b.zdist2)return a.zdist2-b.zdist2}return b._key[pc.SORTKEY_FORWARD]-a._key[pc.SORTKEY_FORWARD]},sortCompareMesh:function(a,b){if(a.layer===b.layer){if(a.drawOrder&&b.drawOrder)return a.drawOrder-b.drawOrder;if(a.zdist&&b.zdist)return b.zdist-a.zdist}T=a._key[pc.SORTKEY_FORWARD];
aa=b._key[pc.SORTKEY_FORWARD];return T===aa&&a.mesh&&b.mesh?b.mesh.id-a.mesh.id:aa-T},depthSortCompare:function(a,b){T=a._key[pc.SORTKEY_DEPTH];aa=b._key[pc.SORTKEY_DEPTH];return T===aa&&a.mesh&&b.mesh?b.mesh.id-a.mesh.id:aa-T},lightCompare:function(a,b){return a.key-b.key},_isVisible:function(a,b){if(!b.visible)return!1;P=b.aabb.center;b._aabb._radiusVer!==b._aabbVer&&(b._aabb._radius=b._aabb.halfExtents.length(),b._aabb._radiusVer=b._aabbVer);W.radius=b._aabb._radius;W.center=P;return a.frustum.containsSphere(W)},
getShadowCamera:function(a,b){var c=b._shadowCamera,d;if(null===c){c=b._shadowType;d=pc.CLEARFLAG_DEPTH;var g=c===pc.SHADOW_PCF5||c===pc.SHADOW_PCF3&&a.webgl2;b._type===pc.LIGHTTYPE_POINT&&(g=!1);g||(d|=pc.CLEARFLAG_COLOR);g=new pc.Camera;c>=pc.SHADOW_VSM8&&c<=pc.SHADOW_VSM32?(g.clearColor[0]=0,g.clearColor[1]=0,g.clearColor[2]=0,g.clearColor[3]=0):(g.clearColor[0]=1,g.clearColor[1]=1,g.clearColor[2]=1,g.clearColor[3]=1);g.clearDepth=1;g.clearFlags=d;g.clearStencil=null;g._node=new pc.GraphNode;c=
b._shadowCamera=g;f(a,b)}else d=c.renderTarget,d.width===b._shadowResolution&&d.height===b._shadowResolution||f(a,b);return c},updateCameraFrustum:function(a){if(a.vrDisplay&&a.vrDisplay.presenting){B=a.vrDisplay.combinedProj;var b=a._node.getParent();b?w.copy(b.getWorldTransform()).mul(a.vrDisplay.combinedViewInv).invert():w.copy(a.vrDisplay.combinedView);y.copy(w).invert();this.viewInvId.setValue(y.data)}else{B=a.getProjectionMatrix();a.overrideCalculateProjection&&a.calculateProjection(B,pc.VIEW_CENTER);
if(a.overrideCalculateTransform)a.calculateTransform(y,pc.VIEW_CENTER);else{var b=a._node.getPosition(),c=a._node.getRotation();y.setTRS(b,c,pc.Vec3.ONE);this.viewInvId.setValue(y.data)}w.copy(y).invert()}a.frustum.update(B,w)},setCamera:function(a,b,c,d){var f=a.vrDisplay;if(f&&f.presenting){G=f.leftProj;D=f.rightProj;B=f.combinedProj;a.overrideCalculateProjection&&(a.calculateProjection(G,pc.VIEW_LEFT),a.calculateProjection(D,pc.VIEW_RIGHT),a.calculateProjection(B,pc.VIEW_CENTER));if(a.overrideCalculateTransform)a.calculateTransform(C,
pc.VIEW_LEFT),a.calculateTransform(A,pc.VIEW_RIGHT),a.calculateTransform(y,pc.VIEW_CENTER),z.copy(C).invert(),E.copy(A).invert(),w.copy(y).invert();else{var g=a._node.getParent();if(g){var e=g.getWorldTransform();C.mul2(e,f.leftViewInv);A.mul2(e,f.rightViewInv);z.copy(C).invert();E.copy(A).invert();w.copy(g.getWorldTransform()).mul(f.combinedViewInv).invert()}else C.copy(f.leftViewInv),A.copy(f.rightViewInv),z.copy(f.leftView),E.copy(f.rightView),w.copy(f.combinedView)}k(F,z);k(I,E);S.mul2(G,z);V.mul2(D,
E);J.x=C.data[12];J.y=C.data[13];J.z=C.data[14];H.x=A.data[12];H.y=A.data[13];H.z=A.data[14]}else B=a.getProjectionMatrix(),a.overrideCalculateProjection&&a.calculateProjection(B,pc.VIEW_CENTER),this.projId.setValue(B.data),a.overrideCalculateTransform?a.calculateTransform(y,pc.VIEW_CENTER):(f=a._node.getPosition(),g=a._node.getRotation(),y.setTRS(f,g,pc.Vec3.ONE)),this.viewInvId.setValue(y.data),w.copy(y).invert(),this.viewId.setValue(w.data),k(u,w),this.viewId3.setValue(u.data),x.mul2(B,w),this.viewProjId.setValue(x.data),
f=a._node.getPosition(),this.viewPos[0]=f.x,this.viewPos[1]=f.y,this.viewPos[2]=f.z,this.viewPosId.setValue(this.viewPos);a.frustum.update(B,w);this.nearClipId.setValue(a._nearClip);this.farClipId.setValue(a._farClip);this.cameraParamsId.setValue(a._shaderParams);f=this.device;f.setRenderTarget(b);f.updateBegin();e=a.getRect();g=b?b.width:f.width;b=b?b.height:f.height;var l=Math.floor(e.x*g),h=Math.floor(e.y*b),p=Math.floor(e.width*g),e=Math.floor(e.height*b);f.setViewport(l,h,p,e);f.setScissor(l,
h,p,e);c&&f.clear(a._clearOptions);e=a._scissorRect;l=Math.floor(e.x*g);h=Math.floor(e.y*b);p=Math.floor(e.width*g);e=Math.floor(e.height*b);f.setScissor(l,h,p,e);d&&f.setScissor(1,1,g-2,b-2)},dispatchGlobalLights:function(a){var b;this.mainLight=-1;this.ambientColor[0]=a.ambientLight.r;this.ambientColor[1]=a.ambientLight.g;this.ambientColor[2]=a.ambientLight.b;if(a.gammaCorrection)for(b=0;3>b;b++)this.ambientColor[b]=Math.pow(this.ambientColor[b],2.2);this.ambientId.setValue(this.ambientColor);this.exposureId.setValue(a.exposure);
a.skyboxModel&&this.skyboxIntensityId.setValue(a.skyboxIntensity)},_resolveLight:function(a,b){var c="light"+b;this.lightColorId[b]=a.resolve(c+"_color");this.lightDir[b]=new Float32Array(3);this.lightDirId[b]=a.resolve(c+"_direction");this.lightShadowMapId[b]=a.resolve(c+"_shadowMap");this.lightShadowMatrixId[b]=a.resolve(c+"_shadowMatrix");this.lightShadowParamsId[b]=a.resolve(c+"_shadowParams");this.lightShadowMatrixVsId[b]=a.resolve(c+"_shadowMatrixVS");this.lightShadowParamsVsId[b]=a.resolve(c+
"_shadowParamsVS");this.lightDirVs[b]=new Float32Array(3);this.lightDirVsId[b]=a.resolve(c+"_directionVS");this.lightRadiusId[b]=a.resolve(c+"_radius");this.lightPos[b]=new Float32Array(3);this.lightPosId[b]=a.resolve(c+"_position");this.lightInAngleId[b]=a.resolve(c+"_innerConeAngle");this.lightOutAngleId[b]=a.resolve(c+"_outerConeAngle");this.lightPosVsId[b]=a.resolve(c+"_positionVS");this.lightCookieId[b]=a.resolve(c+"_cookie");this.lightCookieIntId[b]=a.resolve(c+"_cookieIntensity");this.lightCookieMatrixId[b]=
a.resolve(c+"_cookieMatrix");this.lightCookieOffsetId[b]=a.resolve(c+"_cookieOffset")},dispatchDirectLights:function(a,b,c){var d=a.length,f,g,k,e=0;this.mainLight=-1;var l=this.device.scope;for(f=0;f<d;f++)if(a[f]._mask&c){g=a[f];k=g._node.getWorldTransform();this.lightColorId[e]||this._resolveLight(l,e);this.lightColorId[e].setValue(b.gammaCorrection?g._linearFinalColor:g._finalColor);k.getY(g._direction).scale(-1);g._direction.normalize();this.lightDir[e][0]=g._direction.x;this.lightDir[e][1]=
g._direction.y;this.lightDir[e][2]=g._direction.z;this.lightDirId[e].setValue(this.lightDir[e]);if(g.castShadows){var h=g._isPcf&&this.device.webgl2?g._shadowCamera.renderTarget.depthBuffer:g._shadowCamera.renderTarget.colorBuffer;g._isVsm?k=-2E-4:(k=g.shadowBias/g._shadowCamera._farClip*100,!this.device.webgl2&&this.device.extStandardDerivatives&&(k*=-100));var p=g._isVsm?g.vsmBias/(g._shadowCamera._farClip/7):g._normalOffsetBias;this.lightShadowMapId[e].setValue(h);this.lightShadowMatrixId[e].setValue(g._shadowMatrix.data);
h=g._rendererParams;3!==h.length&&(h.length=3);h[0]=g._shadowResolution;h[1]=p;h[2]=k;this.lightShadowParamsId[e].setValue(h);0>this.mainLight&&(this.lightShadowMatrixVsId[e].setValue(g._shadowMatrix.data),this.lightShadowParamsVsId[e].setValue(h),g._direction.normalize(),this.lightDirVs[e][0]=g._direction.x,this.lightDirVs[e][1]=g._direction.y,this.lightDirVs[e][2]=g._direction.z,this.lightDirVsId[e].setValue(this.lightDirVs[e]),this.mainLight=f)}e++}return e},dispatchPointLight:function(a,b,c,d){var f=
...
!1,!1,!1):f.setColorWrite(!0,!0,!0,!0);for(y?u=y+1:y=0;y<u;){v===pc.LIGHTTYPE_POINT&&(w.setRotation(h[y]),D.renderTarget=e._shadowCubeMap[y]);this.setCamera(D,D.renderTarget,!0,v!==pc.LIGHTTYPE_POINT);z=e._visibleList[y];G=e._visibleLength[y];k=e._shadowType;x=k+5*v;for(k=0;k<G;k++){J=z[k];F=J.mesh;n=J.material;this.setBaseConstants(f,n);this.setSkinning(f,J,n);n.dirty&&(n.updateUniforms(),n.dirty=!1);if(n.chunks){B=n.parameters;for(A in B)n=B[A],n.passFlags&H&&(n.scopeId||(n.scopeId=f.scope.resolve(A)),
n.scopeId.setValue(n.data));B=J.parameters;for(A in B)n=B[A],n.passFlags&H&&(n.scopeId||(n.scopeId=f.scope.resolve(A)),n.scopeId.setValue(n.data))}n=J._shader[pc.SHADER_SHADOW+x];if(!n){this.updateShader(J,J._shaderDefs,null,pc.SHADER_SHADOW+x);n=J._shader[pc.SHADER_SHADOW+x];B=J._key;var C=pc.SORTKEY_DEPTH,N=J.material,K=J.skinInstance?10:0,I=0;N.opacityMap&&(N=N.opacityMapChannel)&&(I=p[N]);B[C]=K+I}f.setShader(n);n=J.renderStyle;f.setVertexBuffer(J.morphInstance&&J.morphInstance._vertexBuffer?
J.morphInstance._vertexBuffer:F.vertexBuffer,0);f.setIndexBuffer(F.indexBuffer[n]);k+=this.drawInstance(f,J,F,n);this._shadowDrawCalls++}y++;v===pc.LIGHTTYPE_DIRECTIONAL&&(e._visibleLength[b]=-1)}e._isVsm&&(v=e._vsmBlurSize,1<v&&(D=D.renderTarget,w=d(f,e._shadowResolution,e._shadowType,1),u=e._shadowType===pc.SHADOW_VSM8,y=e.vsmBlurMode,z=(u?this.blurPackedVsmShader:this.blurVsmShader)[y][v],z||(this.blurVsmWeights[v]=c(v),z=pc.shaderChunks.fullscreenQuadVS,k="#define SAMPLES "+v+"\n",k=u?k+this.blurPackedVsmShaderCode[y]:
k+this.blurVsmShaderCode[y],z=pc.shaderChunks.createShaderFromCode(this.device,z,k,"blurVsm"+y+""+v+""+u),u?this.blurPackedVsmShader[y][v]=z:this.blurVsmShader[y][v]=z),m.z=e._shadowResolution-2,m.w=m.z,this.sourceId.setValue(D.colorBuffer),q[0]=1/e._shadowResolution,q[1]=0,this.pixelOffsetId.setValue(q),y===pc.BLUR_GAUSSIAN&&this.weightId.setValue(this.blurVsmWeights[v]),pc.drawQuadWithShader(f,w,z,null,m),this.sourceId.setValue(w.colorBuffer),q[1]=q[0],q[0]=0,this.pixelOffsetId.setValue(q),pc.drawQuadWithShader(f,
D,z,null,m)))}f.webgl2?f.setDepthBias(!1):f.extStandardDerivatives&&(this.polygonOffset[0]=0,this.polygonOffset[1]=0,this.polygonOffsetId.setValue(this.polygonOffset))},updateShader:function(a,b,c,d,f){a.material._scene=this.scene;a.material.updateShader(this.device,this.scene,b,c,d,f);a._shader[d]=a.material.shader},renderForward:function(a,b,c,d,f,g,k,e){var l=this.device,h=this.scene,p=a.vrDisplay,n=1<<f;e=e?e._lightHash:0;var m,q,r,t,v,y,w,u,x=null,B,N,K=.5*l.width;for(m=0;m<c;m++)if(q=b[m],!g||
!q.mask||g&q.mask)if(q.command)q.command();else{r=q.mesh;t=q.material;v=q._shaderDefs;w=q.mask;this.setSkinning(l,q,t);t&&t===x&&v!==y&&(x=null);if(q.isStatic||B)x=null;if(t!==x){this._materialSwitches++;t.dirty&&(t.updateUniforms(),t.dirty=!1);q._shader[f]&&q._shaderDefs===v&&q._lightHash===e||(q.isStatic?this.updateShader(q,v,q._staticLightList,f,d):(y=f+"_"+v+"_"+e,q._shader[f]=t.variants[y],q._shader[f]||(this.updateShader(q,v,null,f,d),t.variants[y]=q._shader[f])),q._shaderDefs=v,q._lightHash=
e);l.setShader(q._shader[f]);y=t.parameters;for(N in y)B=y[N],B.passFlags&n&&(B.scopeId||(B.scopeId=l.scope.resolve(N)),B.scopeId.setValue(B.data));x&&w===u||(u=this.dispatchDirectLights(d[pc.LIGHTTYPE_DIRECTIONAL],h,w),this.dispatchLocalLights(d,h,w,u,q._staticLightList));this.alphaTestId.setValue(t.alphaTest);l.setBlending(t.blend);t.blend&&(t.separateAlphaBlend?(l.setBlendFunctionSeparate(t.blendSrc,t.blendDst,t.blendSrcAlpha,t.blendDstAlpha),l.setBlendEquationSeparate(t.blendEquation,t.blendAlphaEquation)):
(l.setBlendFunction(t.blendSrc,t.blendDst),l.setBlendEquation(t.blendEquation)));l.setColorWrite(t.redWrite,t.greenWrite,t.blueWrite,t.alphaWrite);a._cullFaces?a._flipFaces?l.setCullMode(0<t.cull?t.cull===pc.CULLFACE_FRONT?pc.CULLFACE_BACK:pc.CULLFACE_FRONT:0):l.setCullMode(t.cull):l.setCullMode(pc.CULLFACE_NONE);l.setDepthWrite(t.depthWrite);l.setDepthTest(t.depthTest);l.setAlphaToCoverage(t.alphaToCoverage);t.depthBias||t.slopeDepthBias?(l.setDepthBias(!0),l.setDepthBiasValues(t.depthBias,t.slopeDepthBias)):
l.setDepthBias(!1)}u=q.stencilFront||t.stencilFront;x=q.stencilBack||t.stencilBack;u||x?(l.setStencilTest(!0),u===x?(l.setStencilFunc(u.func,u.ref,u.readMask),l.setStencilOperation(u.fail,u.zfail,u.zpass,u.writeMask)):(u?(l.setStencilFuncFront(u.func,u.ref,u.readMask),l.setStencilOperationFront(u.fail,u.zfail,u.zpass,u.writeMask)):(l.setStencilFuncFront(pc.FUNC_ALWAYS,0,255),l.setStencilOperationFront(pc.STENCILOP_KEEP,pc.STENCILOP_KEEP,pc.STENCILOP_KEEPP,255)),x?(l.setStencilFuncBack(x.func,x.ref,
x.readMask),l.setStencilOperationBack(x.fail,x.zfail,x.zpass,x.writeMask)):(l.setStencilFuncBack(pc.FUNC_ALWAYS,0,255),l.setStencilOperationBack(pc.STENCILOP_KEEP,pc.STENCILOP_KEEP,pc.STENCILOP_KEEP,255)))):l.setStencilTest(!1);y=q.parameters;for(N in y)B=y[N],B.passFlags&n&&(B.scopeId||(B.scopeId=l.scope.resolve(N)),B.scopeId.setValue(B.data));l.setVertexBuffer(q.morphInstance&&q.morphInstance._vertexBuffer?q.morphInstance._vertexBuffer:r.vertexBuffer,0);u=q.renderStyle;l.setIndexBuffer(r.indexBuffer[u]);
k&&k(q,m);p&&p.presenting?(l.setViewport(0,0,K,l.height),this.projId.setValue(G.data),this.viewInvId.setValue(C.data),this.viewId.setValue(z.data),this.viewId3.setValue(F.data),this.viewProjId.setValue(S.data),this.viewPos[0]=J.x,this.viewPos[1]=J.y,this.viewPos[2]=J.z,this.viewPosId.setValue(this.viewPos),m+=this.drawInstance(l,q,r,u,!0),this._forwardDrawCalls++,l.setViewport(K,0,K,l.height),this.projId.setValue(D.data),this.viewInvId.setValue(A.data),this.viewId.setValue(E.data),this.viewId3.setValue(I.data),
this.viewProjId.setValue(V.data),this.viewPos[0]=H.x,this.viewPos[1]=H.y,this.viewPos[2]=H.z,this.viewPosId.setValue(this.viewPos),m+=this.drawInstance2(l,q,r,u)):m+=this.drawInstance(l,q,r,u,!0);this._forwardDrawCalls++;if(m<c-1&&b[m+1].material===t)for(N in y)if(B=t.parameters[N])B.scopeId||(B.scopeId=l.scope.resolve(N)),B.scopeId.setValue(B.data);x=t;y=v;u=w;B=q.isStatic}l.updateEnd()},setupInstancing:function(a){pc._instanceVertexFormat||(pc._instanceVertexFormat=new pc.VertexFormat(a,[{semantic:pc.SEMANTIC_TEXCOORD2,
components:4,type:pc.TYPE_FLOAT32},{semantic:pc.SEMANTIC_TEXCOORD3,components:4,type:pc.TYPE_FLOAT32},{semantic:pc.SEMANTIC_TEXCOORD4,components:4,type:pc.TYPE_FLOAT32},{semantic:pc.SEMANTIC_TEXCOORD5,components:4,type:pc.TYPE_FLOAT32}]));a.enableAutoInstancing&&!pc._autoInstanceBuffer&&(pc._autoInstanceBuffer=new pc.VertexBuffer(a,pc._instanceVertexFormat,a.autoInstancingMaxObjects,pc.BUFFER_DYNAMIC),pc._autoInstanceBufferData=new Float32Array(pc._autoInstanceBuffer.lock()))},revertStaticMeshes:function(a){var b,
c=a.length,d,f=[],g;for(b=0;b<c;b++)d=a[b],d._staticSource?d._staticSource!==g&&(f.push(d._staticSource),g=d._staticSource):f.push(d);a.length=f.length;for(b=0;b<f.length;b++)a[b]=f[b]},prepareStaticMeshes:function(a,b){var c,d,f,g,k,e=this.device,l=a.length,h,p,n=[],m,q,t,r,v,y,u,w,D,x,J,G,z,F,B=new pc.Vec3,A=new pc.Vec3,H=new pc.BoundingBox,C=new pc.Mat4,N=[],K,I,E=[],Q=[],R=[];for(c=0;c<l;c++)if(h=a[c],h.isStatic){m=h.aabb;R.length=0;for(K=pc.LIGHTTYPE_POINT;K<=pc.LIGHTTYPE_SPOT;K++)for(d=0;d<
b.length;d++)p=b[d],p._type===K&&p._enabled&&p._mask&h.mask&&p.isStatic&&(E[d]||(E[d]=new pc.BoundingBox,p._node.getWorldTransform(),p.getBoundingSphere(W),E[d].center.copy(W.center),E[d].halfExtents.x=W.radius,E[d].halfExtents.y=W.radius,E[d].halfExtents.z=W.radius),E[d].intersects(m)&&R.push(d));if(0===R.length)n.push(h);else{m=h.mesh;K=m.vertexBuffer;p=m.indexBuffer[h.renderStyle];q=2===p.bytesPerIndex?new Uint16Array(p.lock()):new Uint32Array(p.lock());t=m.primitive[h.renderStyle].count/3;u=m.primitive[h.renderStyle].base;
r=K.format.elements;v=K.format.size/4;m=new Float32Array(K.storage);for(f=0;f<r.length;f++)r[f].name===pc.SEMANTIC_POSITION&&(y=r[f].offset/4);N.length=t;for(f=0;f<t;f++)N[f]=0;r=!1;Q.length=6*t;for(f=0;f<t;f++){J=x=D=Number.MAX_VALUE;G=-Number.MAX_VALUE;z=-Number.MAX_VALUE;F=-Number.MAX_VALUE;for(g=0;3>g;g++)d=q[3*f+g+u],d=d*v+y,k=m[d],w=m[d+1],d=m[d+2],k<D&&(D=k),w<x&&(x=w),d<J&&(J=d),k>G&&(G=k),w>z&&(z=w),d>F&&(F=d);d=6*f;Q[d]=D;Q[d+1]=x;Q[d+2]=J;Q[d+3]=G;Q[d+4]=z;Q[d+5]=F}for(k=0;k<R.length;k++)for(d=
R[k],C.copy(h.node.worldTransform).invert(),H.setFromTransformedAabb(E[d],C),w=H.getMin(),D=H.getMax(),g=1<<k,f=0;f<t;f++)d=6*f,Q[d]<=D.x&&Q[d+3]>=w.x&&Q[d+1]<=D.y&&Q[d+4]>=w.y&&Q[d+2]<=D.z&&Q[d+5]>=w.z&&(N[f]|=g,r=!0);if(r){r={};for(f=0;f<t;f++)d=3*f+u,I=N[f],r[I]||(r[I]=[]),g=r[I],g.push(q[d]),g.push(q[d+1]),g.push(q[d+2]);for(I in r){g=r[I];q=new pc.IndexBuffer(e,p.format,g.length,p.usage);(2===q.bytesPerIndex?new Uint16Array(q.lock()):new Uint32Array(q.lock())).set(g);q.unlock();J=x=D=Number.MAX_VALUE;
G=-Number.MAX_VALUE;z=-Number.MAX_VALUE;F=-Number.MAX_VALUE;for(f=0;f<g.length;f++)d=g[f],k=m[d*v+y],w=m[d*v+y+1],d=m[d*v+y+2],k<D&&(D=k),w<x&&(x=w),d<J&&(J=d),k>G&&(G=k),w>z&&(z=w),d>F&&(F=d);B.set(D,x,J);A.set(G,z,F);f=new pc.BoundingBox;f.setMinMax(B,A);t=new pc.Mesh;t.vertexBuffer=K;t.indexBuffer[0]=q;t.primitive[0].type=pc.PRIMITIVE_TRIANGLES;t.primitive[0].base=0;t.primitive[0].count=g.length;t.primitive[0].indexed=!0;t.aabb=f;q=new pc.MeshInstance(h.node,t,h.material);q.isStatic=h.isStatic;
q.visible=h.visible;q.layer=h.layer;q.castShadow=h.castShadow;q._receiveShadow=h._receiveShadow;q.cull=h.cull;q.pick=h.pick;q.mask=h.mask;q.parameters=h.parameters;q._shaderDefs=h._shaderDefs;q._staticSource=h;q._staticLightList=h._staticLightList?h._staticLightList:[];for(f=0;f<R.length;f++)g=1<<f,I&g&&(t=b[R[f]],0>q._staticLightList.indexOf(t)&&q._staticLightList.push(t));q._staticLightList.sort(this.lightCompare);n.push(q)}}else n.push(h)}}else n.push(h);a.length=n.length;for(c=0;c<n.length;c++)a[c]=
n[c]},updateShaders:function(a){var b,c=[];for(b=0;b<a.length;b++){var d=a[b];void 0!==d.material&&-1===c.indexOf(d.material)&&c.push(d.material)}for(b=0;b<c.length;b++)a=c[b],a.updateShader!==pc.Material.prototype.updateShader&&(a.clearVariants(),a.shader=null)},beginFrame:function(a){var b=this.device,c=this.scene,d=a._meshInstances;a=a._lights;c.updateSkybox&&(c._updateSkybox(b),c.updateSkybox=!1);c.updateShaders&&(this.updateShaders(d),c.updateShaders=!1,c._shaderVersion++);this.updateCpuSkinMatrices(d);
this.updateMorphedBounds(d);c=d.length;for(b=0;b<c;b++)d[b].visibleThisFrame=!1;c=a.length;for(b=0;b<c;b++)a[b].visibleThisFrame=a[b]._type===pc.LIGHTTYPE_DIRECTIONAL},beginLayers:function(a){var b=this.scene,c=a.layerList.length,d,f,g,k=this.scene._shaderVersion;for(f=0;f<c;f++)a.layerList[f]._postRenderCounter=0;for(f=0;f<c;f++){d=a.layerList[f];d._shaderVersion=k;d._preRenderCalledForCameras=0;d._postRenderCalledForCameras=0;g=a.subLayerList[f];d._postRenderCounter=g?d._postRenderCounter|2:d._postRenderCounter|
1;d._postRenderCounterMax=d._postRenderCounter;for(g=0;g<d.cameras.length;g++)d.instances.visibleOpaque[g]||(d.instances.visibleOpaque[g]=new pc.VisibleInstanceList),d.instances.visibleTransparent[g]||(d.instances.visibleTransparent[g]=new pc.VisibleInstanceList),d.instances.visibleOpaque[g].done=!1,d.instances.visibleTransparent[g].done=!1;d._needsStaticPrepare&&d._staticLightHash&&(d._staticPrepareDone&&(this.revertStaticMeshes(d.opaqueMeshInstances),this.revertStaticMeshes(d.transparentMeshInstances)),
this.prepareStaticMeshes(d.opaqueMeshInstances,d._lights),this.prepareStaticMeshes(d.transparentMeshInstances,d._lights),a._dirty=!0,b.updateShaders=!0,d._needsStaticPrepare=!1,d._staticPrepareDone=!0)}},cullLocalShadowmap:function(a,b){var c,d,f,g,k,e,l,p,n,m,q;d=a._type;if(d!==pc.LIGHTTYPE_DIRECTIONAL)for(a.visibleThisFrame=!0,f=this.getShadowCamera(this.device,a),f.projection=pc.PROJECTION_PERSPECTIVE,f.nearClip=a.attenuationEnd/1E3,f.farClip=a.attenuationEnd,f.aspectRatio=1,d===pc.LIGHTTYPE_SPOT?
(f.fov=2*a._outerConeAngle,k=1):(f.fov=90,k=6),g=f._node,c=a._node,g.setPosition(c.getPosition()),d===pc.LIGHTTYPE_SPOT&&(g.setRotation(c.getRotation()),g.rotateLocal(-90,0,0)),e=0;e<k;e++){d===pc.LIGHTTYPE_POINT&&(g.setRotation(h[e]),f.renderTarget=a._shadowCubeMap[e]);this.updateCameraFrustum(f);(n=a._visibleList[e])||(n=a._visibleList[e]=[]);c=m=a._visibleLength[e]=0;for(l=b.length;c<l;c++)p=b[c],q=!0,p.cull&&(q=this._isVisible(f,p)),q&&(n[m]=p,m++,p.visibleThisFrame=!0);a._visibleLength[e]=m;
n.length!==m&&(n.length=m);n.sort(this.depthSortCompare)}},cullDirectionalShadowmap:function(a,b,c,d){var f,g,k,e,l,h,p,n,m,q,t,y;g=this.device;a.visibleThisFrame=!0;g=this.getShadowCamera(g,a);k=g._node;e=a._node;k.setPosition(e.getPosition());k.setRotation(e.getRotation());k.rotateLocal(-90,0,0);n=a.shadowDistance||c._farClip;m=c._nearClip;f=c._fov*Math.PI/180;l=c._aspect;h=c._projection;t=h===pc.PROJECTION_PERSPECTIVE?Math.tan(f/2)*m:c._orthoHeight;q=t*l;M[0].x=q;M[0].y=-t;M[0].z=-m;M[1].x=q;M[1].y=
t;M[1].z=-m;M[2].x=-q;M[2].y=t;M[2].z=-m;M[3].x=-q;M[3].y=-t;M[3].z=-m;h===pc.PROJECTION_PERSPECTIVE&&(t=Math.tan(f/2)*n,q=t*l);M[4].x=q;M[4].y=-t;M[4].z=-n;M[5].x=q;M[5].y=t;M[5].z=-n;M[6].x=-q;M[6].y=t;M[6].z=-n;M[7].x=-q;M[7].y=-t;M[7].z=-n;l=X.sub2(M[0],M[6]).length();l=Math.max(l,X.sub2(M[4],M[6]).length());r.copy(k.getWorldTransform()).invert();v.copy(r).mul(c._node.worldTransform);for(f=0;8>f;f++)v.transformPoint(M[f],M[f]);n=m=c=1E6;q=t=h=-1E6;for(f=0;8>f;f++)p=M[f],p.x<n&&(n=p.x),p.x>q&&
(q=p.x),p.y<m&&(m=p.y),p.y>t&&(t=p.y),p.z<c&&(c=p.z),p.z>h&&(h=p.z);f=l/a._shadowResolution;n=Math.floor((n-.5*(l-(q-n)))/f)*f;m=Math.floor((m-.5*(l-(t-m)))/f)*f;n=.5*(n+l+n);m=.5*(m+l+m);k.translateLocal(n,m,1E5);g.projection=pc.PROJECTION_ORTHOGRAPHIC;g.nearClip=0;g.farClip=2E5;g.aspectRatio=1;g.orthoHeight=.5*l;this.updateCameraFrustum(g);t=!0;(h=a._visibleList[d])||(h=a._visibleList[d]=[]);f=l=a._visibleLength[d]=0;for(q=b.length;f<q;f++)y=b[f],p=!0,y.cull&&(p=this._isVisible(g,y)),p&&(h[l]=y,
l++,y.visibleThisFrame=!0,p=y.aabb,t?(U.copy(p),t=!1):U.add(p));a._visibleLength[d]=l;h.length!==l&&(h.length=l);h.sort(this.depthSortCompare);b=U.getMin();f=U.getMax();L[0].x=L[1].x=L[2].x=L[3].x=b.x;L[1].y=L[3].y=L[7].y=L[5].y=b.y;L[2].z=L[3].z=L[6].z=L[7].z=b.z;L[4].x=L[5].x=L[6].x=L[7].x=f.x;L[0].y=L[2].y=L[4].y=L[6].y=f.y;L[0].z=L[1].z=L[4].z=L[5].z=f.z;f=9999999999;b=-9999999999;for(h=0;8>h;++h)r.transformPoint(L[h],L[h]),l=L[h].z,l<f&&(f=l),l>b&&(b=l);h=b;f>c&&(c=f);k.setPosition(e.getPosition());
k.translateLocal(n,m,h+.01);g.farClip=h-c;(e=a._visibleCameraSettings[d])||(e=a._visibleCameraSettings[d]={});a=k.getPosition();e.x=a.x;e.y=a.y;e.z=a.z;e.orthoHeight=g.orthoHeight;e.farClip=g.farClip},gpuUpdate:function(a){this.updateGpuSkinMatrices(a);this.updateMorphing(a)},clearView:function(a,b,c){a=a.camera;var d=this.device;d.setRenderTarget(b);d.updateBegin();d.setColorWrite(!0,!0,!0,!0);d.setDepthWrite(!0);var f=a.getRect(),g=b?b.width:d.width,k=b?b.height:d.height;b=Math.floor(f.x*g);var e=
Math.floor(f.y*k),g=Math.floor(f.width*g),f=Math.floor(f.height*k);d.setViewport(b,e,g,f);d.setScissor(b,e,g,f);d.clear(c?c:a._clearOptions)},setSceneConstants:function(){var a,b=this.device,c=this.scene;this.dispatchGlobalLights(c);if(c.fog!==pc.FOG_NONE){this.fogColor[0]=c.fogColor.r;this.fogColor[1]=c.fogColor.g;this.fogColor[2]=c.fogColor.b;if(c.gammaCorrection)for(a=0;3>a;a++)this.fogColor[a]=Math.pow(this.fogColor[a],2.2);this.fogColorId.setValue(this.fogColor);c.fog===pc.FOG_LINEAR?(this.fogStartId.setValue(c.fogStart),
this.fogEndId.setValue(c.fogEnd)):this.fogDensityId.setValue(c.fogDensity)}this._screenSize[0]=b.width;this._screenSize[1]=b.height;this._screenSize[2]=1/b.width;this._screenSize[3]=1/b.height;this.screenSizeId.setValue(this._screenSize)},renderComposition:function(a){var b=this.device,c,d=a._renderedRt,f=a._renderedByCam,g=a._renderedLayer,k,e,l,h,p,n,m,q;this.beginLayers(a);a._update()&pc.COMPUPDATED_LIGHTS&&(this.scene.updateShaders=!0);this.beginFrame(a);this.setSceneConstants();var t=0,r,v;for(k=
0;k<a.layerList.length;k++)if(e=a.layerList[k],e.enabled&&a.subLayerEnabled[k])for(l=a.subLayerList[k],r=e.instances,h=e.cameras,p=0;p<h.length;p++)if(c=h[p]){c.frameBegin(e.renderTarget);v=l?e.transparentMeshInstances:e.opaqueMeshInstances;q=m=!1;for(n=0;n<t;n++)if(f[n]===c&&(m=!0,g[n]===e)){q=!0;break}m||(this.updateCameraFrustum(c.camera),this._camerasRendered++);q||this.cullLights(c.camera,e._lights);m&&q||(f[t]=c,g[t]=e,t++);n=l?r.visibleTransparent[p]:r.visibleOpaque[p];if(!n.done){if(e.onPreCull)e.onPreCull(p);
n.length=this.cull(c.camera,v,n.list);n.done=!0;if(e.onPostCull)e.onPostCull(p)}c.frameEnd()}for(k=0;k<a._lights.length;k++)c=a._lights[k],c.visibleThisFrame&&c._type!==pc.LIGHTTYPE_DIRECTIONAL&&c.castShadows&&c._enabled&&c.shadowUpdateMode!==pc.SHADOWUPDATE_NONE&&(e=a._lightShadowCasters[k],this.cullLocalShadowmap(c,e));l=-1;for(k=0;k<a._lights.length;k++)if(c=a._lights[k],c._type===pc.LIGHTTYPE_DIRECTIONAL&&(l++,c.castShadows&&c._enabled&&c.shadowUpdateMode!==pc.SHADOWUPDATE_NONE))for(e=a._lightShadowCasters[k],
h=a._globalLightCameras[l],p=0;p<h.length;p++)this.cullDirectionalShadowmap(c,e,h[p].camera,a._globalLightCameraIds[l][p]);this.gpuUpdate(a._meshInstances);this.renderShadows(a._sortedLights[pc.LIGHTTYPE_SPOT]);this.renderShadows(a._sortedLights[pc.LIGHTTYPE_POINT]);for(k=t=0;k<a._renderList.length;k++)if(e=a.layerList[a._renderList[k]],e.enabled&&a.subLayerEnabled[a._renderList[k]]){r=e.instances;l=a.subLayerList[a._renderList[k]];h=a._renderListCamera[k];(c=e.cameras[h])&&c.frameBegin(e.renderTarget);
if(!l&&e.onPreRenderOpaque)e.onPreRenderOpaque(h);else if(l&&e.onPreRenderTransparent)e.onPreRenderTransparent(h);if(!(e._preRenderCalledForCameras&1<<h)){if(e.onPreRender)e.onPreRender(h);e._preRenderCalledForCameras|=1<<h;e.overrideClear&&this.clearView(c,e.renderTarget,e._clearOptions)}if(c){p=e.renderTarget;g=!1;for(n=0;n<t;n++)if(d[n]===p&&f[n]===c){g=!0;break}g||(e.overrideClear||this.clearView(c,e.renderTarget),d[t]=p,f[t]=c,t++);this.renderShadows(e._sortedLights[pc.LIGHTTYPE_DIRECTIONAL],
h);e._sortVisible(l,c.node,h);n=l?r.visibleTransparent[h]:r.visibleOpaque[h];this.scene._activeCamera=c.camera;this.setCamera(c.camera,e.renderTarget);this.renderForward(c.camera,n.list,n.length,e._sortedLights,e.shaderPass,e.cullingMask,e.onDrawCall,e);b.setColorWrite(!0,!0,!0,!0);b.setStencilTest(!1);b.setAlphaToCoverage(!1);b.setDepthBias(!1);c.frameEnd()}if(!l&&e.onPostRenderOpaque)e.onPostRenderOpaque(h);else if(l&&e.onPostRenderTransparent)e.onPostRenderTransparent(h);!e.onPostRender||e._postRenderCalledForCameras&
...
!this._dirtyWorld)return this.worldTransform;this._parent&&this._parent.getWorldTransform();this._sync();return this.worldTransform},reparent:function(a,b){var c=this._parent;c&&c.removeChild(this);a&&(0<=b?a.insertChild(this,b):a.addChild(this))},setLocalEulerAngles:function(a,b,c){a instanceof pc.Vec3?this.localRotation.setFromEulerAngles(a.x,a.y,a.z):this.localRotation.setFromEulerAngles(a,b,c);this._dirtifyLocal()},setLocalPosition:function(a,b,c){a instanceof pc.Vec3?this.localPosition.copy(a):
this.localPosition.set(a,b,c);this._dirtifyLocal()},setLocalRotation:function(a,b,c,d){a instanceof pc.Quat?this.localRotation.copy(a):this.localRotation.set(a,b,c,d);this._dirtifyLocal()},setLocalScale:function(a,b,c){a instanceof pc.Vec3?this.localScale.copy(a):this.localScale.set(a,b,c);this._dirtifyLocal()},setName:function(a){this.name=a},_dirtifyLocal:function(){this._dirtyLocal||(this._dirtyLocal=!0,this._dirtifyWorld())},_dirtifyWorld:function(){if(!this._dirtyWorld){this._dirtyWorld=!0;for(var a=
0;a<this._children.length;a++)this._children[a]._dirtyWorld||this._children[a]._dirtifyWorld()}this._dirtyNormal=!0;this._aabbVer++},setPosition:function(){var a=new pc.Vec3,b=new pc.Mat4;return function(c,d,f){c instanceof pc.Vec3?a.copy(c):a.set(c,d,f);null===this._parent?this.localPosition.copy(a):(b.copy(this._parent.getWorldTransform()).invert(),b.transformPoint(a,this.localPosition));this._dirtifyLocal()}}(),setRotation:function(){var a=new pc.Quat,b=new pc.Quat;return function(c,d,f,g){c instanceof
pc.Quat?a.copy(c):a.set(c,d,f,g);null===this._parent?this.localRotation.copy(a):(c=this._parent.getRotation(),b.copy(c).invert(),this.localRotation.copy(b).mul(a));this._dirtifyLocal()}}(),setEulerAngles:function(){var a=new pc.Quat;return function(b,c,d){b instanceof pc.Vec3?this.localRotation.setFromEulerAngles(b.x,b.y,b.z):this.localRotation.setFromEulerAngles(b,c,d);null!==this._parent&&(b=this._parent.getRotation(),a.copy(b).invert(),this.localRotation.mul2(a,this.localRotation));this._dirtifyLocal()}}(),
addChild:function(a){if(null!==a._parent)throw Error("GraphNode is already parented");this._children.push(a);this._onInsertChild(a)},addChildAndSaveTransform:function(a){var b=a.getPosition(),c=a.getRotation(),d=a._parent;d&&d.removeChild(a);void 0===this.tmpMat4&&(this.tmpMat4=new pc.Mat4,this.tmpQuat=new pc.Quat);a.setPosition(this.tmpMat4.copy(this.worldTransform).invert().transformPoint(b));a.setRotation(this.tmpQuat.copy(this.getRotation()).invert().mul(c));this._children.push(a);this._onInsertChild(a)},
insertChild:function(a,b){if(null!==a._parent)throw Error("GraphNode is already parented");this._children.splice(b,0,a);this._onInsertChild(a)},_onInsertChild:function(a){a._parent=this;var b=a._enabled&&this.enabled;a._enabledInHierarchy!==b&&(a._enabledInHierarchy=b,a._notifyHierarchyStateChanged(a,b));a._updateGraphDepth();a._dirtifyWorld();a.fire&&a.fire("insert",this);this.fire&&this.fire("childinsert",a)},_updateGraphDepth:function(){this._graphDepth=this._parent?this._parent._graphDepth+1:
...
a.mask=this._mask;a.innerConeAngle=this._innerConeAngle;a.outerConeAngle=this._outerConeAngle;a.shadowBias=this.shadowBias;a.normalOffsetBias=this._normalOffsetBias;a.shadowResolution=this._shadowResolution;a.shadowDistance=this.shadowDistance;return a},getColor:function(){return this._color},getBoundingSphere:function(c){if(this._type===pc.LIGHTTYPE_SPOT){var d=this.attenuationEnd,f=this._outerConeAngle,l=Math.cos(f*pc.math.DEG_TO_RAD),p=this._node;e.copy(p.up);e.scale(.5*-d*l);e.add(p.getPosition());
...
18|(this._cookieTransform?1:0)<<12;3===this._cookieChannel.length&&(a|=c[this._cookieChannel.charAt(1)]<<16,a|=c[this._cookieChannel.charAt(2)]<<14);this.key=a}});Object.defineProperty(d.prototype,"enabled",{get:function(){return this._type},set:function(a){this._type!==a&&(this._enabled=a,null!==this._scene&&(this._scene.updateShaders=!0))}});Object.defineProperty(d.prototype,"type",{get:function(){return this._type},set:function(a){this._type!==a&&(this._type=a,this._destroyShadowMap(),null!==this._scene&&
(this._scene.updateShaders=!0),this.updateKey(),a=this._shadowType,this._shadowType=null,this.shadowType=a,null!==this._scene&&(this._scene.layers._dirtyLights=!0))}});Object.defineProperty(d.prototype,"mask",{get:function(){return this._mask},set:function(a){this._mask!==a&&(this._mask=a,null!==this._scene&&(this._scene.updateShaders=!0))}});Object.defineProperty(d.prototype,"shadowType",{get:function(){return this._shadowType},set:function(a){if(this._shadowType!==a){var b=pc.Application.getApplication().graphicsDevice;
this._type===pc.LIGHTTYPE_POINT&&(a=pc.SHADOW_PCF3);a!==pc.SHADOW_PCF5||b.webgl2||(a=pc.SHADOW_PCF3);a!==pc.SHADOW_VSM32||b.textureFloatRenderable||(a=pc.SHADOW_VSM16);a!==pc.SHADOW_VSM16||b.textureHalfFloatRenderable||(a=pc.SHADOW_VSM8);this._isVsm=a>=pc.SHADOW_VSM8&&a<=pc.SHADOW_VSM32;this._isPcf=a===pc.SHADOW_PCF5||a===pc.SHADOW_PCF3;this._shadowType=a;this._destroyShadowMap();null!==this._scene&&(this._scene.updateShaders=!0);this.updateKey()}}});Object.defineProperty(d.prototype,"castShadows",
...
set:function(a){this._outerConeAngle!==a&&(this._outerConeAngle=a,this._outerConeAngleCos=Math.cos(a*Math.PI/180))}});Object.defineProperty(d.prototype,"intensity",{get:function(){return this._intensity},set:function(a){this._intensity!==a&&(this._intensity=a,this._updateFinalColor())}});Object.defineProperty(d.prototype,"cookie",{get:function(){return this._cookie},set:function(a){this._cookie!==a&&(this._cookie=a,null!==this._scene&&(this._scene.updateShaders=!0),this.updateKey())}});Object.defineProperty(d.prototype,
"cookieFalloff",{get:function(){return this._cookieFalloff},set:function(a){this._cookieFalloff!==a&&(this._cookieFalloff=a,null!==this._scene&&(this._scene.updateShaders=!0),this.updateKey())}});Object.defineProperty(d.prototype,"cookieChannel",{get:function(){return this._cookieChannel},set:function(a){if(this._cookieChannel!==a){if(3>a.length)for(var b=a.charAt(a.length-1),c=3-a.length,d=0;d<c;d++)a+=b;this._cookieChannel=a;null!==this._scene&&(this._scene.updateShaders=!0);this.updateKey()}}});
Object.defineProperty(d.prototype,"cookieTransform",{get:function(){return this._cookieTransform},set:function(a){this._cookieTransform!==a&&(!(!this._cookieTransformSet&&!this._cookieOffsetSet)!==!(!a&&!this._cookieOffsetSet)&&null!==this._scene&&(this._scene.updateShaders=!0),this._cookieTransform=a,this._cookieTransformSet=!!a,a&&!this._cookieOffset&&(this.cookieOffset=new pc.Vec2,this._cookieOffsetSet=!1),this.updateKey())}});Object.defineProperty(d.prototype,"cookieOffset",{get:function(){return this._cookieOffset},
set:function(a){if(this._cookieOffset!==a){var b=!(!this._cookieTransformSet&&!a);!(!this._cookieTransformSet&&!this._cookieOffsetSet)!==b&&null!==this._scene&&(this._scene.updateShaders=!0);b&&!a&&this._cookieOffset?this._cookieOffset.set(0,0):this._cookieOffset=a;this._cookieOffsetSet=!!a;a&&!this._cookieTransform&&(this.cookieTransform=new pc.Vec4(1,1,0,0),this._cookieTransformSet=!1);this.updateKey()}}});return{Light:d}}());Object.assign(pc,function(){var e=0,b=function(){this.name="Untitled";this.id=e++;this._shader=null;this.variants={};this.parameters={};this.alphaTest=0;this.blend=this.alphaToCoverage=!1;this.blendSrc=pc.BLENDMODE_ONE;this.blendDst=pc.BLENDMODE_ZERO;this.blendEquation=pc.BLENDEQUATION_ADD;this.separateAlphaBlend=!1;this.blendSrcAlpha=pc.BLENDMODE_ONE;this.blendDstAlpha=pc.BLENDMODE_ZERO;this.blendAlphaEquation=pc.BLENDEQUATION_ADD;this.cull=pc.CULLFACE_BACK;this.depthWrite=this.depthTest=!0;this.stencilBack=
...
e=this.mesh.vertexBuffer.numVertices;b=this.mesh.vertexBuffer.format.size;var m,r,t,v,y;for(f=0;f<g.length;f++)g[f].name===pc.SEMANTIC_POSITION?r=g[f].offset:g[f].name===pc.SEMANTIC_BLENDINDICES?t=g[f].offset:g[f].name===pc.SEMANTIC_BLENDWEIGHT&&(v=g[f].offset);var g=new Uint8Array(this.mesh.vertexBuffer.storage),w=new Float32Array(this.mesh.vertexBuffer.storage),u=r/4,x=v/4,B=b/4,C,A,z,E,J;v=[];r=[];d=this.mesh.boneUsed;for(f=0;f<c;f++)v[f]=new pc.Vec3(Number.MAX_VALUE,Number.MAX_VALUE,Number.MAX_VALUE),
r[f]=new pc.Vec3(-Number.MAX_VALUE,-Number.MAX_VALUE,-Number.MAX_VALUE);for(y=0;y<e;y++)for(f=0;4>f;f++)0<w[y*B+x+f]&&(m=g[y*b+t+f],z=w[y*B+u],E=w[y*B+u+1],J=w[y*B+u+2],C=r[m],A=v[m],A.x>z&&(A.x=z),A.y>E&&(A.y=E),A.z>J&&(A.z=J),C.x<z&&(C.x=z),C.y<E&&(C.y=E),C.z<J&&(C.z=J),d[m]=!0);if(this.morphInstance){var H;d=this.morphInstance.morph._targets;var G=new Float32Array(3*e),D=new Float32Array(3*e),F;for(y=0;y<e;y++)G[3*y]=D[3*y]=w[y*B+u],G[3*y+1]=D[3*y+1]=w[y*B+u+1],G[3*y+2]=D[3*y+2]=w[y*B+u+2];for(e=
0;e<d.length;e++)for(f=d[e],u=f.indices,y=u.length,z=f.deltaPositions,f=0;f<y;f++)H=u[f],m=z[3*f],C=z[3*f+1],A=z[3*f+2],0>m?G[3*H]+=m:D[3*H]+=m,0>C?G[3*H+1]+=C:D[3*H+1]+=C,0>A?G[3*H+2]+=A:D[3*H+2]+=A;for(e=0;e<d.length;e++)for(f=d[e],u=f.indices,y=u.length,f=0;f<y;f++)for(H=u[f],F=0;4>F;F++)0<w[H*B+x+F]&&(m=g[H*b+t+F],C=r[m],A=v[m],z=G[3*H],E=G[3*H+1],J=G[3*H+2],A.x>z&&(A.x=z),A.y>E&&(A.y=E),A.z>J&&(A.z=J),z=D[3*H],E=D[3*H+1],J=D[3*H+2],C.x<z&&(C.x=z),C.y<E&&(C.y=E),C.z<J&&(C.z=J))}for(f=0;f<c;f++)b=
...
1;l<w.length;l++)if(w[l].vertices!==u)throw Error("partitionSkin: All meshes that share a skin should also share the same vertex buffer");var x=[],B=[];p=[];var C=0;for(l=0;l<w.length;l++){m=w[l];for(var A=m.indices,z=m.base;z<m.base+m.count;){h=A[z++];B[0]=v(h);p[0]=h;h=A[z++];B[1]=v(h);p[1]=h;h=A[z++];B[2]=v(h);p[2]=h;for(var E=!1,J=C;J<x.length;J++)if(h=x[J],h.addPrimitive(B,p,u,g)){E=!0;break}E||(h=new b,h.originalMesh=m,h.addPrimitive(B,p,u,g),x.push(h))}C=x.length}m=[];w=[];for(l=0;l<x.length;l++)if(h=
x[l],h.vertices.length&&h.indices.length){B=m.length;p=h.vertices.length;C=w.length;A=h.indices.length;h.partition=l;h.vertexStart=B;h.vertexCount=p;h.indexStart=C;h.indexCount=A;z=0;for(E=B;z<p;)m[E++]=h.vertices[z++];z=0;for(E=C;z<A;)w[E++]=h.indices[z++]+B}B=[];for(l=0;l<x.length;l++){h=x[l];C=[];A=[];for(p=0;p<h.boneIndices.length;p++)C.push(y.inverseBindMatrices[h.boneIndices[p]]),A.push(y.boneNames[h.boneIndices[p]]);h={inverseBindMatrices:C,boneNames:A};B.push(h);q.push(h)}var H,y={};for(H in u)y[H]=
{components:u[H].components,data:[],type:u[H].type};for(H in u)if("blendIndices"===H)for(h=y[H].data,l=0;l<m.length;l++)p=m[l].boneIndices,h.push(p[0],p[1],p[2],p[3]);else for(l=u[H],C=l.data,A=l.components,l=0;l<m.length;l++)for(h=m[l].index,p=0;p<A;p++)y[H].data.push(C[h*A+p]);n[n.indexOf(u)]=y;for(l=0;l<x.length;l++)for(h=x[l],m={aabb:{min:[0,0,0],max:[0,0,0]},vertices:y,skin:B[l],indices:w.splice(0,h.indexCount),type:"triangles",base:0,count:h.indexCount},r.push(m),p=t.length-1;0<=p;p--)t[p].mesh===
...
this.meshInstance.cull=!0;this.meshInstance._noDepthDrawGl1=!0;this.meshInstance.aabb=this.worldBounds;this.meshInstance._updateAabb=!1;this.meshInstance.visible=a;this._initializeTextures();this.addTime(0);this.preWarm&&this.prewarm(this.lifetime);this.resetTime()},_isAnimated:function(){return 1<=this.animNumFrames&&(1<this.animTilesX||1<this.animTilesY)&&(this.colorMap&&this.colorMap!==U.DEFAULT_PARAM_TEXTURE||this.normalMap)},calcSpawnPosition:function(a,c){var d=Math.random(),f=Math.random(),
g=Math.random(),k=Math.random();this.useCpu&&(this.particleTex[4*c+0+8*this.numParticlesPot]=d,this.particleTex[4*c+1+8*this.numParticlesPot]=f,this.particleTex[4*c+2+8*this.numParticlesPot]=g);F.x=d-.5;F.y=f-.5;F.z=g-.5;this.emitterShape===pc.EMITTERSHAPE_BOX?I.copy(a).add(D.transformPoint(F)):(F.normalize(),I.copy(a).add(F.scale(k*this.spawnBounds)));if(this.pack8){var l=(I.x-this.worldBounds.center.x)/this.worldBoundsSize.x+.5,k=(I.y-this.worldBounds.center.y)/this.worldBoundsSize.y+.5,g=(I.z-
this.worldBounds.center.z)/this.worldBoundsSize.z+.5,f=pc.math.lerp(this.startAngle*pc.math.DEG_TO_RAD,this.startAngle2*pc.math.DEG_TO_RAD,d),f=f%(2*Math.PI)/(2*Math.PI),l=b(l);this.particleTex[4*c]=l[0];this.particleTex[4*c+1]=l[1];k=b(k);this.particleTex[4*c+2]=k[0];this.particleTex[4*c+3]=k[1];g=b(g);this.particleTex[4*c+0+4*this.numParticlesPot]=g[0];this.particleTex[4*c+1+4*this.numParticlesPot]=g[1];f=b(f);this.particleTex[4*c+2+4*this.numParticlesPot]=f[0];this.particleTex[4*c+3+4*this.numParticlesPot]=
f[1];this.particleTex[4*c+3+8*this.numParticlesPot]=1;d=pc.math.lerp(this.rate,this.rate2,d);f=Math.max(this.lifetime,(this.numParticles-1)*Math.max(this.rate,this.rate2));k=(-d*c+f)/(f+(this.lifetime+1));d=e(k);f=e(255*k);g=e(65025*k);k=e(160581375*k);d-=f/255;f-=g/255;d=[d,f,g-k/255,k-k/255];this.particleTex[4*c+0+12*this.numParticlesPot]=d[0];this.particleTex[4*c+1+12*this.numParticlesPot]=d[1];this.particleTex[4*c+2+12*this.numParticlesPot]=d[2];this.particleTex[4*c+3+12*this.numParticlesPot]=
d[3]}else this.particleTex[4*c]=I.x,this.particleTex[4*c+1]=I.y,this.particleTex[4*c+2]=I.z,this.particleTex[4*c+3]=pc.math.lerp(this.startAngle*pc.math.DEG_TO_RAD,this.startAngle2*pc.math.DEG_TO_RAD,d),d=pc.math.lerp(this.rate,this.rate2,d),this.particleTex[4*c+3+4*this.numParticlesPot]=-d*c},rebuildGraphs:function(){var a=this.precision,b=this.graphicsDevice,c;this.qLocalVelocity=this.localVelocityGraph.quantize(a);this.qVelocity=this.velocityGraph.quantize(a);this.qColor=this.colorGraph.quantize(a);
this.qRotSpeed=this.rotationSpeedGraph.quantize(a);this.qScale=this.scaleGraph.quantize(a);this.qAlpha=this.alphaGraph.quantize(a);this.qLocalVelocity2=this.localVelocityGraph2.quantize(a);this.qVelocity2=this.velocityGraph2.quantize(a);this.qColor2=this.colorGraph2.quantize(a);this.qRotSpeed2=this.rotationSpeedGraph2.quantize(a);this.qScale2=this.scaleGraph2.quantize(a);this.qAlpha2=this.alphaGraph2.quantize(a);for(c=0;c<a;c++)this.qRotSpeed[c]*=pc.math.DEG_TO_RAD,this.qRotSpeed2[c]*=pc.math.DEG_TO_RAD;
this.localVelocityUMax=new Float32Array(3);this.velocityUMax=new Float32Array(3);this.colorUMax=new Float32Array(3);this.rotSpeedUMax=[0];this.scaleUMax=[0];this.alphaUMax=[0];this.qLocalVelocityDiv=g(this.qLocalVelocity,this.qLocalVelocity2,this.localVelocityUMax);this.qVelocityDiv=g(this.qVelocity,this.qVelocity2,this.velocityUMax);this.qColorDiv=g(this.qColor,this.qColor2,this.colorUMax);this.qRotSpeedDiv=g(this.qRotSpeed,this.qRotSpeed2,this.rotSpeedUMax);this.qScaleDiv=g(this.qScale,this.qScale2,
this.scaleUMax);this.qAlphaDiv=g(this.qAlpha,this.qAlpha2,this.alphaUMax);if(this.pack8){var e=[0,0,0];f(this.qVelocity,e);var k=[0,0,0];f(this.qVelocity2,k);c=[0,0,0];f(this.qLocalVelocity,c);var l=[0,0,0];f(this.qLocalVelocity2,l);var h=Math.max(e[0],k[0]),h=Math.max(h,e[1]),h=Math.max(h,k[1]),h=Math.max(h,e[2]),h=Math.max(h,k[2]),e=Math.max(c[0],l[0]),e=Math.max(e,c[1]),e=Math.max(e,l[1]),e=Math.max(e,c[2]),e=Math.max(e,l[2]);this.maxVel=h+e}if(!this.useCpu){this.internalTex0=p(b,a,1,d(this.qLocalVelocity,
this.qLocalVelocityDiv));this.internalTex1=p(b,a,1,d(this.qVelocity,this.qVelocityDiv));c=this.qRotSpeed;for(var l=this.qScale,h=this.qScaleDiv,e=this.qRotSpeedDiv,k=this.qAlphaDiv,n=Array(4*c.length),m=0;m<c.length;m++)n[4*m]=c[m],n[4*m+1]=l[m],n[4*m+2]=0,n[4*m+3]=(255*h[m]<<16|255*e[m]<<8|255*k[m])/16777216;this.internalTex2=p(b,a,1,n)}c=this.qColor;l=this.qAlpha;h=Array(4*l.length);for(e=0;e<l.length;e++)h[4*e]=c[3*e],h[4*e+1]=c[3*e+1],h[4*e+2]=c[3*e+2],h[4*e+3]=l[e];this.internalTex3=p(b,a,1,
h,pc.PIXELFORMAT_R8_G8_B8_A8,1,!0)},_initializeTextures:function(){this.colorMap&&(this.material.setParameter("colorMap",this.colorMap),this.lighting&&this.normalMap&&this.material.setParameter("normalMap",this.normalMap))},regenShader:function(){var a=this.graphicsDevice.getProgramLibrary(),b=null!==this.normalMap;this.normalOption=0;this.lighting&&(this.normalOption=b?2:1);this.material.updateShader=function(){this.emitter.scene&&this.emitter.camera!=this.emitter.scene._activeCamera&&(this.emitter.camera=
this.emitter.scene._activeCamera,this.emitter.onChangeCamera());this.shader=a.getProgram("particle",{useCpu:this.emitter.useCpu,normal:this.emitter.normalOption,halflambert:this.emitter.halfLambert,stretch:this.emitter.stretch,alignToMotion:this.emitter.alignToMotion,soft:this.emitter.depthSoftening,mesh:this.emitter.useMesh,gamma:this.emitter.scene?this.emitter.scene.gammaCorrection:0,toneMap:this.emitter.scene?this.emitter.scene.toneMapping:0,fog:this.emitter.scene&&!this.emitter.noFog?this.emitter.scene.fog:
...
this.precision);a.setParameter("graphSampleSize",1/this.precision);a.setParameter("emitterScale",new Float32Array([1,1,1]));this.pack8&&(this.inBoundsSizeUniform[0]=this.worldBoundsSize.x,this.inBoundsSizeUniform[1]=this.worldBoundsSize.y,this.inBoundsSizeUniform[2]=this.worldBoundsSize.z,a.setParameter("inBoundsSize",this.inBoundsSizeUniform),this.inBoundsCenterUniform[0]=this.worldBounds.center.x,this.inBoundsCenterUniform[1]=this.worldBounds.center.y,this.inBoundsCenterUniform[2]=this.worldBounds.center.z,
a.setParameter("inBoundsCenter",this.inBoundsCenterUniform),a.setParameter("maxVel",this.maxVel));this.wrap&&this.wrapBounds&&(this.wrapBoundsUniform[0]=this.wrapBounds.x,this.wrapBoundsUniform[1]=this.wrapBounds.y,this.wrapBoundsUniform[2]=this.wrapBounds.z,a.setParameter("wrapBounds",this.wrapBoundsUniform));this.colorMap&&a.setParameter("colorMap",this.colorMap);this.lighting&&this.normalMap&&a.setParameter("normalMap",this.normalMap);0<this.depthSoftening&&a.setParameter("softening",1/(this.depthSoftening*
...
components:4,type:pc.TYPE_FLOAT32}];d=new pc.VertexFormat(this.graphicsDevice,d);this.vertexBuffer=new pc.VertexBuffer(this.graphicsDevice,d,b,pc.BUFFER_DYNAMIC);this.indexBuffer=new pc.IndexBuffer(this.graphicsDevice,pc.INDEXFORMAT_UINT16,c);d=new Float32Array(this.vertexBuffer.lock());var f,g;this.useMesh&&(f=new Float32Array(this.mesh.vertexBuffer.lock()),g=f.length/this.mesh.vertexBuffer.numVertices);for(var e,c=0;c<b;c++){e=Math.floor(c/this.numParticleVerts);if(this.useMesh){var k=c%this.numParticleVerts;
d[4*c]=f[k*g];d[4*c+1]=f[k*g+1];d[4*c+2]=f[k*g+2]}else k=c%4,d[4*c]=l[k][0],d[4*c+1]=l[k][1],d[4*c+2]=0;d[4*c+3]=e}this.useCpu&&(this.vbCPU=new Float32Array(d),this.vbOld=new Float32Array(this.vbCPU.length));this.vertexBuffer.unlock();this.useMesh&&this.mesh.vertexBuffer.unlock();b=0;g=new Uint16Array(this.indexBuffer.lock());this.useMesh&&(f=new Uint16Array(this.mesh.indexBuffer[0].lock()));for(c=0;c<a;c++)if(this.useMesh)for(d=0;d<this.numParticleIndices;d++)g[c*this.numParticleIndices+d]=f[d]+
c*this.numParticleVerts;else d=4*c,g[b++]=d,g[b++]=d+1,g[b++]=d+2,g[b++]=d,g[b++]=d+2,g[b++]=d+3;this.indexBuffer.unlock();this.useMesh&&this.mesh.indexBuffer[0].unlock()}},reset:function(){this.beenReset=!0;this.seed=Math.random();this.material.setParameter("seed",this.seed);if(this.useCpu)for(var a=0;a<this.particleTexStart.length;a++)this.particleTex[a]=this.particleTexStart[a];else this._initializeTextures();this.resetTime();a=this.loop;this.loop=!0;this.addTime(0);this.loop=a;this.preWarm&&this.prewarm(this.lifetime)},
prewarm:function(a){var b=Math.min(Math.floor(a/this.lifetime*this.precision),this.precision);a/=b;for(var c=0;c<b;c++)this.addTime(a)},resetTime:function(){var a=Math.max(this.rate,this.rate2)*this.numParticles+this.lifetime;this.endTime=Date.now()+1E3*a},finishFrame:function(){this.useCpu&&this.vertexBuffer.unlock()},addTime:function(b,c){var d,f,g,e,l;e=this.graphicsDevice;this.simTimeTotal+=b;this.calculateWorldBounds();this._isAnimated()&&(l=this.animParams,l[0]=1/this.animTilesX,l[1]=1/this.animTilesY,
l[2]=this.animNumFrames*this.animSpeed,l[3]=this.animNumFrames-1);this.scene&&this.camera!=this.scene._activeCamera&&(this.camera=this.scene._activeCamera,this.onChangeCamera());this.emitterShape===pc.EMITTERSHAPE_BOX&&(null===this.meshInstance.node?D.setTRS(pc.Vec3.ZERO,pc.Quat.IDENTITY,this.emitterExtents):D.setTRS(pc.Vec3.ZERO,this.meshInstance.node.getRotation(),S.copy(this.emitterExtents).mul(this.meshInstance.node.localScale)));var h=null===this.meshInstance.node?pc.Vec3.ONE:this.meshInstance.node.localScale;
this.emitterScaleUniform[0]=h.x;this.emitterScaleUniform[1]=h.y;this.emitterScaleUniform[2]=h.z;this.material.setParameter("emitterScale",this.emitterScaleUniform);this.localSpace&&this.meshInstance.node&&(l=this.meshInstance.node.getPosition(),this.emitterPosUniform[0]=l.x,this.emitterPosUniform[1]=l.y,this.emitterPosUniform[2]=l.z,this.material.setParameter("emitterPos",this.emitterPosUniform));if(this.useCpu){h=new Float32Array(this.vertexBuffer.lock());if(this.meshInstance.node){e=this.meshInstance.node.worldTransform;
for(l=0;12>l;l++)z.data[l]=e.data[l];G=this.meshInstance.node.localScale;H=Math.max(Math.max(G.x,G.y),G.z)}l=null===this.meshInstance.node||this.localSpace?pc.Vec3.ZERO:this.meshInstance.node.getPosition();var p=this.camera?this.camera._node.getPosition():pc.Vec3.ZERO,n,m,q,r,I,V,X,W=this.precision-1;for(e=0;e<this.numParticles;e++){var P=Math.floor(this.vbCPU[e*this.numParticleVerts*4+3]),N=this.particleTex[4*P+0+8*this.numParticlesPot];u.x=N;u.y=this.particleTex[4*P+1+8*this.numParticlesPot];u.z=
this.particleTex[4*P+2+8*this.numParticlesPot];var U=this.rate+(this.rate2-this.rate)*N,R=this.lifetime,Y=this.particleTex[4*P+3+4*this.numParticlesPot]+b,la=Math.max(Math.min(Y/R,1),0),ha=0;d=0;var ka=0<Y&&Y<R;ka?(g=la*W,n=Math.floor(g),m=Math.ceil(g),g%=1,d=this.qRotSpeed[n],f=this.qRotSpeed[m],q=d+(f-d)*g,d=this.qRotSpeed2[n],f=this.qRotSpeed2[m],r=d+(f-d)*g,d=this.qScale[n],f=this.qScale[m],ha=d+(f-d)*g,d=this.qScale2[n],f=this.qScale2[m],I=d+(f-d)*g,d=this.qAlpha[n],f=this.qAlpha[m],V=d+(f-d)*
g,d=this.qAlpha2[n],f=this.qAlpha2[m],X=d+(f-d)*g,n*=3,m*=3,d=this.qLocalVelocity[n],f=this.qLocalVelocity[m],v.x=d+(f-d)*g,d=this.qLocalVelocity[n+1],f=this.qLocalVelocity[m+1],v.y=d+(f-d)*g,d=this.qLocalVelocity[n+2],f=this.qLocalVelocity[m+2],v.z=d+(f-d)*g,d=this.qLocalVelocity2[n],f=this.qLocalVelocity2[m],w.x=d+(f-d)*g,d=this.qLocalVelocity2[n+1],f=this.qLocalVelocity2[m+1],w.y=d+(f-d)*g,d=this.qLocalVelocity2[n+2],f=this.qLocalVelocity2[m+2],w.z=d+(f-d)*g,d=this.qVelocity[n],f=this.qVelocity[m],
t.x=d+(f-d)*g,d=this.qVelocity[n+1],f=this.qVelocity[m+1],t.y=d+(f-d)*g,d=this.qVelocity[n+2],f=this.qVelocity[m+2],t.z=d+(f-d)*g,d=this.qVelocity2[n],f=this.qVelocity2[m],y.x=d+(f-d)*g,d=this.qVelocity2[n+1],f=this.qVelocity2[m+1],y.y=d+(f-d)*g,d=this.qVelocity2[n+2],f=this.qVelocity2[m+2],y.z=d+(f-d)*g,v.x+=(w.x-v.x)*u.x,v.y+=(w.y-v.y)*u.y,v.z+=(w.z-v.z)*u.z,0<this.initialVelocity&&(this.emitterShape===pc.EMITTERSHAPE_SPHERE?(F.copy(u).scale(2).sub(pc.Vec3.ONE).normalize(),v.add(F.scale(this.initialVelocity))):
v.add(pc.Vec3.FORWARD.scale(this.initialVelocity))),t.x+=(y.x-t.x)*u.x,t.y+=(y.y-t.y)*u.y,t.z+=(y.z-t.z)*u.z,q+=(r-q)*u.y,ha=(ha+1E4*N%1*(I-ha))*H,d=1E3*N%1*(X-V),this.meshInstance.node&&z.transformPoint(v,v),v.add(t.mul(G)),A.copy(v),x.x=this.particleTex[4*P],x.y=this.particleTex[4*P+1],x.z=this.particleTex[4*P+2],B.copy(x).add(v.scale(b)),C.copy(B),this.particleTex[4*P]=C.x,this.particleTex[4*P+1]=C.y,this.particleTex[4*P+2]=C.z,this.particleTex[4*P+3]+=q*b,this.wrap&&this.wrapBounds&&(C.sub(l),
C.x=a(C.x,this.wrapBounds.x)-.5*this.wrapBounds.x,C.y=a(C.y,this.wrapBounds.y)-.5*this.wrapBounds.y,C.z=a(C.z,this.wrapBounds.z)-.5*this.wrapBounds.z,C.add(l)),0<this.sort&&(1===this.sort?(S.copy(C).sub(p),this.particleDistance[P]=-(S.x*S.x+S.y*S.y+S.z*S.z)):2===this.sort?this.particleDistance[P]=Y:3===this.sort&&(this.particleDistance[P]=-Y))):this.calcSpawnPosition(l,P);c?0>Y&&(this.particleTex[4*P+3+8*this.numParticlesPot]=-1):(Y>=R&&(Y-=Math.max(R,(this.numParticles-1)*U),this.particleTex[4*P+
3+8*this.numParticlesPot]=this.loop?1:-1),0>Y&&this.loop&&(this.particleTex[4*P+3+8*this.numParticlesPot]=1));0>this.particleTex[4*P+3+8*this.numParticlesPot]&&(ka=!1);this.particleTex[4*P+3+4*this.numParticlesPot]=Y;for(q=0;q<this.numParticleVerts;q++)N=this.vbCPU[e*this.numParticleVerts*4+4*q],U=this.vbCPU[e*this.numParticleVerts*4+4*q+1],R=this.vbCPU[e*this.numParticleVerts*4+4*q+2],ka||(N=U=R=0),Y=e*this.numParticleVerts*14+14*q,h[Y]=C.x,h[Y+1]=C.y,h[Y+2]=C.z,h[Y+3]=la,h[Y+4]=this.alignToMotion?
0:this.particleTex[4*P+3],h[Y+5]=ha,h[Y+6]=d,h[Y+7]=A.x,h[Y+8]=N,h[Y+9]=U,h[Y+10]=R,h[Y+11]=A.y,h[Y+12]=A.z}if(this.sort>pc.PARTICLESORT_NONE&&this.camera){l=this.particleDistance;for(e=0;e<this.numParticles;e++)this.vbToSort[e]=[e,l[Math.floor(this.vbCPU[e*this.numParticleVerts*4+3])]];this.vbOld.set(this.vbCPU);this.vbToSort.sort(function(a,b){return a[1]-b[1]});for(e=0;e<this.numParticles;e++)for(h=this.vbToSort[e][0]*this.numParticleVerts*4,p=e*this.numParticleVerts*4,l=0;l<4*this.numParticleVerts;l++)this.vbCPU[p+
l]=this.vbOld[h+l]}}else e.setBlending(!1),e.setColorWrite(!0,!0,!0,!0),e.setCullMode(pc.CULLFACE_NONE),e.setDepthTest(!1),e.setDepthWrite(!1),this.frameRandomUniform[0]=Math.random(),this.frameRandomUniform[1]=Math.random(),this.frameRandomUniform[2]=Math.random(),this.constantGraphSampleSize.setValue(1/this.precision),this.constantGraphNumSamples.setValue(this.precision),this.constantNumParticles.setValue(this.numParticles),this.constantNumParticlesPot.setValue(this.numParticlesPot),this.constantInternalTex0.setValue(this.internalTex0),
this.constantInternalTex1.setValue(this.internalTex1),this.constantInternalTex2.setValue(this.internalTex2),this.pack8&&(this.worldBoundsMulUniform[0]=this.worldBoundsMul.x,this.worldBoundsMulUniform[1]=this.worldBoundsMul.y,this.worldBoundsMulUniform[2]=this.worldBoundsMul.z,this.constantOutBoundsMul.setValue(this.worldBoundsMulUniform),this.worldBoundsAddUniform[0]=this.worldBoundsAdd.x,this.worldBoundsAddUniform[1]=this.worldBoundsAdd.y,this.worldBoundsAddUniform[2]=this.worldBoundsAdd.z,this.constantOutBoundsAdd.setValue(this.worldBoundsAddUniform),
this.inBoundsSizeUniform[0]=this.prevWorldBoundsSize.x,this.inBoundsSizeUniform[1]=this.prevWorldBoundsSize.y,this.inBoundsSizeUniform[2]=this.prevWorldBoundsSize.z,this.constantInBoundsSize.setValue(this.inBoundsSizeUniform),this.inBoundsCenterUniform[0]=this.prevWorldBoundsCenter.x,this.inBoundsCenterUniform[1]=this.prevWorldBoundsCenter.y,this.inBoundsCenterUniform[2]=this.prevWorldBoundsCenter.z,this.constantInBoundsCenter.setValue(this.inBoundsCenterUniform),l=this.maxVel*Math.max(Math.max(h.x,
h.y),h.z),l=Math.max(l,1),this.constantMaxVel.setValue(l)),l=null===this.meshInstance.node||this.localSpace?pc.Vec3.ZERO:this.meshInstance.node.getPosition(),p=null===this.meshInstance.node?pc.Mat4.IDENTITY:this.meshInstance.node.getWorldTransform(),this.emitterShape===pc.EMITTERSHAPE_BOX?(k(D,E),this.constantSpawnBounds.setValue(E.data)):this.constantSpawnBoundsSphere.setValue(this.emitterRadius),this.constantInitialVelocity.setValue(this.initialVelocity),k(p,J),this.emitterPosUniform[0]=l.x,this.emitterPosUniform[1]=
l.y,this.emitterPosUniform[2]=l.z,this.constantEmitterPos.setValue(this.emitterPosUniform),this.constantFrameRandom.setValue(this.frameRandomUniform),this.constantDelta.setValue(b),this.constantRate.setValue(this.rate),this.constantRateDiv.setValue(this.rate2-this.rate),this.constantStartAngle.setValue(this.startAngle*pc.math.DEG_TO_RAD),this.constantStartAngle2.setValue(this.startAngle2*pc.math.DEG_TO_RAD),this.constantSeed.setValue(this.seed),this.constantLifetime.setValue(this.lifetime),this.emitterScaleUniform[0]=
h.x,this.emitterScaleUniform[1]=h.y,this.emitterScaleUniform[2]=h.z,this.constantEmitterScale.setValue(this.emitterScaleUniform),this.constantEmitterMatrix.setValue(J.data),this.constantLocalVelocityDivMult.setValue(this.localVelocityUMax),this.constantVelocityDivMult.setValue(this.velocityUMax),this.constantRotSpeedDivMult.setValue(this.rotSpeedUMax[0]),l=this.swapTex?this.particleTexOUT:this.particleTexIN,l=this.beenReset?this.particleTexStart:l,h=this.swapTex?this.particleTexIN:this.particleTexOUT,
this.constantParticleTexIN.setValue(l),c?pc.drawQuadWithShader(e,this.swapTex?this.rtParticleTexIN:this.rtParticleTexOUT,this.shaderParticleUpdateOnStop):pc.drawQuadWithShader(e,this.swapTex?this.rtParticleTexIN:this.rtParticleTexOUT,this.loop?this.shaderParticleUpdateRespawn:this.shaderParticleUpdateNoRespawn),this.constantParticleTexOUT.setValue(h),this.material.setParameter("particleTexOUT",l),this.material.setParameter("particleTexIN",h),this.beenReset=!1,this.swapTex=!this.swapTex,e.setDepthTest(!0),
e.setDepthWrite(!0);if(!this.loop&&Date.now()>this.endTime){if(this.onFinished)this.onFinished();this.meshInstance.visible=!1}},destroy:function(){this.particleTexIN&&this.particleTexIN.destroy();this.particleTexOUT&&this.particleTexOUT.destroy();!this.useCpu&&this.particleTexStart&&this.particleTexStart.destroy();this.rtParticleTexIN&&this.rtParticleTexIN.destroy();this.rtParticleTexOUT&&this.rtParticleTexOUT.destroy();this.shaderParticleUpdateOnStop=this.shaderParticleUpdateNoRespawn=this.shaderParticleUpdateRespawn=
this.rtParticleTexOUT=this.rtParticleTexIN=this.particleTexStart=this.particleTexOUT=this.particleTexIN=null;this.vertexBuffer&&this.vertexBuffer.destroy()}});return{ParticleEmitter:U}}());Object.assign(pc,function(){var e=!1,b=function(a,b,d){a instanceof pc.GraphicsDevice&&(a=pc.Application.getApplication(),e||(e=!0));this.app=a;var c=this.device=a.graphicsDevice;this.library=c.getProgramLibrary();this.pickColor=new Float32Array(4);this.pickColor[3]=1;this.scene=null;this.drawCalls=[];this.layerComp=this.layer=null;this.clearOptions={color:[1,1,1,1],depth:1,flags:pc.CLEARFLAG_COLOR|pc.CLEARFLAG_DEPTH};var g=this;this._clearDepthOptions={depth:1,flags:pc.CLEARFLAG_DEPTH};this.clearDepthCommand=
...
pc.calculateTangents=function(e,b,a,c){var d=c.length/3,f=e.length/3,g,k,l,p,h,n,q,m,r,t,v,y,w,u,x=new pc.Vec3,B=new pc.Vec3,C=new pc.Vec3,A=new pc.Vec3,z=new pc.Vec3,E=new pc.Vec2,J=new pc.Vec2,H=new pc.Vec2,G,D=new Float32Array(3*f),F=new Float32Array(3*f),I=[];for(G=0;G<d;G++)g=c[3*G],k=c[3*G+1],l=c[3*G+2],C.set(e[3*g],e[3*g+1],e[3*g+2]),A.set(e[3*k],e[3*k+1],e[3*k+2]),z.set(e[3*l],e[3*l+1],e[3*l+2]),E.set(a[2*g],a[2*g+1]),J.set(a[2*k],a[2*k+1]),H.set(a[2*l],a[2*l+1]),p=A.x-C.x,h=z.x-C.x,n=A.y-
C.y,q=z.y-C.y,m=A.z-C.z,r=z.z-C.z,t=J.x-E.x,v=H.x-E.x,y=J.y-E.y,w=H.y-E.y,u=t*w-v*y,0==u?(x.set(0,1,0),B.set(1,0,0)):(u=1/u,x.set((w*p-y*h)*u,(w*n-y*q)*u,(w*m-y*r)*u),B.set((t*h-v*p)*u,(t*q-v*n)*u,(t*r-v*m)*u)),D[3*g+0]+=x.x,D[3*g+1]+=x.y,D[3*g+2]+=x.z,D[3*k+0]+=x.x,D[3*k+1]+=x.y,D[3*k+2]+=x.z,D[3*l+0]+=x.x,D[3*l+1]+=x.y,D[3*l+2]+=x.z,F[3*g+0]+=B.x,F[3*g+1]+=B.y,F[3*g+2]+=B.z,F[3*k+0]+=B.x,F[3*k+1]+=B.y,F[3*k+2]+=B.z,F[3*l+0]+=B.x,F[3*l+1]+=B.y,F[3*l+2]+=B.z;y=new pc.Vec3;w=new pc.Vec3;e=new pc.Vec3;
a=new pc.Vec3;for(G=0;G<f;G++)e.set(b[3*G],b[3*G+1],b[3*G+2]),y.set(D[3*G],D[3*G+1],D[3*G+2]),w.set(F[3*G],F[3*G+1],F[3*G+2]),c=e.dot(y),a.copy(e).scale(c),a.sub2(y,a).normalize(),I[4*G]=a.x,I[4*G+1]=a.y,I[4*G+2]=a.z,a.cross(e,y),I[4*G+3]=0>a.dot(w)?-1:1;return I};
...
[]};l.prototype.clearVisibleLists=function(a){this.visibleOpaque[a]&&(this.visibleOpaque[a].length=0,this.visibleOpaque[a].list.length=0);this.visibleTransparent[a]&&(this.visibleTransparent[a].length=0,this.visibleTransparent[a].list.length=0)};var p=function(a){a=a||{};void 0!==a.id?(this.id=a.id,k=Math.max(this.id+1,k)):this.id=k++;this.name=a.name;this._refCounter=(this._enabled=void 0===a.enabled?!0:a.enabled)?1:0;this.opaqueSortMode=void 0===a.opaqueSortMode?pc.SORTMODE_MATERIALMESH:a.opaqueSortMode;
...
[];this._pressedElement=this._hoveredElement=null;this._touchedElements={};this._touchesForWhichTouchLeaveHasFired={};this._useMouse=!b||!1!==b.useMouse;this._useTouch=!b||!1!==b.useTouch;"ontouchstart"in window&&(this._clickedEntities={});this.attach(a,b)};Object.assign(E.prototype,{attach:function(a){this._attached&&(this._attached=!1,this.detach());this._target=a;this._attached=!0;this._useMouse&&(window.addEventListener("mouseup",this._upHandler,{passive:!0}),window.addEventListener("mousedown",
this._downHandler,{passive:!0}),window.addEventListener("mousemove",this._moveHandler,{passive:!0}),window.addEventListener("mousewheel",this._wheelHandler,{passive:!0}),window.addEventListener("DOMMouseScroll",this._wheelHandler,{passive:!0}));this._useTouch&&"ontouchstart"in window&&(this._target.addEventListener("touchstart",this._touchstartHandler,{passive:!0}),this._target.addEventListener("touchend",this._touchendHandler,!1),this._target.addEventListener("touchmove",this._touchmoveHandler,!1),
...
d,f,g;for(d=c.length-1;0<=d;d--){var e=c[d],k=0;f=0;for(g=a.changedTouches.length;f<g;f++)if(b[a.changedTouches[f].identifier])k++;else{var l=this._calcTouchCoords(a.changedTouches[f]),h=this._getTargetElement(e,l.x,l.y);h&&(k++,b[a.changedTouches[f].identifier]={element:h,camera:e,x:l.x,y:l.y})}if(k===g)break}return b},_handleTouchStart:function(a){if(this._enabled){for(var b=this._determineTouchedElements(a),c=0,d=a.changedTouches.length;c<d;c++){var f=a.changedTouches[c],g=b[f.identifier],e=this._touchedElements[f.identifier];
!g||e&&g.element===e.element||(this._fireEvent(a.type,new z(a,g.element,g.camera,g.x,g.y,this)),this._touchesForWhichTouchLeaveHasFired[f.identifier]=!1)}for(var k in b)this._touchedElements[k]=b[k]}},_handleTouchEnd:function(a){if(this._enabled){var b=this.app.systems.camera.cameras,c;for(c in this._clickedEntities)delete this._clickedEntities[c];c=0;for(var d=a.changedTouches.length;c<d;c++){var f=a.changedTouches[c],g=this._touchedElements[f.identifier];if(g){var e=g.element,k=g.camera,l=g.x,g=
g.y;delete this._touchedElements[f.identifier];delete this._touchesForWhichTouchLeaveHasFired[f.identifier];this._fireEvent(a.type,new z(a,e,k,l,g,this));if(0===a.touches.length)for(var f=this._calcTouchCoords(f),h=b.length-1;0<=h;h--)this._getTargetElement(b[h],f.x,f.y)!==e||this._clickedEntities[e.entity.getGuid()]||(this._fireEvent("click",new z(a,e,k,l,g,this)),this._clickedEntities[e.entity.getGuid()]=!0)}}}},_handleTouchMove:function(a){if(this._enabled){a.preventDefault();for(var b=this._determineTouchedElements(a),
c=0,d=a.changedTouches.length;c<d;c++){var f=a.changedTouches[c],g=b[f.identifier],e=this._touchedElements[f.identifier];if(e){var k=this._calcTouchCoords(f);g&&g.element===e.element||this._touchesForWhichTouchLeaveHasFired[f.identifier]||(this._fireEvent("touchleave",new z(a,e.element,e.camera,k.x,k.y,this)),this._touchesForWhichTouchLeaveHasFired[f.identifier]=!0);this._fireEvent("touchmove",new z(a,e.element,e.camera,k.x,k.y,this))}}}},_onElementMouseEvent:function(a){var c,d=this._hoveredElement;
this._hoveredElement=null;for(var f=this.app.systems.camera.cameras,g,k=f.length-1;0<=k&&!(g=f[k],c=this._getTargetElement(g,e,b));k--);c&&(this._fireEvent(a.type,new A(a,c,g,e,b,this._lastX,this._lastY)),this._hoveredElement=c,a.type===pc.EVENT_MOUSEDOWN&&(this._pressedElement=c));d!==this._hoveredElement&&(d&&this._fireEvent("mouseleave",new A(a,d,g,e,b,this._lastX,this._lastY)),this._hoveredElement&&this._fireEvent("mouseenter",new A(a,this._hoveredElement,g,e,b,this._lastX,this._lastY)));a.type===
pc.EVENT_MOUSEUP&&this._pressedElement&&(this._pressedElement===this._hoveredElement?(this._pressedElement=null,this._clickedEntities&&this._clickedEntities[this._hoveredElement.entity.getGuid()]||this._fireEvent("click",new A(a,this._hoveredElement,g,e,b,this._lastX,this._lastY))):this._pressedElement=null)},_fireEvent:function(a,b){for(var c=b.element;;){c.fire(a,b);if(b._stopPropagation)break;if(!c.entity.parent)break;c=c.entity.parent.element;if(!c)break}},_calcMouseCoords:function(a){var c=this._target.getBoundingClientRect(),
d=Math.floor(c.left),c=Math.floor(c.top);a.clientX<d||a.clientX>=d+this._target.clientWidth||a.clientY<c||a.clientY>=c+this._target.clientHeight?b=e=null:(e=a.clientX-d,b=a.clientY-c)},_calcTouchCoords:function(a){for(var b=0,c=0,d=a.target;!(d instanceof HTMLElement);)d=d.parentNode;do b+=d.offsetLeft-d.scrollLeft,c+=d.offsetTop-d.scrollTop,d=d.offsetParent;while(d);return{x:a.pageX-b,y:a.pageY-c}},_sortElements:function(a,b){return a.screen&&!b.screen?-1:!a.screen&&b.screen?1:a.screen||b.screen?
a.screen.screen.screenSpace&&!b.screen.screen.screenSpace?-1:b.screen.screen.screenSpace&&!a.screen.screen.screenSpace?1:b.drawOrder-a.drawOrder:0},_getTargetElement:function(a,b,c){var d=null;this._elements.sort(this._sortElements);for(var f=0,g=this._elements.length;f<g;f++){var e=this._elements[f];if(e.screen&&e.screen.screen.screenSpace){if(this._checkElement2d(b,c,e,a)){d=e;break}}else if(this._checkElement3d(b,c,e,a)){d=e;break}}return d},_buildHitCorners:function(a,b,c,d){if(a.entity&&a.entity.button){var f=
a.entity.button.hitPadding||x;q.copy(a.entity.up);m.copy(q).scale(-1);t.copy(a.entity.right);r.copy(t).scale(-1);q.scale(f.w*d);m.scale(f.y*d);t.scale(f.z*c);r.scale(f.x*c);v.copy(b[0]).add(m).add(r);y.copy(b[1]).add(m).add(t);w.copy(b[2]).add(q).add(t);u.copy(b[3]).add(q).add(r);b=[v,y,w,u]}return b},_calculateScaleToScreen:function(a){var b=a.entity;a=a.screen.screen.scale;for(n.set(a,a);b&&!b.screen;)n.mul(b.getLocalScale()),b=b.parent;return n},_checkElement2d:function(b,d,f,g){if(f.maskedBy&&
!this._checkElement2d(b,d,f.maskedBy.element,g))return!1;var e=this.app.graphicsDevice.width,k=this.app.graphicsDevice.height,l=g.rect.z*e,h=g.rect.w*k,p=g.rect.x*e;g=(1-g.rect.y)*k;var n=g-h;b=b*e/this._target.clientWidth;d=d*k/this._target.clientHeight;return b>=p&&b<=p+l&&d<=g&&d>=n&&(b=e*(b-p)/l,d=k-k*(d-n)/h,e=this._calculateScaleToScreen(f),f=this._buildHitCorners(f,f.screenCorners,e.x,e.y),a.set(b,d,1),c.set(b,d,-1),B(a,c,f))?!0:!1},_checkElement3d:function(b,d,f,g){if(f.maskedBy&&!this._checkElement3d(b,
d,f.maskedBy.element,g))return!1;var e=this._target.clientWidth,k=this._target.clientHeight,l=g.rect.z*e,h=g.rect.w*k,p=g.rect.x*e,n=(1-g.rect.y)*k,m=n-h,q=d;return b>=p&&b<=p+l&&d<=n&&q>=m&&(b=e*(b-p)/l,q=k*(q-m)/h,k=f.entity.getWorldTransform().getScale(),f=this._buildHitCorners(f,f.worldCorners,k.x,k.y),g.screenToWorld(b,q,g.nearClip,a),g.screenToWorld(b,q,g.farClip,c),B(a,c,f))?!0:!1}});Object.defineProperty(E.prototype,"enabled",{get:function(){return this._enabled},set:function(a){this._enabled=
a}});Object.defineProperty(E.prototype,"app",{get:function(){return this._app||pc.app},set:function(a){this._app=a}});return{ElementInput:E,ElementInputEvent:C,ElementMouseEvent:A,ElementTouchEvent:z}}());Object.assign(pc,function(){var e=function(b){pc.events.attach(this);var a=this;this.isSupported=e.isSupported;this.usesPolyfill=e.usesPolyfill;window.InitializeWebVRPolyfill&&window.InitializeWebVRPolyfill();this._index={};this.displays=[];this.display=null;this._app=b;this._onDisplayConnect=this._onDisplayConnect.bind(this);this._onDisplayDisconnect=this._onDisplayDisconnect.bind(this);a._attach();this._getDisplays(function(b,d){if(b)a.fire("error",b);else{for(var c=0;c<d.length;c++)a._addDisplay(d[c]);
...
null);pc.ComponentSystem.destroy();for(var a=this.assets.list(),b=0;b<a.length;b++)a[b].unload();this.loader.destroy();this.scene=this.loader=null;this.systems=[];this.context=null;pc.destroyPostEffectQuad();this.graphicsDevice.destroy();this.tick=this.renderer=this.graphicsDevice=null;this._audioManager&&(this._audioManager.destroy(),this._audioManager=null);pc.http=new pc.Http;pc.ParticleEmitter.DEFAULT_PARAM_TEXTURE=null}});var a={},c=function(b){return function(c){if(b.graphicsDevice){e._currentApplication=
...
d;for(d in b)if(b.hasOwnProperty(d)){var f=b[d],e=f.data;e.enabled&&e.playing&&f.entity.enabled&&(f=e.skeleton,null!==f&&null!==e.model&&(e.blending?(e.blendTimeRemaining-=a,0>e.blendTimeRemaining&&(e.blendTimeRemaining=0),f.blend(e.fromSkel,e.toSkel,1-e.blendTimeRemaining/e.blendTime)):(f.addTime(a*e.speed),f._time!==f._animation.duration||e.loop||(e.playing=!1)),e.blending&&0===e.blendTimeRemaining&&(e.blending=!1,f.animation=e.toSkel._animation),f.updateGraph()))}}});return{AnimationComponentSystem:b}}());Object.assign(pc,function(){return{AnimationComponentData:function(){this.assets=[];this.speed=1;this.enabled=this.activate=this.loop=!0;this.animations={};this.toSkel=this.fromSkel=this.currAnim=this.prevAnim=this.model=this.skeleton=null;this.blending=!1;this.blendTimeRemaining=this.blendTime=0;this.playing=!1}}}());Object.assign(pc,function(){var e=function(b,a){pc.Component.call(this,b,a);this._type="asset";this._model=this._asset=null;this._mapping={};this._receiveShadows=this._castShadows=!0;this._materialAsset=null;this._material=pc.ModelHandler.DEFAULT_MATERIAL;this._castShadowsLightmap=!0;this._lightmapped=!1;this._lightmapSizeMultiplier=1;this._isStatic=!1;this._layers=[pc.LAYERID_WORLD];this._batchGroupId=-1;this._area=null;this._assetOld=0;this._materialEvents=null;this._clonedModel=this._dirtyMaterialAsset=
this._dirtyModelAsset=!1};e.prototype=Object.create(pc.Component.prototype);e.prototype.constructor=e;Object.assign(e.prototype,{setVisible:function(b){console.warn("WARNING: setVisible: Function is deprecated. Set enabled property instead.");this.enabled=b},addModelToLayers:function(){for(var b,a=this.system.app.scene.layers,c=0;c<this._layers.length;c++)(b=a.getLayerById(this._layers[c]))&&b.addMeshInstances(this.meshInstances)},removeModelFromLayers:function(b){for(var a,c=this.system.app.scene.layers,
d=0;d<this._layers.length;d++)(a=c.getLayerById(this._layers[d]))&&a.removeMeshInstances(b.meshInstances)},onRemove:function(){this.materialAsset=this.asset=null},onLayersChanged:function(b,a){this.addModelToLayers();b.off("add",this.onLayerAdded,this);b.off("remove",this.onLayerRemoved,this);a.on("add",this.onLayerAdded,this);a.on("remove",this.onLayerRemoved,this)},onLayerAdded:function(b){0>this.layers.indexOf(b.id)||b.addMeshInstances(this.meshInstances)},onLayerRemoved:function(b){0>this.layers.indexOf(b.id)||
...
pc.MASK_LIGHTMAP)}}}});Object.defineProperty(e.prototype,"castShadows",{get:function(){return this._castShadows},set:function(b){if(this._castShadows!==b){var a,c,d=this._model;if(d){var f=this.layers,e=this.system.app.scene;if(this._castShadows&&!b)for(c=0;c<f.length;c++)(a=this.system.app.scene.layers.getLayerById(this.layers[c]))&&a.removeShadowCasters(d.meshInstances);a=d.meshInstances;for(c=0;c<a.length;c++)a[c].castShadow=b;if(!this._castShadows&&b)for(c=0;c<f.length;c++)(a=e.layers.getLayerById(f[c]))&&
...
if(this.meshInstances)for(a=0;a<this._layers.length;a++)(c=d.getLayerById(this._layers[a]))&&c.removeMeshInstances(this.meshInstances);for(a=this._layers.length=0;a<b.length;a++)this._layers[0]=b[0];if(this.enabled&&this.entity.enabled&&this.meshInstances)for(a=0;a<this._layers.length;a++)(c=d.getLayerById(this._layers[a]))&&c.addMeshInstances(this.meshInstances)}});Object.defineProperty(e.prototype,"batchGroupId",{get:function(){return this._batchGroupId},set:function(b){if(this._batchGroupId!==
b){this._batchGroupId=b;var a=this.system.app.batcher;0<=this._batchGroupId&&a.markGroupDirty(this._batchGroupId);0<=b&&a.markGroupDirty(b);0>b&&0<=this._batchGroupId&&this.enabled&&this.entity.enabled&&this.addModelToLayers()}}});Object.defineProperty(e.prototype,"materialAsset",{get:function(){return this._materialAsset},set:function(b){var a=b;b instanceof pc.Asset&&(a=b.id);b=this.system.app.assets;if(a!==this._materialAsset){if(this._materialAsset){b.off("add:"+this._materialAsset,this._onMaterialAdded,
this);var c=b.get(this._materialAsset);c&&this._unbindMaterialAsset(c)}(this._materialAsset=a)?(a=b.get(this._materialAsset))?this._bindMaterialAsset(a):(this.material=pc.ModelHandler.DEFAULT_MATERIAL,b.on("add:"+this._materialAsset,this._onMaterialAdded,this)):this.material=pc.ModelHandler.DEFAULT_MATERIAL}}});Object.defineProperty(e.prototype,"material",{get:function(){return this._material},set:function(b){if(this._material!==b){this._material=b;var a=this._model;if(a&&"asset"!==this._type)for(var a=
a.meshInstances,c=0,d=a.length;c<d;c++)a[c].material=b}}});Object.defineProperty(e.prototype,"mapping",{get:function(){return this._mapping},set:function(b){if("asset"===this._type&&(this._unsetMaterialEvents(),b||(b={}),this._mapping=b,this._model))for(var a=this._model.meshInstances,c=this.asset?this.system.app.assets.get(this.asset):null,c=c?c.data.mapping:null,d=null,f=0,e=a.length;f<e;f++)if(void 0!==b[f])b[f]?(d=this.system.app.assets.get(b[f]),this._loadAndSetMeshInstanceMaterial(d,a[f],f)):
a[f].material=pc.ModelHandler.DEFAULT_MATERIAL;else if(c)if(c[f]&&(c[f].material||c[f].path)){if(void 0!==c[f].material)d=this.system.app.assets.get(c[f].material);else if(void 0!==c[f].path){var k=this._getMaterialAssetUrl(c[f].path);k&&(d=this.system.app.assets.getByUrl(k))}this._loadAndSetMeshInstanceMaterial(d,a[f],f)}else a[f].material=pc.ModelHandler.DEFAULT_MATERIAL}});return{ModelComponent:e}}());Object.assign(pc,function(){var e=["enabled"],b=function(a){pc.ComponentSystem.call(this,a);this.id="model";this.description="Renders a 3D model at the location of the Entity.";this.ComponentType=pc.ModelComponent;this.DataType=pc.ModelComponentData;this.schema=e;this.sphere=this.plane=this.cylinder=this.cone=this.capsule=this.box=null;this.defaultMaterial=new pc.StandardMaterial;this.on("beforeremove",this.onRemove,this)};b.prototype=Object.create(pc.ComponentSystem.prototype);b.prototype.constructor=
...
d=Math.floor(c.z*this.app.graphicsDevice.width*this.renderTargetScale),c=Math.floor(c.w*this.app.graphicsDevice.height*this.renderTargetScale),f=this.app.graphicsDevice,e=a?f.getHdrFormat():pc.PIXELFORMAT_R8_G8_B8_A8,d=new pc.Texture(f,{format:e,width:d,height:c});d.minFilter=pc.FILTER_NEAREST;d.magFilter=pc.FILTER_NEAREST;d.addressU=pc.ADDRESS_CLAMP_TO_EDGE;d.addressV=pc.ADDRESS_CLAMP_TO_EDGE;return new pc.RenderTarget(this.app.graphicsDevice,d,{depth:b})},_resizeOffscreenTarget:function(b){var a=
this.camera.rect,c=Math.floor(a.z*this.app.graphicsDevice.width*this.renderTargetScale),a=Math.floor(a.w*this.app.graphicsDevice.height*this.renderTargetScale),d=this.app.graphicsDevice,f=b.colorBuffer.format;b._colorBuffer.destroy();c=new pc.Texture(d,{format:f,width:c,height:a});c.minFilter=pc.FILTER_NEAREST;c.magFilter=pc.FILTER_NEAREST;c.addressU=pc.ADDRESS_CLAMP_TO_EDGE;c.addressV=pc.ADDRESS_CLAMP_TO_EDGE;b._colorBuffer=c;b.destroy()},setRenderTargetScale:function(b){this.renderTargetScale=b;
this.resizeRenderTargets()},addEffect:function(b){var a=this.effects;b={effect:b,inputTarget:this._createOffscreenTarget(0===this.effects.length,b.hdr),outputTarget:null};if(!this.layer){this.layer=new pc.Layer({opaqueSortMode:pc.SORTMODE_NONE,transparentSortMode:pc.SORTMODE_NONE,passThrough:!0,name:"PostEffectQueue",renderTarget:this.camera.renderTarget,clear:!1,onPostRender:function(){for(var a=0;a<this._commandList.length;a++)this._commandList[a]()}});var c=this.app.scene.layers.layerList,d=0,
f,e=c.length-1;for(f=e;0<=f;f--)if(c[f].id===pc.LAYERID_UI){e=f-1;this._origOverrideClear=c[f].overrideClear;this._origClearColorBuffer=c[f].clearColorBuffer;this._origDepthColorBuffer=c[f].clearDepthBuffer;this._origStencilColorBuffer=c[f].clearStencilBuffer;c[f].overrideClear=!0;c[f].clearColorBuffer=!1;c[f].clearDepthBuffer=this.camera.clearDepthBuffer;c[f].clearStencilBuffer=this.camera.clearStencilBuffer;break}for(f=e;0<=f;f--)0<=c[f].cameras.indexOf(this.camera)&&(0===d&&(d=f+1),c[f].renderTarget=
...
h.bilinear;this.layer.postEffect=this;this.layer.shader=h.shader;this.layer.renderTarget=h.destRenderTarget;if(!a){a=q.scope.resolve("uColorBuffer");c=q.scope.resolve("uScreenSize");for(var G=q.supportsMsaa?4:1,H=0;2>H;H++)b[H]=new pc.RenderTarget({depth:!0,stencil:q.supportsStencil,samples:G,autoResolve:!1}),b[H].name="backbuffer"+H;e.on("prerender",function(){var a=e.scene.layers.layerList,c,d,f=0,h=0;p=l=k=!1;var n=pc.PIXELFORMAT_R8_G8_B8_A8;if(e.scene.layers._dirty){var D=0,G,F;for(c=0;c<a.length;c++){d=
!1;var H;if((H=a[c].isPostEffect)&&!(H=0===D)&&(H=a[c].unmodifiedUvs&&a[c].shader)){a:{var I,S,aa,M;G=a;F=g;var ba=D,L=J(a[c].shader.definition.fshader);if(0!==L.length)for(M=0;M<ba;M++)for(aa=0;aa<L.length;aa++)for(H=L[aa],I=J(G[F[M]].shader.definition.fshader),S=0;S<I.length;S++)if(I[S]===H){H=!0;break a}H=!1}H=!H}H?(g[D]=c,D++,c===a.length-1&&(d=!0)):0<D&&(d=!0);if(d){if(1<D){H="post_";for(d=0;d<D;d++)I=a[g[d]],H+=I.name?I.name:I.id,d<D-1&&(H+="_");I=q.programLib._cache[H];if(!I){S="vec4 shaderOutput;\n";
aa="void main() {\n";M=[];for(d=0;d<D;d++){I=a[g[d]].shader.definition.fshader+"\n";I=I.replace(y,"//").replace(w,"//").replace(u,"//").replace(x,"shaderOutput");0<d&&(I=I.replace(B,"//").replace(C,"//").replace(A,"shaderOutput;//"));I=I.replace(z,"void main"+d);var ca,ea,fa,L=I;F=M;ea=L.length;var ga=0,Z=ca=0,ba="";for(G=0;G<ea;G++)fa=L.charAt(G),"{"===fa?(0===ca&&(ga=G),ca++):"}"===fa&&(1===ca&&(fa=G,ba+=L.substr(Z,ga-Z+1),Z=fa),ca--);ba+=L.substr(Z,L.length-Z+1);ga=null;Z=ba.match(m)||[];for(G=
0;G<Z.length;G++)for(L=Z[G].split(","),ea=0;ea<L.length;ea++)ca=L[ea].replace(r,"").trim(),0<=F.indexOf(ca)?(ga||(ga=[]),ga.push(ca)):F.push(ca);ba=ba.match(t)||[];for(G=0;G<ba.length;G++)for(L=ba[G].split(","),ea=0;ea<L.length;ea++)ca=L[ea].replace(v,"").trim(),ca=F.indexOf(ca),0<=ca&&F.splice(ca,1);if(G=ga)for(F=0;F<G.length;F++)I=I.replace(new RegExp("\\b"+G[F]+"\\b","g"),G[F]+"NNNN"+d);S+=I;aa+="main"+d+"();\n"}aa+="gl_FragColor = shaderOutput;\n}\n";I=pc.shaderChunks.createShaderFromCode(q,pc.shaderChunks.fullscreenQuadVS,
S+aa,H)}for(d=0;d<D;d++)a[g[d]]._postEffectCombined=d===D-1?1:-1;a[g[D-1]]._postEffectCombinedShader=I;a[g[D-1]]._postEffectCombinedBilinear=a[g[0]].postEffectBilinear;a[g[D-1]]._postEffectCombinedSrc=a[g[0]].postEffect.srcRenderTarget}g[0]=c;D=1}}}for(c=0;c<a.length;c++){if(a[c].isPostEffect&&(!a[c].postEffect.srcRenderTarget&&!a[c]._postEffectCombined||!a[c].postEffect._postEffectCombinedSrc&&0<=a[c]._postEffectCombined)){for(d=c-1;d>=f;d--)a[d].renderTarget||(a[d].renderTarget=b[h]);a[c]._backbufferRtId=
...
var c=pc.makeArray(arguments).slice(2),f,e,k,l=this.store;for(f in l)l.hasOwnProperty(f)&&(e=l[f].data,e.instances[a]&&(k=e.instances[a].instance[b])&&k.apply(e.instances[a].instance,c))},_preRegisterInstance:function(a,b,d,f){if(a.script){a.script.data._instances=a.script.data._instances||{};if(a.script.data._instances[d])throw Error(pc.string.format("Script name collision '{0}'. Scripts from '{1}' and '{2}' {{3}}",d,b,a.script.data._instances[d].url,a.getGuid()));a.script.data._instances[d]={url:b,
...
if(f&&e){g=l.getCollisionFlags();var p=h.getCollisionFlags(),n=k.getNumContacts(),C=[],h=[],A;if(0<n)if(g&pc.BODYFLAG_NORESPONSE_OBJECT||p&pc.BODYFLAG_NORESPONSE_OBJECT)A=f.collision?f.collision.hasEvent("triggerenter")||f.collision.hasEvent("triggerleave"):!1,l=e.collision?e.collision.hasEvent("triggerenter")||e.collision.hasEvent("triggerleave"):!1,A&&(k=this._storeCollision(f,e))&&(!f.collision||p&pc.BODYFLAG_NORESPONSE_OBJECT||f.collision.fire("triggerenter",e)),l&&(k=this._storeCollision(e,f))&&
(!e.collision||g&pc.BODYFLAG_NORESPONSE_OBJECT||e.collision.fire("triggerenter",f));else if(A=f.collision?f.collision.hasEvent("collisionstart")||f.collision.hasEvent("collisionend")||f.collision.hasEvent("contact"):!1,l=e.collision?e.collision.hasEvent("collisionstart")||e.collision.hasEvent("collisionend")||e.collision.hasEvent("contact"):!1,(p=this.hasEvent("contact"))||A||l){for(g=0;g<n;g++){var z=k.getContactPoint(g),E=this._createContactPointFromAmmo(z);if(A||l)z=this._createReverseContactPointFromAmmo(z),
C.push(E),h.push(z);p&&(E=this._createSingleContactResult(f,e,E),this.fire("contact",E))}A&&(n=this._createContactResult(e,C),f.collision&&f.collision.fire("contact",n),(k=this._storeCollision(f,e))&&f.collision&&f.collision.fire("collisionstart",n));l&&(h=this._createContactResult(f,h),e.collision&&e.collision.fire("contact",h),(k=this._storeCollision(e,f))&&e.collision&&e.collision.fire("collisionstart",h))}}}this._cleanOldCollisions();this.contactPointPool.freeAll();this.contactResultPool.freeAll();
...
3,y=new Ammo.btVector3,x=new Ammo.btVector3,B=new Ammo.btVector3,C,A,z=k.primitive[0].base,E=new Ammo.btTriangleMesh;for(e=0;e<v;e++)k=l[z+3*e]*w,C=l[z+3*e+1]*w,A=l[z+3*e+2]*w,y.setValue(u[k],u[k+1],u[k+2]),x.setValue(u[C],u[C+1],u[C+2]),B.setValue(u[A],u[A+1],u[A+2]),E.addTriangle(y,x,B,!0);e=new Ammo.btBvhTriangleMeshShape(E,!0);w=g.node.getWorldTransform().getScale();e.setLocalScaling(new Ammo.btVector3(w.x,w.y,w.z));w=g.node.getPosition();g=g.node.getRotation();l=new Ammo.btTransform;l.setIdentity();
...
!0,this.emitter.loop=this.data.loop,this.emitter.resetTime())},isPlaying:function(){return this.data.paused?!1:this.emitter&&this.emitter.loop?!0:Date.now()<=this.emitter.endTime},rebuild:function(){var a=this.enabled;this.enabled=!1;this.emitter&&(this.emitter.rebuild(),this.emitter.meshInstance.node=this.entity,this.data.model.meshInstances=[this.emitter.meshInstance]);this.enabled=a},onDestroy:function(){var a=this.data;a.model&&(this.entity.removeChild(a.model.getGraph()),a.model.destroy(),a.model=
null);this.emitter&&(this.emitter.destroy(),this.emitter=null);for(var b=0;b<c.length;b++){var d=c[b];a[d]&&(this[d]=null)}}});return{ParticleSystemComponent:f}}());Object.assign(pc,function(){var e="enabled autoPlay numParticles lifetime rate rate2 startAngle startAngle2 loop preWarm lighting halfLambert intensity depthWrite noFog depthSoftening sort blendType stretch alignToMotion emitterShape emitterExtents emitterRadius initialVelocity wrap wrapBounds localSpace colorMapAsset normalMapAsset mesh meshAsset localVelocityGraph localVelocityGraph2 velocityGraph velocityGraph2 rotationSpeedGraph rotationSpeedGraph2 scaleGraph scaleGraph2 colorGraph colorGraph2 alphaGraph alphaGraph2 colorMap normalMap animTilesX animTilesY animNumFrames animSpeed animLoop layers".split(" "),
...
pc.Entity.prototype._sync;this.entity.setPosition=pc.Entity.prototype.setPosition;this.entity.setLocalPosition=pc.Entity.prototype.setLocalPosition},_setPosition:function(){var a=new pc.Vec3,b=new pc.Mat4;return function(c,d,f){if(!this.element.screen)return pc.Entity.prototype.setPosition.call(this,c,d,f);c instanceof pc.Vec3?a.copy(c):a.set(c,d,f);this.getWorldTransform();b.copy(this.element._screenToWorld).invert();b.transformPoint(a,this.localPosition);this._dirtifyLocal()}}(),_setLocalPosition:function(a,
b,c){a instanceof pc.Vec3?this.localPosition.copy(a):this.localPosition.set(a,b,c);a=this.element;b=this.localPosition;c=a._pivot;a._margin.x=b.x-a._calculatedWidth*c.x;a._margin.z=a._localAnchor.z-a._localAnchor.x-a._calculatedWidth-a._margin.x;a._margin.y=b.y-a._calculatedHeight*c.y;a._margin.w=a._localAnchor.w-a._localAnchor.y-a._calculatedHeight-a._margin.y;this._dirtifyLocal()},_sync:function(){var f=this.element,g=f.screen;if(g){if(f._anchorDirty){var k,n,q=0,m=1;this._parent&&this._parent.element?
(k=this._parent.element.calculatedWidth,n=this._parent.element.calculatedHeight,q=this._parent.element.pivot.x,m=this._parent.element.pivot.y):(n=g.screen.resolution,k=n.x/g.screen.scale,n=n.y/g.screen.scale);f._anchorTransform.setTranslate(k*(f.anchor.x-q),-(n*(m-f.anchor.y)),0);f._anchorDirty=!1;f._calculateLocalAnchors()}f._sizeDirty&&f._calculateSize()}this._dirtyLocal&&(this.localTransform.setTRS(this.localPosition,this.localRotation,this.localScale),k=this.localPosition,q=f._pivot,f._margin.x=
k.x-f._calculatedWidth*q.x,f._margin.z=f._localAnchor.z-f._localAnchor.x-f._calculatedWidth-f._margin.x,f._margin.y=k.y-f._calculatedHeight*q.y,f._margin.w=f._localAnchor.w-f._localAnchor.y-f._calculatedHeight-f._margin.y,this._dirtyLocal=!1);if(!g)return this._dirtyWorld&&(f._cornersDirty=!0,f._canvasCornersDirty=!0,f._worldCornersDirty=!0),pc.Entity.prototype._sync.call(this);this._dirtyWorld&&(null===this._parent?this.worldTransform.copy(this.localTransform):(this._parent.element?f._screenToWorld.mul2(this._parent.element._modelTransform,
f._anchorTransform):f._screenToWorld.copy(f._anchorTransform),f._modelTransform.mul2(f._screenToWorld,this.localTransform),g?(f._screenToWorld.mul2(g.screen._screenMatrix,f._screenToWorld),g.screen.screenSpace||f._screenToWorld.mul2(g.worldTransform,f._screenToWorld),this.worldTransform.mul2(f._screenToWorld,this.localTransform),k=f._parentWorldTransform,k.setIdentity(),(q=this._parent)&&q.element&&q!==g&&(a.setTRS(pc.Vec3.ZERO,q.getLocalRotation(),q.getLocalScale()),k.mul2(q.element._parentWorldTransform,
a)),e.set(0,0,this.localPosition.z),b.set(f._absLeft+f._pivot.x*f.calculatedWidth,f._absBottom+f._pivot.y*f.calculatedHeight,0),a.setTranslate(-b.x,-b.y,-b.z),c.setTRS(e,this.getLocalRotation(),this.getLocalScale()),d.setTranslate(b.x,b.y,b.z),f._screenTransform.mul2(f._parentWorldTransform,d).mul(c).mul(a),f._cornersDirty=!0,f._canvasCornersDirty=!0,f._worldCornersDirty=!0):this.worldTransform.copy(f._modelTransform)),this._dirtyWorld=!1)},_onInsert:function(a){a=this._parseUpToScreen();this.entity._dirtifyWorld();
...
Object.defineProperty(g.prototype,"width",{get:function(){return this._width},set:function(a){this._width=a;this._hasSplitAnchorsX||this._setCalculatedWidth(a,!0);this.fire("set:width",this._width)}});Object.defineProperty(g.prototype,"height",{get:function(){return this._height},set:function(a){this._height=a;this._hasSplitAnchorsY||this._setCalculatedHeight(a,!0);this.fire("set:height",this._height)}});Object.defineProperty(g.prototype,"calculatedWidth",{get:function(){return this._calculatedWidth},
set:function(a){this._setCalculatedWidth(a,!0)}});Object.defineProperty(g.prototype,"calculatedHeight",{get:function(){return this._calculatedHeight},set:function(a){this._setCalculatedHeight(a,!0)}});Object.defineProperty(g.prototype,"pivot",{get:function(){return this._pivot},set:function(a){var b=this._pivot.x,c=this._pivot.y;a instanceof pc.Vec2?this._pivot.set(a.x,a.y):this._pivot.set(a[0],a[1]);a=this._margin.x+this._margin.z;b=this._pivot.x-b;this._margin.x+=a*b;this._margin.z-=a*b;b=this._margin.y+
...
this._anchorDirty=!0;this.entity._dirtifyLocal();this.fire("set:anchor",this._anchor)}});Object.defineProperty(g.prototype,"_hasSplitAnchorsX",{get:function(){return.001<Math.abs(this._anchor.x-this._anchor.z)}});Object.defineProperty(g.prototype,"_hasSplitAnchorsY",{get:function(){return.001<Math.abs(this._anchor.y-this._anchor.w)}});Object.defineProperty(g.prototype,"aabb",{get:function(){return this._image?this._image.aabb:this._text?this._text.aabb:null}});Object.defineProperty(g.prototype,"screenCorners",
{get:function(){if(!this._cornersDirty||!this.screen)return this._screenCorners;var a=this.entity.parent&&this.entity.parent.element&&this.entity.parent.element.screenCorners[0];this._screenCorners[0].set(this._absLeft,this._absBottom,0);this._screenCorners[1].set(this._absRight,this._absBottom,0);this._screenCorners[2].set(this._absRight,this._absTop,0);this._screenCorners[3].set(this._absLeft,this._absTop,0);for(var b=this.screen.screen.screenSpace,c=0;4>c;c++)this._screenTransform.transformPoint(this._screenCorners[c],
this._screenCorners[c]),b&&this._screenCorners[c].scale(this.screen.screen.scale),a&&this._screenCorners[c].add(a);this._cornersDirty=!1;this._worldCornersDirty=this._canvasCornersDirty=!0;return this._screenCorners}});Object.defineProperty(g.prototype,"canvasCorners",{get:function(){if(!this._canvasCornersDirty||!this.screen||!this.screen.screen.screenSpace)return this._canvasCorners;for(var a=this.system.app.graphicsDevice,b=this.screenCorners,c=a.canvas.clientWidth/a.width,d=a.canvas.clientHeight/
a.height,f=0;4>f;f++)this._canvasCorners[f].set(b[f].x*c,(a.height-b[f].y)*d);this._canvasCornersDirty=!1;return this._canvasCorners}});Object.defineProperty(g.prototype,"worldCorners",{get:function(){if(!this._worldCornersDirty)return this._worldCorners;if(this.screen){var b=this.screenCorners;if(!this.screen.screen.screenSpace){a.copy(this.screen.screen._screenMatrix);a.data[13]=-a.data[13];a.mul2(this.screen.getWorldTransform(),a);for(var g=0;4>g;g++)a.transformPoint(b[g],this._worldCorners[g])}}else b=
this.entity.getLocalPosition(),a.setTranslate(-b.x,-b.y,-b.z),c.setTRS(pc.Vec3.ZERO,this.entity.getLocalRotation(),this.entity.getLocalScale()),d.setTranslate(b.x,b.y,b.z),f.copy(this.entity.parent.getWorldTransform()),f.mul(d).mul(c).mul(a),e.set(b.x-this.pivot.x*this.calculatedWidth,b.y-this.pivot.y*this.calculatedHeight,b.z),f.transformPoint(e,this._worldCorners[0]),e.set(b.x+(1-this.pivot.x)*this.calculatedWidth,b.y-this.pivot.y*this.calculatedHeight,b.z),f.transformPoint(e,this._worldCorners[1]),
e.set(b.x+(1-this.pivot.x)*this.calculatedWidth,b.y+(1-this.pivot.y)*this.calculatedHeight,b.z),f.transformPoint(e,this._worldCorners[2]),e.set(b.x-this.pivot.x*this.calculatedWidth,b.y+(1-this.pivot.y)*this.calculatedHeight,b.z),f.transformPoint(e,this._worldCorners[3]);this._worldCornersDirty=!1;return this._worldCorners}});Object.defineProperty(g.prototype,"textWidth",{get:function(){return this._text?this._text.width:0}});Object.defineProperty(g.prototype,"textHeight",{get:function(){return this._text?
this._text.height:0}});Object.defineProperty(g.prototype,"useInput",{get:function(){return this._useInput},set:function(a){this._useInput!==a&&(this._useInput=a,this.system.app.elementInput&&(a?this.enabled&&this.entity.enabled&&this.system.app.elementInput.addElement(this):this.system.app.elementInput.removeElement(this)),this.fire("set:useInput",a))}});Object.defineProperty(g.prototype,"batchGroupId",{get:function(){return this._batchGroupId},set:function(a){this._batchGroupId!==a&&(0<=this._batchGroupId&&
this.system.app.batcher.markGroupDirty(this._batchGroupId),0<=a&&this.system.app.batcher.markGroupDirty(a),0>a&&0<=this._batchGroupId&&this.enabled&&this.entity.enabled&&(this._image._model?this.addModelToLayers(this._image._model):this._text._model&&this.addModelToLayers(this._text._model)),this._batchGroupId=a)}});Object.defineProperty(g.prototype,"maskedBy",{get:function(){return this._maskedBy}});var k=function(a){Object.defineProperty(g.prototype,a,{get:function(){return this._text?this._text[a]:
this._image?this._image[a]:null},set:function(b){this._text?this._text[a]=b:this._image&&(this._image[a]=b)}})};k("fontSize");k("color");k("font");k("fontAsset");k("spacing");k("lineHeight");k("wrapLines");k("lines");k("alignment");k("autoWidth");k("autoHeight");k("rtlReorder");k("unicodeConverter");k("text");k("texture");k("textureAsset");k("material");k("materialAsset");k("sprite");k("spriteAsset");k("spriteFrame");k("pixelsPerUnit");k("opacity");k("rect");k("mask");return{ElementComponent:g}}());Object.assign(pc,function(){var e=["enabled"],b=function(a){pc.ComponentSystem.call(this,a);this.id="element";this.app=a;this.ComponentType=pc.ElementComponent;this.DataType=pc.ElementComponentData;this.schema=e;this._rtlReorder=this._unicodeConverter=null;this._defaultTexture=new pc.Texture(a.graphicsDevice,{width:1,height:1,format:pc.PIXELFORMAT_R8_G8_B8_A8});a=this._defaultTexture.lock();var b=new Uint8Array(4);b[0]=255;b[1]=255;b[2]=255;b[3]=255;a.set(b);this._defaultTexture.unlock();this.defaultScreenSpaceBitmapTextMaterial=
...
(this._spriteFrame=pc.math.clamp(this._spriteFrame,0,this._sprite.frameKeys.length-1));this._updateSprite()},_onSpritePpuChange:function(){this.sprite.renderMode!==pc.SPRITE_RENDERMODE_SIMPLE&&null===this._pixelsPerUnit&&this._updateSprite()},_onAtlasTextureChange:function(){this.sprite&&this.sprite.atlas&&this.sprite.atlas.texture?(this._renderable.setParameter("texture_emissiveMap",this._sprite.atlas.texture),this._renderable.setParameter("texture_opacityMap",this._sprite.atlas.texture)):(this._renderable.deleteParameter("texture_emissiveMap"),
this._renderable.deleteParameter("texture_opacityMap"))},_onTextureAtlasLoad:function(a){a=this._spriteAsset;a instanceof pc.Asset?this._onSpriteAssetLoad(a):this._onSpriteAssetLoad(this._system.app.assets.get(a))},_isScreenSpace:function(){return this._element.screen&&this._element.screen.screen?this._element.screen.screen.screenSpace:!1},onEnable:function(){var a;this._materialAsset&&(a=this._system.app.assets.get(this._materialAsset))&&a.resource!==this._material&&this._bindMaterialAsset(a);this._textureAsset&&
(a=this._system.app.assets.get(this._textureAsset))&&a.resource!==this._texture&&this._bindTextureAsset(a);this._spriteAsset&&(a=this._system.app.assets.get(this._spriteAsset))&&a.resource!==this._sprite&&this._bindSpriteAsset(a);this._element.addModelToLayers(this._renderable.model)},onDisable:function(){this._element.removeModelFromLayers(this._renderable.model)},_setStencil:function(a){this._renderable.meshInstance.stencilFront=a;this._renderable.meshInstance.stencilBack=a;a=0;this._element.maskedBy&&
(a=this._element.maskedBy.element._image._maskRef);this._renderable.unmaskMeshInstance&&(a=new pc.StencilParameters({ref:a+1,func:pc.FUNC_EQUAL,zpass:pc.STENCILOP_DECREMENT}),this._renderable.unmaskMeshInstance.stencilFront=a,this._renderable.unmaskMeshInstance.stencilBack=a)}});Object.defineProperty(b.prototype,"color",{get:function(){return this._color},set:function(a){var b=a.r,d=a.g;a=a.b;if(this._color.r!==b||this._color.g!==d||this._color.b!==a)this._color.r=b,this._color.g=d,this._color.b=
a,this._colorUniform[0]=b,this._colorUniform[1]=d,this._colorUniform[2]=a,this._renderable.setParameter("material_emissive",this._colorUniform),this._element&&this._element.fire("set:color",this._color)}});Object.defineProperty(b.prototype,"opacity",{get:function(){return this._color.a},set:function(a){a!==this._color.a&&(this._color.a=a,this._renderable.setParameter("material_opacity",a),this._element&&this._element.fire("set:opacity",a))}});Object.defineProperty(b.prototype,"rect",{get:function(){return this._rect},
set:function(a){var b,d,f;a instanceof pc.Vec4?(b=a.x,d=a.y,f=a.z,a=a.w):(b=a[0],d=a[1],f=a[2],a=a[3]);if(b!==this._rect.x||d!==this._rect.y||f!==this._rect.z||a!==this._rect.w)this._rect.set(b,d,f,a),this._renderable.mesh&&(this._element._beingInitialized?this._meshDirty=!0:this._updateMesh(this._renderable.mesh))}});Object.defineProperty(b.prototype,"material",{get:function(){return this._material},set:function(a){this._material!==a&&(a||(a=this._isScreenSpace(),a=this.mask?a?this._system.defaultScreenSpaceImageMaskMaterial:
...
this._color.a)))}});Object.defineProperty(b.prototype,"materialAsset",{get:function(){return this._materialAsset},set:function(a){var b=this._system.app.assets,d=a;a instanceof pc.Asset&&(d=a.id);this._materialAsset!==d&&(this._materialAsset&&(b.off("add:"+this._materialAsset,this._onMaterialAdded,this),a=b.get(this._materialAsset))&&(a.off("load",this._onMaterialLoad,this),a.off("change",this._onMaterialChange,this),a.off("remove",this._onMaterialRemove,this)),(this._materialAsset=d)?(d=b.get(this._materialAsset))?
this._bindMaterialAsset(d):(this.material=null,b.on("add:"+this._materialAsset,this._onMaterialAdded,this)):this.material=null)}});Object.defineProperty(b.prototype,"texture",{get:function(){return this._texture},set:function(a){if(this._texture!==a){if(this._textureAsset){var b=this._system.app.assets.get(this._textureAsset);b&&b.resource!==a&&(this.textureAsset=null)}(this._texture=a)?(this._spriteAsset&&(this.spriteAsset=null),this._renderable.setParameter("texture_emissiveMap",this._texture),
this._renderable.setParameter("texture_opacityMap",this._texture),this._colorUniform[0]=this._color.r,this._colorUniform[1]=this._color.g,this._colorUniform[2]=this._color.b,this._renderable.setParameter("material_emissive",this._colorUniform),this._renderable.setParameter("material_opacity",this._color.a)):(this._renderable.deleteParameter("texture_emissiveMap"),this._renderable.deleteParameter("texture_opacityMap"))}}});Object.defineProperty(b.prototype,"textureAsset",{get:function(){return this._textureAsset},
...
0;this._lineWidths=[];this._lineContents=[];var l=d.length,p=0,h=0,n=0,q=1,m=0,r=0,t=0,v=0,y=0,w=1E-4<=Math.abs(this._element.anchor.x-this._element.anchor.z),u=this._element.calculatedWidth;if(this.autoWidth&&!w||!this._wrapLines)u=Number.POSITIVE_INFINITY;var x=0,w=0,B=1,C,A,z;for(A in e.chars)z=e.chars[A],B=z.height/32*this._fontSize/z.height,z.bounds&&(x=Math.min(x,z.bounds[1]*B),w=Math.max(w,z.bounds[3]*B));for(A=0;A<this._meshInfo.length;A++)this._meshInfo[A].quad=0,this._meshInfo[A].lines=
{};for(A=0;A<l;A++){C=d[A];var E=0,J=0,H=0,G=1,D=0,F=0;(z=e.chars[C])&&z.scale?(E=(z.width+z.height)/2,B=E/32*this._fontSize/E,G=E/32*this._fontSize/z.scale,H=z.xadvance*B,E=z.xoffset*B,J=z.yoffset*B,z.bounds?(F=(z.bounds[2]-z.bounds[0])*B,D=z.bounds[0]*B):(F=E,D=0)):(H=1,J=E=0,G=this._fontSize);var I=b.test(C),S=c.test(C),V=a.test(C);if(I)f(A,h),r=A+1,t=A+1;else{B=this._meshInfo[z&&z.map||0];if(p+F+D>=u&&0<y&&!V)if(0===v)r=A,f(A,h);else{z=Math.max(A-r,0);if(1>=this._meshInfo.length)B.lines[q-1]-=
z,B.quad-=z;else for(C=r;C<A;C++)E=e.chars[d[C]],E=this._meshInfo[E&&E.map||0],--E.lines[q-1],--E.quad;A-=z+1;f(r,m);continue}z=B.quad;B.lines[q-1]=z;B.positions[12*z+0]=p-E;B.positions[12*z+1]=n-J;B.positions[12*z+2]=0;B.positions[12*z+3]=p-E+G;B.positions[12*z+4]=n-J;B.positions[12*z+5]=0;B.positions[12*z+6]=p-E+G;B.positions[12*z+7]=n-J+G;B.positions[12*z+8]=0;B.positions[12*z+9]=p-E;B.positions[12*z+10]=n-J+G;B.positions[12*z+11]=0;this.width=Math.max(this.width,p+F+D);this.height=Math.max(this.height,
w-(n+x));p+=this._spacing*H;V||I||(h=p);S&&(v++,m=h,r=A+1);y++;C=this._getUv(C);B.uvs[8*z+0]=C[0];B.uvs[8*z+1]=C[1];B.uvs[8*z+2]=C[2];B.uvs[8*z+3]=C[1];B.uvs[8*z+4]=C[2];B.uvs[8*z+5]=C[3];B.uvs[8*z+6]=C[0];B.uvs[8*z+7]=C[3];B.quad++}}t<l&&f(l,p);this._noResize=!0;this.autoWidth=this._autoWidth;this.autoHeight=this._autoHeight;this._noResize=!1;e=this._element.pivot.x;l=this._element.pivot.y;h=this._alignment.x;r=this._alignment.y;for(A=0;A<this._meshInfo.length;A++)if(0!==this._meshInfo[A].count){z=
0;for(var X in this._meshInfo[A].lines){u=this._meshInfo[A].lines[X];x=-e*this._element.calculatedWidth+h*(this._element.calculatedWidth-this._lineWidths[parseInt(X,10)]);for(C=(1-l)*this._element.calculatedHeight-w-(1-r)*(this._element.calculatedHeight-this.height);z<=u;z++)this._meshInfo[A].positions[12*z]+=x,this._meshInfo[A].positions[12*z+3]+=x,this._meshInfo[A].positions[12*z+6]+=x,this._meshInfo[A].positions[12*z+9]+=x,this._meshInfo[A].positions[12*z+1]+=C,this._meshInfo[A].positions[12*z+
4]+=C,this._meshInfo[A].positions[12*z+7]+=C,this._meshInfo[A].positions[12*z+10]+=C;z=u+1}u=4*this._meshInfo[A].quad;x=new pc.VertexIterator(this._meshInfo[A].meshInstance.mesh.vertexBuffer);for(z=0;z<u;z++)x.element[pc.SEMANTIC_POSITION].set(this._meshInfo[A].positions[3*z+0],this._meshInfo[A].positions[3*z+1],this._meshInfo[A].positions[3*z+2]),x.element[pc.SEMANTIC_TEXCOORD0].set(this._meshInfo[A].uvs[2*z+0],this._meshInfo[A].uvs[2*z+1]),x.next();x.end();this._meshInfo[A].meshInstance.mesh.aabb.compute(this._meshInfo[A].positions);
this._meshInfo[A].meshInstance._aabbVer=-1}this._aabbDirty=!0},_onFontAdded:function(a){this._system.app.assets.off("add:"+a.id,this._onFontAdded,this);a.id===this._fontAsset&&this._bindFont(a)},_bindFont:function(a){this._entity.enabled&&(a.on("load",this._onFontLoad,this),a.on("change",this._onFontChange,this),a.on("remove",this._onFontRemove,this),a.resource?this._onFontLoad(a):this._system.app.assets.load(a))},_unbindFont:function(a){a.off("load",this._onFontLoad,this);a.off("change",this._onFontChange,
...
1)*A.spacing[b.axis]}function q(a,b,c){var d=u(a,c.maxSize),f=w(a,c.fittingProportion),e=C(f,d);b=g[c.axis]-b;for(var k=0;k<a.length;++k){var h=d[k],l=r(h,b,f,e),m=a[h][c.size]+l,p=Math.min(m,a[h][c.maxSize]);a[h][c.size]=p;b-=l-Math.max(m-p,0)}}function m(a,b,c){var d=u(a,c.minSize,!0),f;f=w(a,c.fittingProportion);if(1===f.length)f=[1];else{for(var e=[],k=f.length,h=0;h<k;++h)e.push((1-f[h])/(k-1));f=e}e=C(f,d);b-=g[c.axis];for(k=0;k<a.length;++k){var h=d[k],l=r(h,b,f,e),m=a[h][c.size]-l,p=Math.max(m,
a[h][c.minSize]);a[h][c.size]=p;b-=l-Math.max(p-m,0)}}function r(a,b,c,d){c=c[a];a=d[a];return 1E-5>Math.abs(c)&&1E-5>Math.abs(a)?b:b*c/a}function t(a){for(var b=[],c=0;c<a.length;++c){var d=a[c],f=Math.max(v(d,"minWidth"),0),e=Math.max(v(d,"minHeight"),0),g=Math.max(v(d,"maxWidth"),f),k=Math.max(v(d,"maxHeight"),e),h;h=v(d,"width");h=Math.min(Math.max(h,f),g);var l;l=v(d,"height");l=Math.min(Math.max(l,e),k);var m=v(d,"fitWidthProportion"),d=v(d,"fitHeightProportion");b.push({minWidth:f,minHeight:e,
...
d[b]}).map(B)}function x(a,b){a.index=b}function B(a){return a.index}function C(a,b){var c=[];c[b[a.length-1]]=a[b[a.length-1]];for(var d=a.length-2;0<=d;--d)c[b[d]]=c[b[d+1]]+a[b[d]];return c}var A,z=a[b],E=a[c[b]];return function(a,b){a=a.filter(e);A=b;g.x=A.containerSize.x-A.padding.x-A.padding.z;g.y=A.containerSize.y-A.padding.y-A.padding.w;for(var c=a,d=0;d<c.length;++d){var h=c[d],p=h.anchor;if(0!==p.x||0!==p.y||0!==p.z||0!==p.w)h.anchor=pc.Vec4.ZERO}c=a;if(A.wrap){for(var d=[[]],h=t(c),p=0,
n=A[z.fitting]===pc.FITTING_SHRINK,r=0;r<c.length;++r){0<d[d.length-1].length&&(p+=A.spacing[z.axis]);var u=h[r][z.size],p=p+u;!n&&p>g[z.axis]&&0!==d[d.length-1].length&&(p=u,d.push([]));d[d.length-1].push(c[r]);n&&p>g[z.axis]&&r!==c.length-1&&(p=0,d.push([]))}c=d}else c=[c];d=A.orientation===pc.ORIENTATION_HORIZONTAL&&A.reverseX||A.orientation===pc.ORIENTATION_VERTICAL&&A.reverseY;h=A.orientation===pc.ORIENTATION_HORIZONTAL&&A.reverseY||A.orientation===pc.ORIENTATION_VERTICAL&&A.reverseX;if(d)for(p=
0;p<c.length;++p)d&&c[p].reverse();h&&c.reverse();d=[];for(h=0;h<c.length;++h)p=t(c[h]),n=l(p,z),r=k(A[z.fitting],n,g[z.axis]),r===f.APPLY_STRETCHING?q(p,n,z):r===f.APPLY_SHRINKING&&m(p,n,z),d.push(p);u=[];r=[];for(p=0;p<c.length;++p){n=c[p];n.largestElement=null;n.largestSize={width:Number.NEGATIVE_INFINITY,height:Number.NEGATIVE_INFINITY};for(h=0;h<n.length;++h){var v=d[p][h];v[E.size]>n.largestSize[E.size]&&(n.largestElement=n[h],n.largestSize=v)}u.push(n.largestElement);r.push(n.largestSize)}h=
l(r,E);p=k(A[E.fitting],h,g[E.axis]);p===f.APPLY_STRETCHING?q(r,h,E):p===f.APPLY_SHRINKING&&m(r,h,E);for(p=0;p<c.length;++p)for(n=c[p],h=0;h<n.length;++h)r=d[p][h],u=1===c.length?g[E.axis]:n.largestSize[E.size],v=k(A[E.fitting],r[E.size],u),v===f.APPLY_STRETCHING?r[E.size]=Math.min(u,r[E.maxSize]):v===f.APPLY_SHRINKING&&(r[E.size]=Math.max(u,r[E.minSize]));a:{h={};h[z.axis]=0;h[E.axis]=0;c[z.size]=Number.NEGATIVE_INFINITY;p=[];for(n=0;n<c.length;++n){r=c[n];if(0===r.length){h=void 0;break a}for(var u=
[],v=d[n],w=0;w<r.length;++w){var x=r[w],y=v[w];h[E.axis]-=-y[E.size]*x.pivot[E.axis];h[z.axis]-=-y[z.size]*x.pivot[z.axis];u[w]={};u[w][z.axis]=h[z.axis];u[w][E.axis]=h[E.axis];h[E.axis]+=-y[E.size]*x.pivot[E.axis];h[z.axis]+=y[z.size]*(1-x.pivot[z.axis])+A.spacing[z.axis]}r[z.size]=h[z.axis]-A.spacing[z.axis];r[E.size]=r.largestSize[E.size];c[z.size]=Math.max(c[z.size],r[z.size]);h[z.axis]=0;h[E.axis]+=r[E.size]+A.spacing[E.axis];p.push(u)}c[E.size]=h[E.axis]-A.spacing[E.axis];h=p}p=h;n=A.alignment[z.axis];
r=A.alignment[E.axis];u=A.padding[z.axis];v=A.padding[E.axis];for(w=0;w<c.length;++w)for(var x=c[w],y=d[w],B=p[w],C=(g[z.axis]-x[z.size])*n+u,H=(g[E.axis]-c[E.size])*r+v,J=0;J<x.length;++J){var O=(x[E.size]-y[J][E.size])*A.alignment[E.axis];B[J][z.axis]+=C;B[J][E.axis]+=H+O}for(p=0;p<c.length;++p)for(n=c[p],r=d[p],u=h[p],v=0;v<n.length;++v)w=n[v],w[z.calculatedSize]=r[v][z.size],w[E.calculatedSize]=r[v][E.size],A.orientation===pc.ORIENTATION_HORIZONTAL?w.entity.setLocalPosition(u[v][z.axis],u[v][E.axis],
w.entity.getLocalPosition().z):w.entity.setLocalPosition(u[v][E.axis],u[v][z.axis],w.entity.getLocalPosition().z);d=c.width;c=c.height;return{bounds:new pc.Vec4((g.x-d)*A.alignment.x+A.padding.x,(g.y-c)*A.alignment.y+A.padding.y,d,c)}}}var a={};a[pc.ORIENTATION_HORIZONTAL]={axis:"x",size:"width",calculatedSize:"calculatedWidth",minSize:"minWidth",maxSize:"maxWidth",fitting:"widthFitting",fittingProportion:"fitWidthProportion"};a[pc.ORIENTATION_VERTICAL]={axis:"y",size:"height",calculatedSize:"calculatedHeight",
...
(c=!0);a._beingEnabled=!0;a._onHierarchyStateChanged(b);a._onHierarchyStatePostChanged&&this._app._enableList.push(a);var f,e,k=a._children;f=0;for(e=k.length;f<e;f++)k[f]._enabled&&this._notifyHierarchyStateChanged(k[f],b);a._beingEnabled=!1;if(c){f=0;for(e=this._app._enableList.length;f<e;f++)this._app._enableList[f]._onHierarchyStatePostChanged();this._app._enableList.length=0}};b.prototype._onHierarchyStateChanged=function(a){pc.GraphNode.prototype._onHierarchyStateChanged.call(this,a);var b,
...
this._device.getBoneLimit(),pc.partitionSkin(c,null,l));for(l=0;l<c.skins.length;l++){var h=c.skins[l],n=[];for(p=0;p<h.inverseBindMatrices.length;p++){var q=h.inverseBindMatrices[p];n[p]=(new pc.Mat4).set(q)}h=new pc.Skin(this._device,n,h.boneNames);d.push(h);n=new pc.SkinInstance(h);q=[];for(p=0;p<h.boneNames.length;p++){var m=b[0].findByName(h.boneNames[p]);q.push(m)}n.bones=q;e.push(n)}return{skins:d,instances:e}},_parseMorphs:function(a,b){var c=a.model,d=[],e=[],l,p,h,n,q;if(c.morphs)for(l=
0;l<c.morphs.length;l++){h=c.morphs[l].targets;q=[];for(p=0;p<h.length;p++){var m=h[p].aabb;n=m.min;m=m.max;n=new pc.BoundingBox(new pc.Vec3(.5*(m[0]+n[0]),.5*(m[1]+n[1]),.5*(m[2]+n[2])),new pc.Vec3(.5*(m[0]-n[0]),.5*(m[1]-n[1]),.5*(m[2]-n[2])));n=new pc.MorphTarget({indices:h[p].indices,deltaPositions:h[p].deltaPositions,deltaNormals:h[p].deltaNormals,name:h[p].name,aabb:n});q.push(n)}p=new pc.Morph(q);d.push(p);p=new pc.MorphInstance(p);e.push(p)}return{morphs:d,instances:e}},_calculateTangentsMorphTarget:function(a,
b,e,g,k,l,p,h){var c,d,f,r,t,v,y,w,u,x,B,C,A,z,E,J,H,G,D,F;J=g.length/3;for(F=0;F<J;F++)H=g[3*F],G=g[3*F+1],D=g[3*F+2],y=a[3*H],t=a[3*H+1],v=a[3*H+2],r=a[3*G],w=a[3*G+1],u=a[3*G+2],x=a[3*D],B=a[3*D+1],C=a[3*D+2],A=e[2*H],c=e[2*H+1],z=e[2*G],f=e[2*G+1],E=e[2*D],d=e[2*D+1],r-=y,y=x-y,w-=t,t=B-t,u-=v,v=C-v,z-=A,A=E-A,f-=c,C=d-c,c=z*C-A*f,0==c?(c=0,d=1,f=0,r=1,v=t=0):(E=1/c,c=(C*r-f*y)*E,d=(C*w-f*t)*E,f=(C*u-f*v)*E,r=(z*y-A*r)*E,t=(z*t-A*w)*E,v=(z*v-A*u)*E),k[3*H+0]+=c,k[3*H+1]+=d,k[3*H+2]+=f,k[3*G+0]+=
c,k[3*G+1]+=d,k[3*G+2]+=f,k[3*D+0]+=c,k[3*D+1]+=d,k[3*D+2]+=f,l[3*H+0]+=r,l[3*H+1]+=t,l[3*H+2]+=v,l[3*G+0]+=r,l[3*G+1]+=t,l[3*G+2]+=v,l[3*D+0]+=r,l[3*D+1]+=t,l[3*D+2]+=v;c=p.length;for(D=0;D<c;D++)F=p[D],u=b[3*F],d=b[3*F+1],w=b[3*F+2],a=k[3*F],e=k[3*F+1],g=k[3*F+2],J=l[3*F],H=l[3*F+1],G=l[3*F+2],v=u*a+d*e+w*g,y=u*v,t=d*v,v*=w,r=d*g-e*w,w=w*a-g*u,u=u*e-a*d,a-=y,e-=t,g-=v,v=1/Math.sqrt(a*a+e*e+g*g),a*=v,e*=v,g*=v,h[4*F]=a,h[4*F+1]=e,h[4*F+2]=g,h[4*F+3]=0>r*J+w*H+u*G?-1:1;return h},_initMorphs:function(a,
b,e,g){a=a.model;var c,d,f,h,n,q,m,r,t,v,y,w,u,x,B,C=[],A=[];for(e=0;e<g.length;e++)if(t=a.meshes[e].vertices,!A[t]&&(d=a.vertices[t],d.tangent)){var z=new Float32Array(d.tangent.data);A[t]=!0;if(d.position&&d.normal&&d.texCoord0){var E=[];for(c=0;c<a.meshes.length;c++)a.meshes[c].vertices===t&&(E=E.concat(a.meshes[c].indices));t=d.position.data;v=d.normal.data;y=d.texCoord0.data;w=t.length/3;u=E.length;var J=new Float32Array(4*w),H=new Float32Array(3*w),G=new Float32Array(3*w);x=new Float32Array(3*
w);x.set(t);B=new Float32Array(3*w);B.set(v);for(c=0;c<b.length;c++)if(a.meshes[e].morph===c)for(f=0;f<b[c]._targets.length;f++){d=b[c]._targets[f];var D=d.indices,F=D.length;if(0!==F){d.deltaTangents=new Float32Array(4*F);if(!r||r.length<w)r=new Uint8Array(w);else for(h=0;h<w;h++)r[h]=0;for(h=0;h<F;h++)n=D[h],r[n]=1;var I=0;for(h=0;h<u;h+=3)if(n=E[h],q=E[h+1],m=E[h+2],r[n]||r[q]||r[m])C[I]=n,C[I+1]=q,C[I+2]=m,I+=3;C.length=I;q=d.deltaPositions;m=d.deltaNormals;for(h=0;h<F;h++)n=D[h],x[3*n]+=q[3*
h],x[3*n+1]+=q[3*h+1],x[3*n+2]+=q[3*h+2],B[3*n]+=m[3*h],B[3*n+1]+=m[3*h+1],B[3*n+2]+=m[3*h+2];this._calculateTangentsMorphTarget(x,B,y,C,H,G,D,J);q=d.deltaTangents;for(h=0;h<F;h++)n=D[h],q[4*h]=J[4*h]-z[4*n],q[4*h+1]=J[4*h+1]-z[4*n+1],q[4*h+2]=J[4*h+2]-z[4*n+2],q[4*h+3]=J[4*h+3]-z[4*n+3];if(f!==b[c]._targets.length-1){for(h=0;h<u;h+=3)n=E[h],q=E[h+1],m=E[h+2],H[3*n+0]=0,H[3*n+1]=0,H[3*n+2]=0,H[3*q+0]=0,H[3*q+1]=0,H[3*q+2]=0,H[3*m+0]=0,H[3*m+1]=0,H[3*m+2]=0,G[3*n+0]=0,G[3*n+1]=0,G[3*n+2]=0,G[3*q+0]=
0,G[3*q+1]=0,G[3*q+2]=0,G[3*m+0]=0,G[3*m+1]=0,G[3*m+2]=0;for(h=0;h<F;h++)n=d.indices[h],x[3*n]=t[3*n],x[3*n+1]=t[3*n+1],x[3*n+2]=t[3*n+2],B[3*n]=v[3*n],B[3*n+1]=v[3*n+1],B[3*n+2]=v[3*n+2]}}}}}},_parseVertexBuffers:function(a){a=a.model;var c=[],e,g,k={position:pc.SEMANTIC_POSITION,normal:pc.SEMANTIC_NORMAL,tangent:pc.SEMANTIC_TANGENT,blendWeight:pc.SEMANTIC_BLENDWEIGHT,blendIndices:pc.SEMANTIC_BLENDINDICES,color:pc.SEMANTIC_COLOR,texCoord0:pc.SEMANTIC_TEXCOORD0,texCoord1:pc.SEMANTIC_TEXCOORD1,texCoord2:pc.SEMANTIC_TEXCOORD2,
texCoord3:pc.SEMANTIC_TEXCOORD3,texCoord4:pc.SEMANTIC_TEXCOORD4,texCoord5:pc.SEMANTIC_TEXCOORD5,texCoord6:pc.SEMANTIC_TEXCOORD6,texCoord7:pc.SEMANTIC_TEXCOORD7},l,p;for(l=0;l<a.vertices.length;l++){var h=a.vertices[l],n=[];for(g in h)e=h[g],n.push({semantic:k[g],components:e.components,type:b[e.type],normalize:k[g]===pc.SEMANTIC_COLOR});e=new pc.VertexFormat(this._device,n);var n=h.position.data.length/h.position.components,q=new pc.VertexBuffer(this._device,e,n),m=new pc.VertexIterator(q);for(p=
0;p<n;p++){for(g in h)switch(e=h[g],e.components){case 1:m.element[k[g]].set(e.data[p]);break;case 2:m.element[k[g]].set(e.data[2*p],e.data[2*p+1]);break;case 3:m.element[k[g]].set(e.data[3*p],e.data[3*p+1],e.data[3*p+2]);break;case 4:m.element[k[g]].set(e.data[4*p],e.data[4*p+1],e.data[4*p+2],e.data[4*p+3])}m.next()}m.end();c.push(q)}return c},_parseIndexBuffers:function(a,b){var c=a.model,d=null,e=null,l,p=0;for(l=0;l<c.meshes.length;l++){var h=c.meshes[l];void 0!==h.indices&&(p+=h.indices.length)}for(l=
c=0;l<b.length;l++)c=Math.max(c,b[l].numVertices);0<p&&(65535<c&&this._device.extUintElement?(d=new pc.IndexBuffer(this._device,pc.INDEXFORMAT_UINT32,p),e=new Uint32Array(d.lock())):(d=new pc.IndexBuffer(this._device,pc.INDEXFORMAT_UINT16,p),e=new Uint16Array(d.lock())));return{buffer:d,data:e}},_parseMeshes:function(a,b,f,g,k,l){a=a.model;var c=[],d=0,n;for(n=0;n<a.meshes.length;n++){var q=a.meshes[n],m=q.aabb,r=m.min,m=m.max,r=new pc.BoundingBox(new pc.Vec3(.5*(m[0]+r[0]),.5*(m[1]+r[1]),.5*(m[2]+
r[2])),new pc.Vec3(.5*(m[0]-r[0]),.5*(m[1]-r[1]),.5*(m[2]-r[2]))),m=void 0!==q.indices,t=new pc.Mesh;t.vertexBuffer=g[q.vertices];t.indexBuffer[0]=m?k:null;t.primitive[0].type=e[q.type];t.primitive[0].base=m?q.base+d:q.base;t.primitive[0].count=q.count;t.primitive[0].indexed=m;t.skin=void 0!==q.skin?b[q.skin]:null;t.morph=void 0!==q.morph?f[q.morph]:null;t.aabb=r;m&&(l.set(q.indices,d),d+=q.indices.length);c.push(t)}null!==k&&k.unlock();return c},_parseMeshInstances:function(a,b,e,g,k,l,p){a=a.model;
...
0;a<this._immediateData.layers.length;a++)this._immediateData.layers[a].clearMeshInstances(!0);this._immediateData.layers.length=0}}}());Object.assign(pc,function(){function e(a,b,c,d){if(a.enabled){var f;if(a.model&&a.model.model&&a.model.enabled&&(d&&d.push(a),a.model.lightmapped&&b)){var g=!0,k=a.model.model.meshInstances;for(f=0;f<k.length;f++)if(!k[f].mesh.vertexBuffer.format.hasUv1){g=!1;break}if(g){var h,l=[];for(f=0;f<k.length;f++){h=!1;for(g=0;g<k.length;g++)f!==g&&k[f].mesh===k[g].mesh&&(h=!0);h?(b.push(a),c.push([k[f]])):l.push(k[f])}0<l.length&&(b.push(a),c.push(l))}}for(f=0;f<a._children.length;f++)e(a._children[f],b,
c,d)}}var b=[],a=[],c,d=new pc.Vec3,f=new pc.BoundingBox,g=new pc.BoundingBox,k={},l=["texture_lightMap","texture_dirLightMap"],p=[],h=function(a,b,c,d,e){this.device=a;this.root=b;this.scene=c;this.renderer=d;this.assets=e};Object.assign(h.prototype,{calculateLightmapSize:function(a){var b,c=this.scene.lightmapSizeMultiplier||16,e=1,f=1,g=1,k=1;a.model.asset?(b=this.assets.get(a.model.asset).data,b.area&&(e=b.area.x,f=b.area.y,g=b.area.z,k=b.area.uv)):a.model._area&&(b=a.model,b._area&&(e=b._area.x,
f=b._area.y,g=b._area.z,k=b._area.uv));b=a.model.lightmapSizeMultiplier||1;e*=b;f*=b;g*=b;d.copy(a.localScale);for(a=a._parent;a;)d.mul(a.localScale),a=a._parent;d.x=Math.abs(d.x);d.y=Math.abs(d.y);d.z=Math.abs(d.z);e=e*d.y*d.z+f*d.x*d.z+g*d.x*d.y;e=Math.sqrt(e/k);return Math.min(pc.math.nextPowerOfTwo(e*c),this.scene.lightmapMaxResolution||2048)},bake:function(h,q){var m,n,t=this.device,v=this.scene,y=1;void 0===q&&(q=pc.BAKE_COLORDIR);q===pc.BAKE_COLORDIR&&(y=2);var w,u=[],x=[];if(h){var B;for(m=
a.length-1;0<=m;m--)for(n=0;n<h.length;n++)if(a[m]===h[n]){for(B=0;B<b[m].length;B++)b[m][B].destroy();b.splice(m,1);a.splice(m,1)}B=[];for(m=0;m<h.length;m++)e(h[m],B,x);h=B;e(this.root,null,null,u)}else{for(m=0;m<b.length;m++)for(n=0;n<b[m].length;n++)b[m][n].destroy();b=[];a=[];h=[];e(this.root,h,x,u)}if(0===h.length)t.fire("lightmapper:end",{timestamp:pc.now(),target:this});else{B=!1;v._needsStaticPrepare&&(v._needsStaticPrepare=!1,B=!0);var C=[],A=[[],[]],z={},E,J;n=new pc.Texture(this.device,
{width:4,height:4,format:pc.PIXELFORMAT_R8_G8_B8_A8,rgbm:!0});for(m=0;m<h.length;m++){E=this.calculateLightmapSize(h[m]);C.push(E);for(w=0;w<y;w++)J=new pc.Texture(t,{width:E,height:E,format:pc.PIXELFORMAT_R8_G8_B8_A8,mipmaps:!1,rgbm:0===w,minFilter:pc.FILTER_NEAREST,magFilter:pc.FILTER_NEAREST}),A[w].push(J);if(!z[E]){var H=new pc.Texture(t,{width:E,height:E,format:pc.PIXELFORMAT_R8_G8_B8_A8,mipmaps:!1,rgbm:!0,minFilter:pc.FILTER_NEAREST,magFilter:pc.FILTER_NEAREST}),H=new pc.RenderTarget(t,H,{depth:!1});
z[E]=H}}var G=v.layers;G._update();C=[];E=[];var H=[],D=[],F=G._lights;for(m=0;m<F.length;m++)F[m]._enabled&&(J=F[m]._mask,0!==(J&4)&&(E.push(J),H.push(F[m].shadowUpdateMode),F[m]._mask=4294967295,F[m].shadowUpdateMode=F[m]._type===pc.LIGHTTYPE_DIRECTIONAL?pc.SHADOWUPDATE_REALTIME:pc.SHADOWUPDATE_THISFRAME,C.push(F[m]),F[m].isStatic=!1)),D.push(F[m]._enabled),F[m].enabled=!1;var I=pc.shaderChunks,S="#define UV1LAYOUT\n"+I.transformVS,V=I.bakeLmEndPS;J=I.createShaderFromCode(t,I.fullscreenQuadVS,I.dilatePS,
"lmDilate");var X=t.scope.resolve("source"),W=t.scope.resolve("pixelOffset"),P=t.scope.resolve("bakeDir"),U=new Float32Array(2),G=G._meshInstances;for(m=0;m<G.length;m++)G[m].node&&G[m].node.getWorldTransform();var G=v.fog,N=v.ambientLight.r,R=v.ambientLight.g,Q=v.ambientLight.b;v.fog=pc.FOG_NONE;v.ambientLight.set(0,0,0);c||(c=new pc.Camera,c._node=new pc.GraphNode,c.clearColor[0]=0,c.clearColor[1]=0,c.clearColor[2]=0,c.clearColor[3]=0,c.clearDepth=1,c.clearFlags=pc.CLEARFLAG_COLOR,c.clearStencil=
null,c.frustumCulling=!1);var K,da,O,T,aa=[];aa.length=a.length;var M;for(K=0;K<u.length;K++){O=u[K].model.model.meshInstances;M=[];for(m=0;m<O.length;m++)M.push(O[m]._shaderDefs),O[m]._shaderDefs&=~(pc.SHADERDEF_LM|pc.SHADERDEF_DIRLM);for(m=0;m<a.length;m++)if(a[m]===u[K]){aa[m]=M;break}}M=[];var ba=[],L;for(K=0;K<u.length;K++)if(M[K]=u[K].model.castShadows,u[K].model.castShadows=u[K].model.castShadowsLightmap,u[K].model.castShadowsLightmap)for(L=u[K].model.meshInstances,m=0;m<L.length;m++)L[m].visibleThisFrame=
!0,ba.push(L[m]);this.renderer.updateCpuSkinMatrices(ba);this.renderer.gpuUpdate(ba);var ca=[],ea=[];L=[[],[]];var fa,ga,Z,ia=[];ia.length=h.length;for(w=0;w<y;w++)p[w]||(m=new pc.StandardMaterial,m.chunks.transformVS=S,0===w?(m.chunks.endPS=V,m.ambient=new pc.Color(0,0,0),m.ambientTint=!0,m.lightMap=n):(m.chunks.basePS=I.basePS+"\nuniform sampler2D texture_dirLightMap;\nuniform float bakeDir;\n",m.chunks.endPS=I.bakeDirLmEndPS),m.chunks.outputAlphaPS="\n",m.chunks.outputAlphaOpaquePS="\n",m.chunks.outputAlphaPremulPS=
...
k[b])}};a.prototype.prepare=function(a,b,f){if(0===a.length)return[];void 0===f&&(f=Number.POSITIVE_INFINITY);f*=.5;for(var c=this.device.supportsBoneTextures?1024:this.device.boneLimit,d,l,p,h,n,q,m,r,t,v=new pc.BoundingBox,y=new pc.BoundingBox,w=[],u=0,x=a,B,C;0<x.length;){w[u]=[];B=[];d=x[0].material;l=x[0].layer;t=x[0]._shaderDefs;h=x[0].parameters;r=x[0]._staticLightList;p=x[0].mesh.vertexBuffer.getNumVertices();v.copy(x[0].aabb);for(a=0;a<x.length;a++){if(0<a){if(d!==x[a].material){B.push(x[a]);
continue}if(l!==x[a].layer){B.push(x[a]);continue}if(t!==x[a]._shaderDefs){B.push(x[a]);continue}if(65535<p+x[a].mesh.vertexBuffer.getNumVertices()){B.push(x[a]);continue}y.copy(v);y.add(x[a].aabb);if(y.halfExtents.x>f||y.halfExtents.y>f||y.halfExtents.z>f){B.push(x[a]);continue}n=x[a].parameters;m=!1;for(q in h)if(h.hasOwnProperty(q)&&!e(h[q],n[q])){m=!0;break}if(!m)for(q in n)if(n.hasOwnProperty(q)&&!e(n[q],h[q])){m=!0;break}if(m){B.push(x[a]);continue}n=x[a]._staticLightList;if(r&&!n||!r&&n){B.push(x[a]);
continue}if(r&&n){m=!1;for(C=0;C<r.length;C++)if(0>n.indexOf(r[C])){m=!0;break}for(C=0;C<n.length;C++)if(0>r.indexOf(n[C])){m=!0;break}if(m){B.push(x[a]);continue}}}v.add(x[a].aabb);p+=x[a].mesh.vertexBuffer.getNumVertices();w[u].push(x[a]);if(b&&w[u].length===c){B=a===x.length?[]:x.slice(a+1);break}}u++;x=B}return w};a.prototype.create=function(a,d,e){this._init||(this.transformVS="#define BONE_LIMIT "+this.device.getBoneLimit()+"\n#define DYNAMICBATCH\n"+pc.shaderChunks.transformVS,this.skinTexVS=
...
h[f].name===pc.SEMANTIC_TEXCOORD1?t=!0:h[f].name===pc.SEMANTIC_TANGENT?v=!0:h[f].name===pc.SEMANTIC_COLOR&&(y=!0);u+=p.primitive[0].count}if(q){var x=3+(m?3:0)+(r?2:0)+(t?2:0)+(v?4:0)+(y?1:0)+(d?1:0),B=m?6:3,C=(m?6:3)+(r?2:0),A=(m?6:3)+(r?2:0)+(t?2:0),z=(m?6:3)+(r?2:0)+(t?2:0)+(v?4:0),E=(m?6:3)+(r?2:0)+(t?2:0)+(v?4:0)+(y?1:0),J=new ArrayBuffer(w*x*4);q=new Float32Array(J);var H=new Uint8Array(J),J=new pc.IndexBuffer(this.device,pc.INDEXFORMAT_UINT16,u,pc.BUFFER_STATIC),G=new Uint16Array(J.lock()),
D,F,I,S=0,V=0;I=0;var X,W,P,U,N,R,Q,K;d||(K=new pc.Vec3);for(c=0;c<a.length;c++){p=a[c].mesh;h=p.vertexBuffer.format.elements;n=p.vertexBuffer.numVertices;f=p.vertexBuffer.format.size;D=f/4;for(f=0;f<h.length;f++)h[f].name===pc.SEMANTIC_POSITION?X=h[f].offset/4:h[f].name===pc.SEMANTIC_NORMAL?W=h[f].offset/4:h[f].name===pc.SEMANTIC_TEXCOORD0?P=h[f].offset/4:h[f].name===pc.SEMANTIC_TEXCOORD1?U=h[f].offset/4:h[f].name===pc.SEMANTIC_TANGENT?N=h[f].offset/4:h[f].name===pc.SEMANTIC_COLOR&&(R=h[f].offset/
4);h=new Float32Array(p.vertexBuffer.storage);F=new Uint8Array(p.vertexBuffer.storage);if(d)for(f=0;f<n;f++)q[f*x+I]=h[f*D+X],q[f*x+I+1]=h[f*D+X+1],q[f*x+I+2]=h[f*D+X+2],m&&(q[f*x+I+3]=h[f*D+W],q[f*x+I+3+1]=h[f*D+W+1],q[f*x+I+3+2]=h[f*D+W+2]),r&&(q[f*x+I+B]=h[f*D+P],q[f*x+I+B+1]=h[f*D+P+1]),t&&(q[f*x+I+C]=h[f*D+U],q[f*x+I+C+1]=h[f*D+U+1]),v&&(q[f*x+I+A]=h[f*D+N],q[f*x+I+A+1]=h[f*D+N+1],q[f*x+I+A+2]=h[f*D+N+2],q[f*x+I+A+3]=h[f*D+N+3]),y&&(H[f*x*4+4*I+4*z]=F[f*D*4+4*R],H[f*x*4+4*I+4*z+1]=F[f*D*4+4*
R+1],H[f*x*4+4*I+4*z+2]=F[f*D*4+4*R+2],H[f*x*4+4*I+4*z+3]=F[f*D*4+4*R+3]),q[f*x+E+I]=c;else for(Q=a[c].node.getWorldTransform(),f=0;f<n;f++)K.set(h[f*D+X],h[f*D+X+1],h[f*D+X+2]),Q.transformPoint(K,K),q[f*x+I]=K.x,q[f*x+I+1]=K.y,q[f*x+I+2]=K.z,m&&(K.set(h[f*D+W],h[f*D+W+1],h[f*D+W+2]),Q.transformVector(K,K),q[f*x+I+3]=K.x,q[f*x+I+3+1]=K.y,q[f*x+I+3+2]=K.z),r&&(q[f*x+I+B]=h[f*D+P],q[f*x+I+B+1]=h[f*D+P+1]),t&&(q[f*x+I+C]=h[f*D+U],q[f*x+I+C+1]=h[f*D+U+1]),v&&(K.set(h[f*D+N],h[f*D+N+1],h[f*D+N+2]),Q.transformVector(K,
K),q[f*x+I+A]=K.x,q[f*x+I+A+1]=K.y,q[f*x+I+A+2]=K.z,q[f*x+I+A+3]=h[f*D+N+3]),y&&(H[f*x*4+4*I+4*z]=F[f*D*4+4*R],H[f*x*4+4*I+4*z+1]=F[f*D*4+4*R+1],H[f*x*4+4*I+4*z+2]=F[f*D*4+4*R+2],H[f*x*4+4*I+4*z+3]=F[f*D*4+4*R+3]);I=p.primitive[0].base;D=p.primitive[0].count;p=new Uint16Array(p.indexBuffer[0].storage);for(f=0;f<D;f++)G[f+V]=p[I+f]+S;V+=D;S+=n;I=S*x}a=0;m&&(a|=2);r&&(a|=4);t&&(a|=8);v&&(a|=16);y&&(a|=32);d&&(a|=64);c=this.vertexFormats[a];c||(c=[],c.push({semantic:pc.SEMANTIC_POSITION,components:3,
